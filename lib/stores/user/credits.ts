...
export const useUserCreditsStore = create<CreditsStoreState>(() => ({
  ...
  consume: (kind, amount) => {
    ...
    const bucket = (sub as Record<CreditKey, { allotted?: number; used?: number }>)[key];
    const remaining = Math.max(0, (bucket?.allotted ?? 0) - (bucket?.used ?? 0));
    const delta = Math.min(remaining, amt);
    if (delta === 0) return false;
    useUserProfileStore.setState(
      produce((state) => {
        const s = state.userProfile?.subscription as Record<CreditKey, { allotted?: number; used?: number }> | undefined;
        if (!s) return;
  ...
  refund: (kind, amount) => {
    ...
    useUserProfileStore.setState(
      produce((state) => {
        const sub = state.userProfile?.subscription as Record<CreditKey, { allotted?: number; used?: number }> | undefined;
        if (!sub) return;
        const key = keyOf(kind);
