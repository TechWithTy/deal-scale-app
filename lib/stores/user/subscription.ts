...
export const useUserSubscriptionStore = create<SubscriptionState>(() => ({
	...
	planName: () =>
		useUserProfileStore.getState().userProfile?.subscription?.name ?? "",

	status: () =>
		useUserProfileStore.getState().userProfile?.subscription?.status ??
		"unknown",

	renewalDate: () =>
		useUserProfileStore.getState().userProfile?.subscription?.renewalDate ??
		null,
