...
export const useCompanyStore = create<CompanyStoreState>(() => ({
  getCompanyInfo: () => useUserProfileStore.getState().userProfile?.companyInfo ?? null,
  ...
  disconnect: (platform) => {
    const cur = useUserProfileStore.getState().userProfile;
    if (!cur) return;
    const next = { ...cur.connectedAccounts } as Required<typeof cur>["connectedAccounts"];
    // eslint-disable-next-line @typescript-eslint/no-dynamic-delete
    delete (next as Record<string, unknown>)[platform];
    useUserProfileStore.getState().updateUserProfile({ connectedAccounts: next });
  },
}));
