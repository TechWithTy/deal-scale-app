...
function itemsByType(profile?: UserProfile | null, k?: ChannelKey): AnyCampaign[] {
  const c = profile?.companyInfo?.campaigns ?? MockUserProfile?.companyInfo?.campaigns;
  if (!c) return [];
  switch (k) {
    case "text":
...
export const useUserCampaignReportsStore = create<CampaignReportsState>((set, get) => ({
  refreshFromProfile: () => {
  ...
  dmSummary: () => {
    ...
    return emails.reduce<DMSummary>(
      ...
      { recipients: 0, sent: 0, delivered: 0, opened: 0, bounced: 0, failed: 0 },
    );
  },
