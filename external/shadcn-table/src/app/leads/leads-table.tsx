...
export function LeadsTable() {
	...
	const [leadLists, setLeadLists] = React.useState<
	...
	return (
		<>
			<DataTable table={table} onRowClick={(row) => carousel.openAt(row)}>
				<DataTableAdvancedToolbar table={table}>
					...
					<DataTableExportButton
						table={table}
						filename="leads"
						excelZipItems={excelZipItems}
					/>
					<div className="ml-auto flex items-center gap-2">
						<Button
							type="button"
							variant="outline"
							onClick={() => setIsAddLeadListOpen(true)}
						>
							Add Lead List
						</Button>
						...
						<Button
							type="button"
							variant="outline"
							onClick={() => setIsSkipTraceOpen(true)}
						>
							Skip Trace
						</Button>
					</div>
			...
			<DataTableRowModalCarousel
				...
				render={(row) => (
					<div className="grid gap-2">
						...
						<div className="text-sm">
							<strong>Nested:</strong> {row.original.nested?.length ?? 0} items
						</div>
			...
			/>
			{/* Modals */}
			<AddLeadListModalMain
				isOpen={isAddLeadListOpen}
				onClose={() => setIsAddLeadListOpen(false)}
