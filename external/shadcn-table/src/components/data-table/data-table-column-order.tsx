...
export function DataTableColumnOrder<TData>({
	...
	const reorderable = React.useMemo(
		() =>
			allColumns
				.filter(
					(c) => c.getCanHide() && c.id !== "select" && c.id !== "actions",
				)
				.map((c) => ({
					id: c.id,
	...
	return (
		...
		>
			<Popover open={open} onOpenChange={setOpen}>
				...
				<PopoverContent
					className="flex w-full max-w-[var(--radix-popover-content-available-width)] origin-[var(--radix-popover-content-transform-origin)] flex-col gap-3.5 p-4 sm:min-w-[360px]"
					align="end"
					{...props}
				>
