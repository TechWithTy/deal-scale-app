...
export function generateSocialCampaignData(): CallCampaign[] {
	...
	return base.map((r, i) => {
		...
		if (isFacebook) {
			...
			const interactionsDetails: SocialInteraction[] = Array.from({
				length: interactionsCount,
			}).map((_, idx) => {
				const id = `fb_${i + 1}_${idx + 1}`;
				const type = FB_INTERACTION_TYPES[
					idx % FB_INTERACTION_TYPES.length
				] as string;
				const user = `user_${(i * 37 + idx * 11) % 1000}`;
				const text =
					type === "reaction" ? "👍 Like" : `Sample ${type} #${idx + 1}`;
				const createdAt = new Date(now - (idx + 1) * hour).toISOString();
				const assetFilename = idx % 4 === 0 ? `asset_${id}.txt` : undefined;
				const assetContent = assetFilename
					? `Asset for ${id} - ${text}`
					: undefined;
			...
			const fb: Partial<SocialCampaign> = {
				...
				manychatGrowthTools: Array.from({ length: (i % 4) + 1 }).map(
					(_, k) => ({
						id: 10_000 + i * 10 + k,
				...
				subscribers: Array.from({ length: (i % 5) + 1 }).map((_, k) => ({
					...
					phone:
						k % 3 === 0
							? `+1-555-01${(k + 1).toString().padStart(2, "0")}`
							: undefined,
					tags:
						k % 2 === 0
							? [{ id: 1, name: "Lead" }]
							: [{ id: 2, name: "Prospect" }],
					customFields: [{ id: 10, name: "Source", type: "text", value: "FB" }],
				...
				facebookSubscriberId:
					i % 4 !== 0 ? `sub_${(i * 12345) % 1_000_000}` : undefined,
				facebookExternalId:
					i % 5 === 0 ? `${(i * 99991) % 1_000_000}` : undefined,
		...
		const interactionsDetails: SocialInteraction[] = Array.from({
			...
			const type = LI_INTERACTION_TYPES[
				idx % LI_INTERACTION_TYPES.length
			] as string;
							...
							attachments: [
								{
									...
									url_expires_at: Date.now() + 86_400_000,
									type: "img",
		...
		const li: Partial<SocialCampaign> = {
			...
			linkedinChatId:
				i % 3 === 0 ? `li_chat_${(i * 7777) % 1_000_000}` : undefined,
			linkedinProfileUrl:
				i % 5 === 0
					? `https://www.linkedin.com/in/user${(i * 13) % 10000}/`
					: undefined,
