...
export function mapManyChatSubscriber(
	s: ManyChatSubscriberAPI,
): InternalSubscriber {
	const firstName = s.first_name?.trim();
	const lastName = s.last_name?.trim();
	const name =
		(
			s.name?.trim() || [firstName, lastName].filter(Boolean).join(" ")
		).trim() || s.id;
	return {
		id: String(s.id),
		name,
		...
		tags: Array.isArray(s.tags)
			? s.tags.map((t) => ({ id: Number(t.id), name: String(t.name) }))
			: undefined,
		customFields: Array.isArray(s.custom_fields)
			? s.custom_fields.map((cf) => ({
					id: Number(cf.id),
					name: String(cf.name),
					type: String(cf.type),
					value: cf.value,
...
export function mapManyChatPage(p: ManyChatPageAPI): {
	id: number | string;
	name?: string;
} {
