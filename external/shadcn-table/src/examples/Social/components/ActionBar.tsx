...
export const ActionBar: React.FC<ActionBarProps> = ({
	...
	function canSendFB(row: SocialRow): boolean {
		...
		const hasAudience =
			hasValue(row.facebookSubscriberId) || hasValue(row.facebookExternalId);
		const hasFlow =
			hasValue(row.manychatFlowId) || hasValue(row.manychatFlowName);
	...
	function canSendLI(row: SocialRow): boolean {
		...
		const hasAudience =
			hasValue(row.linkedinChatId) ||
			hasValue(row.linkedinProfileUrl) ||
			hasValue(row.linkedinPublicId);
		const hasTemplate =
			hasValue(row.liTemplateType) || hasValue(row.liTemplateName);
		return hasAudience && hasTemplate;
	}

	function mockSend(network: "fb" | "li", rows: CallCampaign[]) {
		...
		window.setTimeout(() => {
			...
			console.log(
				`Mock send to ${network === "fb" ? "Facebook (ManyChat)" : "LinkedIn (Unipile)"}`,
				{
					count: rows.length,
			...
			try {
				window.dispatchEvent(
					new CustomEvent("social-mock-sent", {
						detail: { network, count: rows.length },
					}),
				);
			} catch {}
		}, 600);
