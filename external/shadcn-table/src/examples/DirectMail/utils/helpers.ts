...
export function filterCampaigns<
	T extends CampaignBase &
		Partial<{ calls: number; leads: number; inQueue: number }>,
>(data: T[], query: string): T[] {
	...
	return data.filter((r) =>
		[
			...
			(r as Partial<DirectMailCampaign>)?.template?.id ??
				(r as LegacyDirectMailCampaign)?.templateId,
