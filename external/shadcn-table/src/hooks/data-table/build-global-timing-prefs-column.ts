	...
	return {
		...
		accessorFn: (row) => {
			...
			if (hasModalSignals) {
				...
				if (typeof tzFromLead === "boolean")
					parts.push(
						`Timezone=${tzFromLead ? "Lead location" : "Account default"}`,
					);
			...
			if (typeof vmAsAns === "boolean")
				parts.push(`VM=${vmAsAns ? "Yes" : "No"}`);
			...
			if (parts.length === 0) {
				...
				if (goal) {
					const isAggressive =
						/(aggressive|speed|speed\s*to\s*lead|fast|hot|urgent)/.test(goal);
					const isBalanced =
						/(balanced|standard|follow[-\s]?up|convert|appointment)/.test(goal);
					const isNurture =
						/(nurture|long[-\s]?term|light|drip|awareness)/.test(goal);
