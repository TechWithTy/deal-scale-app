...
export function useAdjustedInitialState(props: AdjustedInitialStateProps) {
	...
	const adjustedInitialState = React.useMemo(() => {
		...
		if (!(disableAll || disabled.script))
			insertAt = ensure("globalSalesScriptTitle", insertAt);
		if (!(disableAll || disabled.agent))
			insertAt = ensure("globalAgentTitle", insertAt);
		if (!(disableAll || disabled.transfer))
			insertAt = ensure("globalTransferAgentTitle", insertAt);
		if (!(disableAll || disabled.goal))
			insertAt = ensure("globalCampaignGoal", insertAt);
		if (!(disableAll || disabled.timing))
			insertAt = ensure("globalTimingPrefs", insertAt);
