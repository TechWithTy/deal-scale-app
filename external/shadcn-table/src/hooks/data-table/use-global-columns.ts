...
export function useGlobalColumns<TData>(props: GlobalColumnsProps<TData>) {
	...
	const columns = React.useMemo<ColumnDef<TData>[]>(() => {
		...
		const disabled = ((typeof disableGlobalColumns === "object" &&
		...
		const hasDnc = (providedColumns ?? []).some(
			(c) =>
				c.id === "globalDnc" ||
				c.id === "dnc" ||
				("accessorKey" in c && c.accessorKey === "dnc"),
		);
		const hasDncSource = (providedColumns ?? []).some(
			(c) =>
				c.id === "globalDncSource" ||
				("accessorKey" in c && c.accessorKey === "dncSource"),
		);
		const hasScript = (providedColumns ?? []).some(
			(c) => c.id === "globalSalesScriptTitle",
		);
		const hasAgent = (providedColumns ?? []).some(
			(c) => c.id === "globalAgentTitle",
		);
		const hasTransfer = (providedColumns ?? []).some(
			(c) => c.id === "globalTransferAgentTitle",
		);
		const hasGoal = (providedColumns ?? []).some(
			(c) => c.id === "globalCampaignGoal",
		);
		const hasTiming = (providedColumns ?? []).some(
			(c) => c.id === "globalTimingPrefs",
		);

		// Always make sure DNC column exists; if user already has one, keep as-is but still consider injecting source next to it
		const dncCol =
			disableAll || disabled.dnc
				? undefined
				: hasDnc
					? undefined
					: buildGlobalDncColumn<TData>();
		const sourceCol =
			disableAll || disabled.dncSource
				? undefined
				: hasDncSource
					? undefined
					: buildGlobalDncSourceColumn<TData>();
		const scriptCol =
			disableAll || disabled.script
				? undefined
				: hasScript
					? undefined
					: buildGlobalScriptColumn<TData>();
		const agentCol =
			disableAll || disabled.agent
				? undefined
				: hasAgent
					? undefined
					: buildGlobalAgentColumn<TData>();
		const goalCol =
			disableAll || disabled.goal
				? undefined
				: hasGoal
					? undefined
					: buildGlobalGoalColumn<TData>();
		const timingCol =
			disableAll || disabled.timing
				? undefined
				: hasTiming
					? undefined
					: buildGlobalTimingPrefsColumn<TData>();
		...
		let insertBaseIdx =
			controlsIdx >= 0 ? controlsIdx + 1 : selectIdx >= 0 ? selectIdx + 1 : 0;
