...
export function buildGlobalAgentColumn<TData>(): ColumnDef<TData> {
	return {
		...
		accessorFn: (row) => {
			...
			const v = r.aiAvatarAgent ?? r.agentTitle ?? r.agentName ?? r.agent ?? "";
		...
		cell: ({ getValue, row }) => {
			...
			const inferAiType = (rowObj: Record<string, unknown>): string | null => {
				...
				// Campaign type hints
				const canonical = (rowObj.campaignType ||
					rowObj.channel ||
					rowObj.primaryType) as string | undefined;
			...
			const humanRole = ((): string => {
				const role = (r.agentRole || r.role || r.humanRole) as
					| string
					| undefined;
			...
			children.push(
				React.createElement(
					"span",
					{ key: "label", className: "truncate max-w-[12rem]" },
					label,
				),
			);
			children.push(
