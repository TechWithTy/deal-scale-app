...
export function useNuqsIntegration<TData>(props: NuqsIntegrationProps<TData>) {
	...
	try {
		...
		perPageTuple = makeLocalQueryState<number>(
	...
	let sortingTuple: [
		...
		(
			value:
				| ExtendedColumnSort<TData>[]
				| ((
						old: ExtendedColumnSort<TData>[] | null,
	...
	try {
		...
		sortingTuple = makeLocalQueryState<ExtendedColumnSort<TData>[]>(
	...
	const onSortingChange: OnChangeFn<SortingState> = React.useCallback(
		(updaterOrValue) => {
			if (typeof updaterOrValue === "function") {
				...
				setSortingUrlState(
					updaterOrValue as unknown as ExtendedColumnSort<TData>[],
				);
			}
		},
