...
export function MapsTestComposite({
	...
	return (
		<main className="container mx-auto max-w-5xl p-6">
					...
					<SearchControls
						...
						onTextSearch={() => {
							try {
								...
								const svc: google.maps.places.PlacesService =
			...
			{/* Street View Preview Modal Trigger */}
			<div className="mb-6">
				<Dialog open={svOpen} onOpenChange={setSvOpen}>
					<DialogTrigger asChild>
						{(() => {
							...
							const canPreview =
								hasValidSelection ||
								coordsValid ||
								!!(lastACPlaceRef.current?.location || selectedPlace?.location);
							return (
								<button
									type="button"
									...
									onClick={() => canPreview && setSvOpen(true)}
									disabled={!canPreview}
									aria-disabled={!canPreview}
									title={
										canPreview
											? undefined
											: "Select a valid address or enter valid coordinates"
									}
								>
									Preview Street View
								</button>
					...
					<DialogContent className="max-w-5xl">
						<DialogHeader>
							<DialogTitle>Street View Preview</DialogTitle>
						</DialogHeader>
						<div className="mt-2">
							<StreetViewPreview
								initialLocation={
									(lastACPlaceRef.current?.location as
										| google.maps.LatLngLiteral
										| undefined) ?? coords
								}
				...
				{!(
					hasValidSelection ||
					(Number.isFinite(Number.parseFloat(String(lat).trim())) &&
						Number.isFinite(Number.parseFloat(String(lng).trim()))) ||
