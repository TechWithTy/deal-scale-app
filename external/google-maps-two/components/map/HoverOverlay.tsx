					...
					<div className="space-y-1.5">
						...
						{typeof details?.openNow === "boolean" && (
							<div className="text-xs">
								<span
									className={`${details.openNow ? "bg-primary/15 text-primary" : "bg-destructive/15 text-destructive"} rounded px-1`}
								>
						...
						{(details?.formattedAddress ||
							...
							<div className="space-y-1 pt-1 text-xs">
								...
								{details?.priceLevel !== undefined && (
									<div className="text-muted-foreground">
										{"$".repeat(Math.max(1, Math.min(4, details.priceLevel)))}{" "}
										price range
									</div>
								)}
								{details?.businessStatus &&
							...
							details!.reviews!.length > 0 && (
								<div className="pt-2 text-xs">
									...
									{(() => {
										...
										const short =
											text.length > 180 && !reviewsExpanded
												? text.slice(0, 180) + "…"
												: text;
										return (
						...
						{validation && (
							<div className="mt-2 rounded border bg-card p-2 text-xs">
								...
								<div className="text-muted-foreground">
									Formatted: {validation.address?.formattedAddress || ""}
								</div>
								<div className="text-muted-foreground">
									Entered: {validation.verdict?.inputGranularity || ""}
								</div>
								<div className="text-muted-foreground">
									Validated: {validation.verdict?.validationGranularity || ""}
								</div>
								<div className="text-muted-foreground">
									Complete: {String(validation.verdict?.addressComplete)}
								</div>
								<div className="text-muted-foreground">
									Unconfirmed:{" "}
									{String(validation.verdict?.hasUnconfirmedComponents)}
								</div>
							</div>
						)}
						{validating && (
