...
const _child_process = /*#__PURE__*/ _interop_require_default(
...
Next.js Config:
  output: ${nextConfig.output}`);
	if (
		versionInfo == null ? void 0 : versionInfo.staleness.startsWith("stale")
	) {
		(0, _log.warn)(`${title}
 ...
 */ async function runSharedDependencyCheck(tools, skipMessage) {
	...
	const triples =
		((_getSupportedArchTriples_currentPlatform =
			getSupportedArchTriples()[currentPlatform]) == null
			? void 0
			: _getSupportedArchTriples_currentPlatform[_os.default.arch()]) ?? [];
		...
		},
		{
			...
			scripts: {
				default: async () => {
					...
					try {
						...
						const bindings = await loadBindings(
							(_nextConfig_experimental = nextConfig.experimental) == null
								? void 0
								: _nextConfig_experimental.useWasmBinary,
						);
					...
					const triples =
						(_platformArchTriples_currentPlatform =
							platformArchTriples[currentPlatform]) == null
							? void 0
							: _platformArchTriples_currentPlatform[_os.default.arch()];
					...
					const tryResolve = (pkgName) => {
						try {
							...
							output.push(
								`${pkgName} resolve failed: ${e.message ?? "unknown"}`,
							);
						}
						return false;
					};
