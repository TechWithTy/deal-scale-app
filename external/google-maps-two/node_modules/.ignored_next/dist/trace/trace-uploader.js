...
const _nodefetch = /*#__PURE__*/ _interop_require_default(
...
const {
	NEXT_TRACE_UPLOAD_DEBUG, // An external env to allow to upload full trace without picking up the relavant spans.
...
(async function upload() {
	...
	const projectPkgJson = JSON.parse(
		await _promises.default.readFile(projectPkgJsonPath, "utf-8"),
	);
	const pkgName = projectPkgJson.name;
	const commit = _child_process.default
		.spawnSync(
			_os.default.platform() === "win32" ? "git.exe" : "git",
			["rev-parse", "HEAD"],
			{
				shell: true,
			},
