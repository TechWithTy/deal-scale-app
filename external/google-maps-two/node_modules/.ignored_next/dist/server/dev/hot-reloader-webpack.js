...
class HotReloaderWebpack {
	...
	async refreshServerComponents() {
		this.send({
			action:
				_hotreloadertypes.HMR_ACTIONS_SENT_TO_BROWSER.SERVER_COMPONENT_CHANGES,
		});
	}
	onHMR(req, _socket, head) {
		wsServer.handleUpgrade(req, req.socket, head, (client) => {
			...
			(_this_webpackHotMiddleware = this.webpackHotMiddleware) == null
				? void 0
				: _this_webpackHotMiddleware.onHMR(client);
	...
	async clean(span) {
		return span
			.traceChild("clean")
			.traceAsyncFn(() =>
				(0, _recursivedelete.recursiveDelete)(
	...
	async start() {
				...
				await Promise.all(
					Object.keys(entries).map(async (entryKey) => {
						...
						const staticInfo = isEntry
							? await (0, _entries.getStaticInfoIncludingLayouts)({
						...
						const isServerComponent =
							isAppPath &&
							staticInfo.rsc !== _constants1.RSC_MODULE_TYPES.client;
		...
		this.multiCompiler.hooks.done.tap("NextjsHotReloaderForServer", () => {
			...
			const middlewareChanges = Array.from(changedEdgeServerPages).filter(
				(name) => (0, _utils.isMiddlewareFilename)(name),
			);
			if (middlewareChanges.length > 0) {
				this.send({
					event:
						_hotreloadertypes.HMR_ACTIONS_SENT_TO_BROWSER.MIDDLEWARE_CHANGES,
				});
			}
			if (pageChanges.length > 0) {
				this.send({
					event:
						_hotreloadertypes.HMR_ACTIONS_SENT_TO_BROWSER.SERVER_ONLY_CHANGES,
				...
				if (this.prevChunkNames) {
					...
					if (addedPages.size > 0) {
						for (const addedPage of addedPages) {
							...
							this.send({
								action:
									_hotreloadertypes.HMR_ACTIONS_SENT_TO_BROWSER.ADDED_PAGE,
								data: [page],
							});
						}
					}
	...
	async stop() {
		...
		if (this.fallbackWatcher) {
			await new Promise((resolve, reject) => {
				this.fallbackWatcher.close((err) =>
