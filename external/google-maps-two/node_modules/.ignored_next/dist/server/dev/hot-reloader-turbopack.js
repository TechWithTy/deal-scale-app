...
async function createHotReloaderTurbopack(opts, serverFields, distDir) {
	...
	function sendEnqueuedMessages() {
		...
		for (const client of clients) {
			...
			if (state.turbopackUpdates.length > 0) {
				sendToClient(client, {
					action:
						_hotreloadertypes.HMR_ACTIONS_SENT_TO_BROWSER.TURBOPACK_MESSAGE,
	...
	const sendHmr = (id, payload) => {
		for (const client of clients) {
			...
			(_clientStates_get = clientStates.get(client)) == null
				? void 0
				: _clientStates_get.hmrPayloads.set(id, payload);
		}
	...
	async function subscribeToChanges(key, includeIssues, endpoint, makePayload) {
		...
		for await (const change of changed) {
			(0, _turbopackutils.processIssues)(
				currentEntryIssues,
				key,
				change,
				false,
	...
	const overlayMiddleware = (0, _middlewareturbopack.getOverlayMiddleware)(
		project,
	);
	...
	const hotReloader = {
		...
		async run(req, res, _parsedUrl) {
			...
			if (
				...
				const params = (0, _hotreloaderwebpack.matchNextPageBundleRequest)(
					req.url,
				);
				if (params) {
		...
		onHMR(req, socket, head) {
			wsServer.handleUpgrade(req, socket, head, (client) => {
				...
				const turbopackConnected = {
					action:
						_hotreloadertypes.HMR_ACTIONS_SENT_TO_BROWSER.TURBOPACK_CONNECTED,
				};
							...
							for (const [key, issue] of issueMap) {
								...
								errorsMap.set(key, {
									...
									details: issue.detail
										? (0, _turbopackutils.renderStyledStringToErrorAnsi)(
												issue.detail,
											)
										: undefined,
								});
