...
const _getassetpathfromroute = /*#__PURE__*/ _interop_require_default(
...
async function readPartialManifest(distDir, name, pageName, type = "pages") {
	const manifestPath = _path.posix.join(
		distDir,
		`server`,
		type,
		type === "middleware" || type === "instrumentation"
			...
			: type === "app"
				? pageName
				: (0, _getassetpathfromroute.default)(pageName),
		name,
	);
	return JSON.parse(
		await (0, _promises.readFile)(_path.posix.join(manifestPath), "utf-8"),
	);
}
class TurbopackManifestLoader {
	...
	async loadActionManifest(pageName) {
		this.actionManifests.set(
			(0, _entrykey.getEntryKey)("app", "server", pageName),
	...
	async writeActionManifest() {
		...
		const actionManifestJsonPath = (0, _path.join)(
			this.distDir,
			"server",
			`${_constants.SERVER_REFERENCE_MANIFEST}.json`,
		);
		const actionManifestJsPath = (0, _path.join)(
			this.distDir,
			"server",
			`${_constants.SERVER_REFERENCE_MANIFEST}.js`,
		);
	...
	async writeAppBuildManifest() {
		...
		const appBuildManifestPath = (0, _path.join)(
			this.distDir,
			_constants.APP_BUILD_MANIFEST,
		);
		(0, _nextjsrequirecachehotreloader.deleteCache)(appBuildManifestPath);
		await (0, _writeatomic.writeFileAtomic)(
	...
	async writeAppPathsManifest() {
		...
		const appPathsManifestPath = (0, _path.join)(
			this.distDir,
			"server",
			_constants.APP_PATHS_MANIFEST,
		);
	...
	async loadBuildManifest(pageName, type = "pages") {
		this.buildManifests.set(
			(0, _entrykey.getEntryKey)(type, "server", pageName),
			await readPartialManifest(
				this.distDir,
	...
	async writeBuildManifest(pageEntrypoints, rewrites) {
		...
		const buildManifestPath = (0, _path.join)(
			this.distDir,
			_constants.BUILD_MANIFEST,
		);
		const middlewareBuildManifestPath = (0, _path.join)(
			this.distDir,
			"server",
			`${_constants.MIDDLEWARE_BUILD_MANIFEST}.js`,
		);
		const interceptionRewriteManifestPath = (0, _path.join)(
			this.distDir,
			"server",
			`${_constants.INTERCEPTION_ROUTE_REWRITE_MANIFEST}.js`,
		);
		(0, _nextjsrequirecachehotreloader.deleteCache)(buildManifestPath);
		(0, _nextjsrequirecachehotreloader.deleteCache)(
		...
		const content = {
			__rewrites: rewrites
				? (0, _buildmanifestplugin.normalizeRewritesForBuildManifest)(rewrites)
				: {
						afterFiles: [],
						beforeFiles: [],
						fallback: [],
		...
		await (0, _writeatomic.writeFileAtomic)(
			(0, _path.join)(this.distDir, "static", this.buildId, "_ssgManifest.js"),
			_buildmanifestplugin.srcEmptySsgManifest,
		);
	}
	async writeFallbackBuildManifest() {
		const fallbackBuildManifest = this.mergeBuildManifests(
			[
				this.buildManifests.get(
					(0, _entrykey.getEntryKey)("pages", "server", "_app"),
				),
				this.buildManifests.get(
					(0, _entrykey.getEntryKey)("pages", "server", "_error"),
				),
			].filter(Boolean),
		);
		const fallbackBuildManifestPath = (0, _path.join)(
			this.distDir,
			`fallback-${_constants.BUILD_MANIFEST}`,
		);
	...
	async writeNextFontManifest() {
		...
		const fontManifestJsonPath = (0, _path.join)(
			this.distDir,
			"server",
			`${_constants.NEXT_FONT_MANIFEST}.json`,
		);
		const fontManifestJsPath = (0, _path.join)(
			this.distDir,
			"server",
			`${_constants.NEXT_FONT_MANIFEST}.js`,
		);
	...
	async writeLoadableManifest() {
		...
		const loadableManifestPath = (0, _path.join)(
			this.distDir,
			_constants.REACT_LOADABLE_MANIFEST,
		);
		const middlewareloadableManifestPath = (0, _path.join)(
			this.distDir,
			"server",
			`${_constants.MIDDLEWARE_REACT_LOADABLE_MANIFEST}.js`,
		);
	...
	mergeMiddlewareManifests(manifests) {
		...
		for (const fun of Object.values(manifest.functions).concat(
			...
			for (const matcher of fun.matchers) {
				if (!matcher.regexp) {
					matcher.regexp = (0, _pathtoregexp.pathToRegexp)(
	...
	async writeMiddlewareManifest() {
		...
		const middlewareManifestPath = (0, _path.join)(
			this.distDir,
			"server",
			_constants.MIDDLEWARE_MANIFEST,
		);
	...
	async loadPagesManifest(pageName) {
		this.pagesManifests.set(
			(0, _entrykey.getEntryKey)("pages", "server", pageName),
			await readPartialManifest(
				this.distDir,
	...
	async writePagesManifest() {
		const pagesManifest = this.mergePagesManifests(
			this.pagesManifests.values(),
		);
		const pagesManifestPath = (0, _path.join)(
			this.distDir,
			"server",
			_constants.PAGES_MANIFEST,
		);
