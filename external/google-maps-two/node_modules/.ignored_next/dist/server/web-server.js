...
const _web1 = /*#__PURE__*/ _interop_require_default(
	require("./response-cache/web"),
);
...
class NextWebServer extends _baseserver.default {
		...
		this.handleCatchallRenderRequest = async (req, res, parsedUrl) => {
			...
			if (pathname !== normalizedPage) {
				...
				if ((0, _utils.isDynamicRoute)(pathname)) {
					const routeRegex = (0, _routeregex.getNamedRouteRegex)(
						pathname,
						false,
					);
					const dynamicRouteMatcher = (0, _routematcher.getRouteMatcher)(
						routeRegex,
					);
					const defaultRouteMatches = dynamicRouteMatcher(pathname);
					const paramsResult = (0, _serverutils.normalizeDynamicRouteParams)(
						query,
						false,
						routeRegex,
						defaultRouteMatches,
					...
					const normalizedParams = paramsResult.hasValidParams
						? paramsResult.params
						: query;
					pathname = (0, _serverutils.interpolateDynamicPath)(
						pathname,
						normalizedParams,
						routeRegex,
					);
					(0, _serverutils.normalizeVercelUrl)(
						req,
						true,
	...
	getinterceptionRoutePatterns() {
		...
		return (
			((_this_serverOptions_webServerConfig_interceptionRouteRewrites =
				this.serverOptions.webServerConfig.interceptionRouteRewrites) == null
				? void 0
				: _this_serverOptions_webServerConfig_interceptionRouteRewrites.map(
						(rewrite) =>
							new RegExp(
								(0, _buildcustomroute.buildCustomRoute)("rewrite", rewrite)
									.regex,
							),
					)) ?? []
		);
