...
const _baseserver = /*#__PURE__*/ _interop_require_wildcard(
	_export_star(require("./base-server"), exports),
);
...
function getMiddlewareMatcher(info) {
	...
	const matcher = (0, _middlewareroutematcher.getMiddlewareRouteMatcher)(
		info.matchers,
	);
...
class NextNodeServer extends _baseserver.default {
	constructor(options) {
		...
		this.handleCatchallMiddlewareRequest = async (req, res, parsed) => {
			const isMiddlewareInvoke = (0, _requestmeta.getRequestMeta)(
				req,
				"middlewareInvoke",
			);
			...
			const normalizedPathname = (0, _removetrailingslash.removeTrailingSlash)(
				parsed.pathname || "",
			);
			...
			try {
				...
				if ("response" in result) {
					...
					for (const [key, value] of Object.entries(
						(0, _utils1.toNodeOutgoingHttpHeaders)(result.response.headers),
					)) {
			...
			try {
				const instrumentationHook = await dynamicRequire(
					(0, _path.resolve)(
						...
						"server",
						_constants1.INSTRUMENTATION_HOOK_FILENAME,
					),
				);
				await (instrumentationHook.register == null
					? void 0
					: instrumentationHook.register.call(instrumentationHook));
			} catch (err) {
	...
	getPagesManifest() {
		return (0, _loadmanifest.loadManifest)(
			(0, _path.join)(this.serverDistDir, _constants.PAGES_MANIFEST),
		);
	}
	getAppPathsManifest() {
		if (!this.enabledDirectories.app) return undefined;
		return (0, _loadmanifest.loadManifest)(
			(0, _path.join)(this.serverDistDir, _constants.APP_PATHS_MANIFEST),
		);
	}
		...
		return (
			(routesManifest == null
				...
				: routesManifest.rewrites.beforeFiles
						.filter(
							_generateinterceptionroutesrewrites.isInterceptionRouteRewrite,
						)
	...
	async hasPage(pathname) {
		...
		return !!(0, _require.getMaybePagePath)(
			pathname,
			this.distDir,
	...
	async runApi(req, res, query, match) {
		...
		const module = await _routemoduleloader.RouteModuleLoader.load(
	...
	async renderHTML(req, res, pathname, query, renderOpts) {
		return (0, _tracer.getTracer)().trace(
			_constants2.NextNodeServerSpan.renderHTML,
			async () => this.renderHTMLImpl(req, res, pathname, query, renderOpts),
		);
	}
	async renderHTMLImpl(req, res, pathname, query, renderOpts) {
		if (process.env.NEXT_MINIMAL) {
			...
			return (0, _modulerender1.lazyRenderPagesPage)(
				req.originalRequest,
				res.originalResponse,
				pathname,
				query,
	...
	async findPageComponents({ page, query, params, isAppPath, url }) {
		return (0, _tracer.getTracer)().trace(
			_constants2.NextNodeServerSpan.findPageComponents,
			{
				spanName: "resolve page components",
				attributes: {
					"next.route": isAppPath
	...
	async findPageComponentsImpl({ page, query, params, isAppPath, url: _url }) {
		...
		if (query.amp) {
			...
			pagePaths.unshift(
				(isAppPath
					? (0, _apppaths.normalizeAppPath)(page)
					: (0, _normalizepagepath.normalizePagePath)(page)) + ".amp",
			);
		}
	...
	getFallback(page) {
		...
		return cacheFs.readFile(
			(0, _path.join)(this.serverDistDir, "pages", `${page}.html`),
			"utf8",
		);
	}
	...
	getPrefetchRsc(pathname) {
		return this.getCacheFilesystem().readFile(
			(0, _path.join)(
				this.serverDistDir,
				"app",
				`${pathname}${_constants1.RSC_PREFETCH_SUFFIX}`,
			),
			"utf8",
		);
	}
	...
	normalizeReq(req) {
		return !(req instanceof _node.NodeNextRequest)
			? new _node.NodeNextRequest(req)
			: req;
	}
	normalizeRes(res) {
		return !(res instanceof _node.NodeNextResponse)
			? new _node.NodeNextResponse(res)
			: res;
	}
	getRequestHandler() {
	...
	makeRequestHandler() {
		...
		return (req, res, parsedUrl) => {
			...
			const loggingFetchesConfig =
				(_this_nextConfig_logging = this.nextConfig.logging) == null
					? void 0
					: _this_nextConfig_logging.fetches;
			const enabledVerboseLogging = !!loggingFetchesConfig;
			const shouldTruncateUrl = !(loggingFetchesConfig == null
				? void 0
				: loggingFetchesConfig.fullUrl);
			if (this.renderOpts.dev) {
				const {
					blue,
					green,
				...
				const reqCallback = () => {
					...
					writeStdoutLine(
						`${method} ${req.url ?? ""} ${color((res.statusCode ?? 200).toString())} in ${reqDuration}ms`,
					);
	...
	async revalidate({ urlPath, revalidateHeaders, opts }) {
		...
		await handler(
			new _node.NodeNextRequest(mocked.req),
			new _node.NodeNextResponse(mocked.res),
		);
	...
	async renderErrorToResponseImpl(ctx, err) {
		...
		if (is404 && this.enabledDirectories.app) {
			...
			if (
				this.getEdgeFunctionsPages().includes(
	...
	/** Returns the middleware routing item if there is one. */ getMiddleware() {
		...
		const middleware =
			manifest == null
				? void 0
				: (_manifest_middleware = manifest.middleware) == null
					? void 0
					: _manifest_middleware["/"];
		if (!middleware) {
			return;
		}
		return {
	 ...
	 */ getEdgeFunctionInfo(params) {
		...
		let foundPage;
		try {
			foundPage = (0, _denormalizepagepath.denormalizePagePath)(
				(0, _normalizepagepath.normalizePagePath)(params.page),
			);
		} catch (err) {
			return null;
		}
	 ...
	 */ async runMiddleware(params) {
		...
		if (
			(0, _apiutils.checkIsOnDemandRevalidate)(
				params.request,
		...
		if (this.nextConfig.skipMiddlewareUrlNormalize) {
			...
			const query = (0, _querystring.urlQueryToSearchParams)(
				params.parsed.query,
			).toString();
		...
		const result = await run({
			...
			request: {
				...
				signal: (0, _nextrequest.signalFromNodeResponse)(
		...
		if (result.response.headers.has("set-cookie")) {
			const cookies = result.response.headers
				...
				.flatMap((maybeCompoundCookie) =>
					(0, _utils1.splitCookiesString)(maybeCompoundCookie),
				);
	...
	getPrerenderManifest() {
		...
		if (
			((_this_renderOpts = this.renderOpts) == null
				? void 0
				: _this_renderOpts.dev) ||
			((_this_serverOptions = this.serverOptions) == null
				? void 0
				: _this_serverOptions.dev) ||
			process.env.NODE_ENV === "development" ||
			process.env.NEXT_PHASE === _constants.PHASE_PRODUCTION_BUILD
		) {
		...
		this._cachedPreviewManifest = (0, _loadmanifest.loadManifest)(
			(0, _path.join)(this.distDir, _constants.PRERENDER_MANIFEST),
		);
		return this._cachedPreviewManifest;
	}
	getRoutesManifest() {
		return (0, _tracer.getTracer)().trace(
			_constants2.NextNodeServerSpan.getRoutesManifest,
			() => {
				const manifest = (0, _loadmanifest.loadManifest)(
					(0, _path.join)(this.distDir, _constants.ROUTES_MANIFEST),
				);
	...
	attachRequestMeta(req, parsedUrl, isUpgradeReq) {
		...
		if (!isUpgradeReq) {
			(0, _requestmeta.addRequestMeta)(
				req,
				"clonableBody",
				(0, _bodystreams.getCloneableBody)(req.body),
			);
		}
	}
	async runEdgeFunction(params) {
		...
		const initialUrl = new URL(
			(0, _requestmeta.getRequestMeta)(params.req, "initURL") || "/",
			"http://n",
		);
		...
		const result = await run({
			...
			incrementalCache:
				...
				(0, _requestmeta.getRequestMeta)(params.req, "incrementalCache"),
		});
