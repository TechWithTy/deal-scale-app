...
const serverLayer = {
	...
	filterCompletionsAtPosition(entries) {
		return entries.filter((e) => {
			...
			if (
				_constant.DISALLOWED_SERVER_REACT_APIS.includes(e.name) &&
				e.source === "react"
			) {
				return false;
			}
	...
	hasDisallowedReactAPIDefinition(definitions) {
		return definitions == null
			...
			: definitions.some(
					(d) =>
						_constant.DISALLOWED_SERVER_REACT_APIS.includes(d.name) &&
	...
	getSemanticDiagnosticsForImportDeclaration(source, node) {
		...
		const namedBindings =
			importClause == null ? void 0 : importClause.namedBindings;
		if (importClause) {
