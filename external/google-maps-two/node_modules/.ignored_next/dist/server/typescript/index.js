	...
	function create(info) {
		...
		proxy.getCompletionsAtPosition = (fileName, position, options) => {
			...
			if (!entryInfo.client) {
				...
				prior = _metadata.default.filterCompletionsAtPosition(
					fileName,
					position,
					options,
					prior,
		...
		proxy.getQuickInfoAtPosition = (fileName, position) => {
			...
			if (!entryInfo.client) {
				...
				const metadataInfo = _metadata.default.getQuickInfoAtPosition(
					fileName,
					position,
				);
				...
				if (ts.isImportDeclaration(node)) {
					...
					if (isAppEntry) {
						...
						const metadataDiagnostics = isClientEntry
							? _metadata.default.getSemanticDiagnosticsForExportVariableStatementInClientEntry(
									fileName,
									node,
								)
							: _metadata.default.getSemanticDiagnosticsForExportVariableStatement(
									fileName,
									node,
								);
					...
					if (isAppEntry) {
						const metadataDiagnostics = isClientEntry
							? _metadata.default.getSemanticDiagnosticsForExportDeclarationInClientEntry(
									fileName,
									node,
								)
							: _metadata.default.getSemanticDiagnosticsForExportDeclaration(
									fileName,
									node,
								);
						prior.push(...metadataDiagnostics);
		...
		proxy.getDefinitionAndBoundSpan = (fileName, position) => {
			...
			if ((0, _utils.isAppEntryFile)(fileName) && !entryInfo.client) {
				const metadataDefinition = _metadata.default.getDefinitionAndBoundSpan(
					fileName,
					position,
				);
