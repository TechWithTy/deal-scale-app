...
const _lrucache = /*#__PURE__*/ _interop_require_default(
	require("next/dist/compiled/lru-cache"),
);
const _loadcustomroutes = /*#__PURE__*/ _interop_require_default(
...
const buildCustomRoute = (type, item, basePath, caseSensitive) => {
	...
	const match = (0, _pathmatch.getPathMatch)(item.source, {
		...
		regexModifier: !item.internal
			? (regex) =>
					(0, _redirectstatus.modifyRouteRegex)(
						regex,
...
async function setupFsCheck(opts) {
	...
	if (!opts.dev) {
		...
		const buildIdPath = _path.default.join(
			opts.dir,
			opts.config.distDir,
			_constants.BUILD_ID_FILE,
		);
		try {
		...
		try {
			for (const file of await (0, _recursivereaddir.recursiveReadDir)(
				publicFolderPath,
			)) {
				// Ensure filename is encoded and normalized.
				publicFolderItems.add(
					encodeURI((0, _normalizepathsep.normalizePathSep)(file)),
				);
			}
		} catch (err) {
			if (err.code !== "ENOENT") {
		...
		const prerenderManifestPath = _path.default.join(
			distDir,
			_constants.PRERENDER_MANIFEST,
		);
		const middlewareManifestPath = _path.default.join(
			distDir,
			"server",
		...
		const appRoutesManifestPath = _path.default.join(
			distDir,
			_constants.APP_PATH_ROUTES_MANIFEST,
		);
		const routesManifest = JSON.parse(
			await _promises.default.readFile(routesManifestPath, "utf8"),
		);
		prerenderManifest = JSON.parse(
			await _promises.default.readFile(prerenderManifestPath, "utf8"),
		);
		const middlewareManifest = JSON.parse(
			await _promises.default
				.readFile(middlewareManifestPath, "utf8")
				.catch(() => "{}"),
		);
		const pagesManifest = JSON.parse(
			await _promises.default.readFile(pagesManifestPath, "utf8"),
		);
		const appRoutesManifest = JSON.parse(
			await _promises.default
				.readFile(appRoutesManifestPath, "utf8")
				.catch(() => "{}"),
		);
		...
		for (const route of routesManifest.dynamicRoutes) {
			dynamicRoutes.push({
				...route,
				match: (0, _routematcher.getRouteMatcher)(
					(0, _routeregex.getRouteRegex)(route.page),
				),
			});
		}
		if (
			(_middlewareManifest_middleware = middlewareManifest.middleware) == null
				? void 0
				: (_middlewareManifest_middleware_ =
							_middlewareManifest_middleware["/"]) == null
					? void 0
					: _middlewareManifest_middleware_.matchers
		) {
			var _middlewareManifest_middleware_1, _middlewareManifest_middleware1;
			middlewareMatcher = (0,
			_middlewareroutematcher.getMiddlewareRouteMatcher)(
				(_middlewareManifest_middleware1 = middlewareManifest.middleware) ==
					null
					? void 0
					: (_middlewareManifest_middleware_1 =
								_middlewareManifest_middleware1["/"]) == null
						? void 0
						: _middlewareManifest_middleware_1.matchers,
			);
		}
		customRoutes = {
	...
	const handleLocale = (pathname, locales) => {
		let locale;
		if (i18n) {
			const i18nResult = (0, _normalizelocalepath.normalizeLocalePath)(
				pathname,
	...
	return {
		...
		async getItem(itemPath) {
			...
			const hasAssetPrefix = (0, _pathhasprefix.pathHasPrefix)(
				itemPath,
				assetPrefix,
			);
			...
			if (basePath && hasBasePath) {
				itemPath =
					(0, _removepathprefix.removePathPrefix)(itemPath, basePath) || "/";
			} else if (assetPrefix && hasAssetPrefix) {
				itemPath =
					(0, _removepathprefix.removePathPrefix)(itemPath, assetPrefix) || "/";
			}
			...
			if (opts.minimalMode) {
				...
				if (
					(_normalizers_prefetchRSC = normalizers.prefetchRSC) == null
						? void 0
						: _normalizers_prefetchRSC.match(itemPath)
				) {
					...
					(_normalizers_postponed = normalizers.postponed) == null
						? void 0
						: _normalizers_postponed.match(itemPath)
				) {
			...
			for (let [items, type] of itemsToCheck) {
				...
				if (i18n) {
					...
					const localeResult = handleLocale(
						itemPath, // legacy behavior allows visiting static assets under
						...
						isDynamicOutput
							...
							: [
									...
									...(((_i18n_domains = i18n.domains) == null
										? void 0
										: _i18n_domains.map((item) => item.defaultLocale)) || []),
								],
					);
				...
				if (
					...
					!(0, _pathhasprefix.pathHasPrefix)(curItemPath, "/_next/static")
				) {
					continue;
				}
				...
				if (matchedItem || opts.dev) {
					...
					if (!matchedItem && opts.dev) {
						...
						if (isStaticAsset && itemsRoot) {
							let found =
								fsPath &&
								(await (0, _fileexists.fileExists)(
									fsPath,
							...
							if (!found) {
								try {
									...
									found = await (0, _fileexists.fileExists)(
										fsPath,
										_fileexists.FileType.File,
									);
								} catch {}
							...
							if (
								ensureFn &&
								(await ((_ensureFn = ensureFn({
									type,
									itemPath: isAppFile
										? (0, _getmetadataroute.normalizeMetadataRoute)(curItemPath)
										: curItemPath,
								})) == null
									? void 0
									: _ensureFn.catch(() => "ENSURE_FAILED"))) === "ENSURE_FAILED"
							) {
								continue;
							}
						} else {
