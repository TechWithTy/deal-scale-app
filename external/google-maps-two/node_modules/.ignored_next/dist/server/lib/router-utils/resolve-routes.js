...
function getResolveRoutes(
	...
	async function resolveRoutes({ req, res, isUpgradeReq, invokedOutputs }) {
		...
		if (urlNoQuery == null ? void 0 : urlNoQuery.match(/(\\|\/\/)/)) {
			parsedUrl = _url.default.parse(
				(0, _utils2.normalizeRepeatedSlashes)(req.url),
				true,
			);
			return {
				parsedUrl,
		...
		const protocol =
			(req == null
				? void 0
				: (_req_socket = req.socket) == null
					? void 0
					: _req_socket.encrypted) ||
		...
		const initUrl = config.experimental.trustHostHeader
			...
			: opts.port
				? `${protocol}://${(0, _formathostname.formatHostname)(opts.hostname || "localhost")}:${opts.port}${req.url}`
				: req.url || "";
		...
		if (!isUpgradeReq) {
			(0, _requestmeta.addRequestMeta)(
				req,
				"clonableBody",
				(0, _bodystreams.getCloneableBody)(req),
			);
		}
		...
		if (config.i18n) {
			...
			const hadBasePath = (0, _pathhasprefix.pathHasPrefix)(
				parsedUrl.pathname || "",
				config.basePath,
			);
			initialLocaleResult = (0, _normalizelocalepath.normalizeLocalePath)(
			...
			domainLocale = (0, _detectdomainlocale.detectDomainLocale)(
				config.i18n.domains,
			...
			if (
				...
				parsedUrl.pathname = (0, _addpathprefix.addPathPrefix)(
		...
		const checkLocaleApi = (pathname) => {
			if (
				config.i18n &&
				pathname === urlNoQuery &&
				(initialLocaleResult == null
					? void 0
					: initialLocaleResult.detectedLocale) &&
				(0, _pathhasprefix.pathHasPrefix)(initialLocaleResult.pathname, "/api")
			) {
				return true;
			}
		};
		async function checkTrue() {
			...
			if (config.basePath) {
				if (
					!(0, _pathhasprefix.pathHasPrefix)(curPathname || "", config.basePath)
				) {
					return;
				}
				curPathname =
					(curPathname == null
						? void 0
						: curPathname.substring(config.basePath.length)) || "/";
			}
			...
			for (const route of dynamicRoutes) {
				...
				if (params) {
					...
					if (
						(pageOutput == null ? void 0 : pageOutput.type) === "appFile" &&
						(initialLocaleResult == null
							? void 0
							: initialLocaleResult.detectedLocale)
					) {
					...
					if (
						...
						(curPathname == null
							? void 0
							: curPathname.startsWith("/_next/data"))
					) {
		...
		const normalizers = {
			basePath:
				config.basePath && config.basePath !== "/"
					? new _basepath.BasePathPathnameNormalizer(config.basePath)
					: undefined,
		...
		async function handleRoute(route) {
			...
			if (config.i18n && route.internal) {
				...
				if (config.basePath) {
					curPathname = (0, _removepathprefix.removePathPrefix)(
						curPathname,
						config.basePath,
					);
				}
				const hadBasePath = curPathname !== parsedUrl.pathname;
				const localeResult = (0, _normalizelocalepath.normalizeLocalePath)(
					curPathname,
					config.i18n.locales,
				);
				...
				if (isDefaultLocale) {
					...
					curPathname =
						curPathname === "/"
							? config.basePath
							: (0, _addpathprefix.addPathPrefix)(curPathname, config.basePath);
				}
			...
			if (params) {
				...
				if (route.name === "middleware_next_data" && parsedUrl.pathname) {
					...
					if (
						...
						const hadBasePath =
							(_normalizers_basePath = normalizers.basePath) == null
								? void 0
								: _normalizers_basePath.match(parsedUrl.pathname);
						...
						if (normalizers.data.match(normalized)) {
							...
							(_normalizers_postponed = normalizers.postponed) == null
								? void 0
								: _normalizers_postponed.match(normalized)
						) {
							updated = true;
							normalized = normalizers.postponed.normalize(normalized, true);
						}
						if (config.i18n) {
							const curLocaleResult = (0,
						...
						if (updated) {
							if (hadBasePath) {
								normalized = _nodepath.default.posix.join(
									config.basePath,
									normalized,
								);
							}
				...
				if (route.name === "check_fs") {
					const pathname = parsedUrl.pathname || "";
					if (
						(invokedOutputs == null ? void 0 : invokedOutputs.has(pathname)) ||
						checkLocaleApi(pathname)
					) {
						return;
					}
					const output = await fsChecker.getItem(pathname);
					if (
						...
						!(
							...
							(initialLocaleResult == null
								? void 0
								: initialLocaleResult.detectedLocale) &&
							(0, _pathhasprefix.pathHasPrefix)(pathname, "/api")
						)
					) {
						if (
							config.useFileSystemPublicRoutes ||
				...
				if (!opts.minimalMode && route.name === "middleware") {
					...
					if (
						...
						const serverResult = await (renderServer == null
							? void 0
							: renderServer.initialize(renderServerOpts));
						if (!serverResult) {
							throw new Error(
						...
						const middlewareHeaders = (0, _utils1.toNodeOutgoingHttpHeaders)(
						...
						if (middlewareHeaders["location"]) {
							...
							const isRedirectStatus = _redirectstatus.allowedStatusCodes.has(
				...
				if (
					...
					parsedDestination.search = (0, _serverrouteutils.stringifyQuery)(
						req,
						query,
					);
					parsedDestination.pathname = (0, _utils2.normalizeRepeatedSlashes)(
