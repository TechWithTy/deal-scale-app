...
const _querystring = /*#__PURE__*/ _interop_require_default(
	require("querystring"),
);
...
const _hotreloaderwebpack = /*#__PURE__*/ _interop_require_default(
	require("../../dev/hot-reloader-webpack"),
);
...
async function propagateServerField(opts, field, args) {
	...
	await ((_opts_renderServer = opts.renderServer) == null
		? void 0
		: (_opts_renderServer_instance = _opts_renderServer.instance) == null
			? void 0
			: _opts_renderServer_instance.propagateServerField(
					opts.dir,
					field,
					args,
				));
...
async function startWatcher(opts) {
	...
	const hotReloader = opts.turbo
		? await (0, _hotreloaderturbopack.createHotReloaderTurbopack)(
				opts,
				serverFields,
				distDir,
			)
		: new _hotreloaderwebpack.default(opts.dir, {
				appDir,
				pagesDir,
				distDir: distDir,
	...
	if (opts.nextConfig.experimental.nextScriptWorkers) {
		await (0, _verifypartytownsetup.verifyPartytownSetup)(
			opts.dir,
			_path.default.join(distDir, _constants.CLIENT_STATIC_FILES_PATH),
		);
	}
	...
	await new Promise(async (resolve, reject) => {
		...
		const files = [
			...(0, _utils1.getPossibleMiddlewareFilenames)(
				_path.default.join(rootDir, ".."),
				nextConfig.pageExtensions,
			),
		...
		wp.on("aggregated", async () => {
			...
			const sortedKnownFiles = [...knownFiles.keys()].sort(
				(0, _entries.sortByPageExts)(nextConfig.pageExtensions),
			);
			for (const fileName of sortedKnownFiles) {
				...
				const watchTimeChange =
					watchTime === undefined ||
					(watchTime && watchTime !== (meta == null ? void 0 : meta.timestamp));
				...
				const isAppPath = Boolean(
					appDir &&
						(0, _normalizepathsep.normalizePathSep)(fileName).startsWith(
							(0, _normalizepathsep.normalizePathSep)(appDir) + "/",
						),
				);
				const isPagePath = Boolean(
					pagesDir &&
						(0, _normalizepathsep.normalizePathSep)(fileName).startsWith(
							(0, _normalizepathsep.normalizePathSep)(pagesDir) + "/",
						),
				);
				...
				if ((0, _utils1.isMiddlewareFile)(rootFile)) {
					...
					middlewareMatchers = ((_staticInfo_middleware =
						staticInfo.middleware) == null
						? void 0
						: _staticInfo_middleware.matchers) || [
						{
							regexp: ".*",
							originalSource: "/:path*",
						},
				...
				if (
					(0, _utils1.isInstrumentationHookFile)(rootFile) &&
				...
				if (isAppPath) {
					...
					if (
						(0, _normalizepathsep.normalizePathSep)(pageName).includes("/_")
					) {
						continue;
					}
					const originalPageName = pageName;
					pageName = (0, _apppaths.normalizeAppPath)(pageName).replace(
						/%5F/g,
						"_",
					);
			...
			if (conflictingPageChange !== 0) {
				if (numConflicting > 0) {
					...
					for (const p of conflictingAppPagePaths) {
						const appPath = _path.default.relative(
							dir,
							appPageFilePaths.get(p),
						);
						const pagesPath = _path.default.relative(
							dir,
							pagesPageFilePaths.get(p),
						);
			...
			if (envChange || tsconfigChange) {
				...
				(_hotReloader_activeWebpackConfigs =
					hotReloader.activeWebpackConfigs) == null
					? void 0
					: _hotReloader_activeWebpackConfigs.forEach((config, idx) => {
							...
							if (tsconfigChange) {
								...
								(_config_resolve = config.resolve) == null
									? void 0
									: (_config_resolve_plugins = _config_resolve.plugins) == null
										? void 0
										: _config_resolve_plugins.forEach((plugin) => {
												// look for the JsConfigPathsPlugin and update with
												// the latest paths/baseUrl config
												if (plugin && plugin.jsConfigPlugin && tsconfigResult) {
													var _config_resolve_modules,
														_config_resolve,
													...
													const resolvedUrlIndex =
														(_config_resolve = config.resolve) == null
															? void 0
															: (_config_resolve_modules =
																		_config_resolve.modules) == null
																? void 0
																: _config_resolve_modules.findIndex(
													...
													if (resolvedBaseUrl) {
														if (
															...
															if (resolvedUrlIndex && resolvedUrlIndex > -1) {
																...
																(_config_resolve1 = config.resolve) == null
																	? void 0
																	: (_config_resolve_modules1 =
																				_config_resolve1.modules) == null
																		? void 0
																		: _config_resolve_modules1.splice(
																				resolvedUrlIndex,
																				1,
																			);
															}
															...
															if (!resolvedBaseUrl.isImplicit) {
																...
																(_config_resolve2 = config.resolve) == null
																	? void 0
																	: (_config_resolve_modules2 =
																				_config_resolve2.modules) == null
																		? void 0
																		: _config_resolve_modules2.push(
																				resolvedBaseUrl.baseUrl,
																			);
															}
														}
													...
													if (
														(jsConfig == null
															? void 0
															: (_jsConfig_compilerOptions =
																		jsConfig.compilerOptions) == null
																? void 0
																: _jsConfig_compilerOptions.paths) &&
														resolvedBaseUrl
													) {
							...
							if (envChange) {
								var _config_plugins;
								(_config_plugins = config.plugins) == null
									? void 0
									: _config_plugins.forEach((plugin) => {
			...
			if (nestedMiddleware.length > 0) {
				_log.error(
					new _utils1.NestedMiddlewareError(
						nestedMiddleware,
						dir,
						pagesDir || appDir,
					).message,
			...
			opts.fsChecker.middlewareMatcher = (
				(_serverFields_middleware = serverFields.middleware) == null
					? void 0
					: _serverFields_middleware.matchers
			)
				? (0, _middlewareroutematcher.getMiddlewareRouteMatcher)(
						(_serverFields_middleware1 = serverFields.middleware) == null
							? void 0
							: _serverFields_middleware1.matchers,
					)
				: undefined;
			const interceptionRoutes = (0,
			_generateinterceptionroutesrewrites.generateInterceptionRoutesRewrites)(
				Object.keys(appPaths),
				opts.nextConfig.basePath,
			).map((item) =>
				(0, _filesystem.buildCustomRoute)(
			...
			const exportPathMap =
				(typeof nextConfig.exportPathMap === "function" &&
					(await (nextConfig.exportPathMap == null
						? void 0
						: nextConfig.exportPathMap.call(
								nextConfig,
								{},
								{
									dev: true,
			...
			try {
				...
				for (const page of sortedRoutes) {
					const route = (0, _builddataroute.buildDataRoute)(
						page,
						"development",
					);
				...
				if (
					...
					removedRoutes.forEach((route) => {
						hotReloader.send({
							action:
								_hotreloadertypes.HMR_ACTIONS_SENT_TO_BROWSER.REMOVED_PAGE,
							data: [route],
						});
					});
				}
	...
	async function requestHandler(req, res) {
		...
		if (
			(_parsedUrl_pathname = parsedUrl.pathname) == null
				? void 0
				: _parsedUrl_pathname.includes(clientPagesManifestPath)
		) {
			res.statusCode = 200;
		...
		if (
			(_parsedUrl_pathname1 = parsedUrl.pathname) == null
				? void 0
				: _parsedUrl_pathname1.includes(devMiddlewareManifestPath)
		) {
			...
			try {
				...
				const frame = frames.find(
					({ file }) =>
						...
						!(file == null ? void 0 : file.includes("web/globals")) &&
						!(file == null ? void 0 : file.includes("sandbox/context")) &&
						!(file == null ? void 0 : file.includes("<anonymous>")),
				);
				...
				if ((frame == null ? void 0 : frame.lineNumber) && frameFile) {
					if (hotReloader.turbopackProject) {
						try {
							originalFrame = await (0,
							_middlewareturbopack.createOriginalStackFrame)(
						...
						var _hotReloader_edgeServerStats,
							_hotReloader_serverStats,
							_frame_file,
							_frame_file1;
						...
						const compilation = isEdgeCompiler
							? (_hotReloader_edgeServerStats = hotReloader.edgeServerStats) ==
								null
								? void 0
								: _hotReloader_edgeServerStats.compilation
							: (_hotReloader_serverStats = hotReloader.serverStats) == null
								? void 0
								: _hotReloader_serverStats.compilation;
						const source = await (0, _middleware.getSourceById)(
							!!((_frame_file = frame.file) == null
								? void 0
								: _frame_file.startsWith(_path.default.sep)) ||
								!!((_frame_file1 = frame.file) == null
									? void 0
									: _frame_file1.startsWith("file:")),
							moduleId,
							compilation,
						);
						try {
							...
							originalFrame = await (0, _middleware.createOriginalStackFrame)({
								...
								compilation: isEdgeCompiler
									? (_hotReloader_edgeServerStats1 =
											hotReloader.edgeServerStats) == null
										? void 0
										: _hotReloader_edgeServerStats1.compilation
									: (_hotReloader_serverStats1 = hotReloader.serverStats) ==
											null
										? void 0
										: _hotReloader_serverStats1.compilation,
							});
						} catch {}
					}
					if (
...
async function setupDevBundler(opts) {
	...
	opts.telemetry.record(
		(0, _events.eventCliSession)(
			_path.default.join(opts.dir, opts.nextConfig.distDir),
			opts.nextConfig,
			{
				webpackVersion: 5,
				isSrcDir,
...
async function traceTurbopackErrorStack(project, error, frames) {
	let originalFrames = await Promise.all(
		frames.map(async (f) => {
			try {
				const traced = await (0, _middlewareturbopack.batchedTraceSource)(
					project,
					{
						file: f.file,
	...
	return (
		...
		originalFrames
			.map((f) => {
				...
				if (f.file != null) {
					const file =
						f.file.startsWith("/") || // Built-in "filenames" like `<anonymous>` shouldn't be made relative
						f.file.startsWith("<") ||
