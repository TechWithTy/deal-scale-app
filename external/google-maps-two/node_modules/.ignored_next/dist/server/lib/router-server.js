...
async function initialize(opts) {
	...
	if (opts.dev) {
		...
		devBundlerService = new _devbundlerservice.DevBundlerService(
	...
	const requestHandlerImpl = async (req, res) => {
		...
		async function invokeRender(
			...
			if (
				...
				(0, _removepathprefix.removePathPrefix)(
					invokePath,
					config.basePath,
				).startsWith(`/${parsedUrl.query.__nextLocale}/api`)
			) {
		...
		const handleRequest = async (handleIndex) => {
			...
			if (developmentBundler) {
				if (
					(0, _blockcrosssite.blockCrossSite)(
						req,
						res,
			...
			if (
				...
				try {
					...
					if (typeof err.statusCode === "number") {
						...
						return await invokeRender(
							_url.default.parse(invokePath, true),
							invokePath,
							handleIndex,
							{
								invokeStatus,
	...
	const logError = async (type, err) => {
		...
		await (developmentBundler == null
			? void 0
			: developmentBundler.logErrorWithOriginalStack(err, type));
	};
	...
	const resolveRoutes = (0, _resolveroutes.getResolveRoutes)(
		...
		renderServerOpts,
		developmentBundler == null ? void 0 : developmentBundler.ensureMiddleware,
	);
