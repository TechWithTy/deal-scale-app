					...
					length({ value }) {
						...
						return (
							...
							(((_JSON_stringify = JSON.stringify(
								value.kind === "PAGE" && value.pageData,
							)) == null
								? void 0
		...
		for (let i = 0; i < Math.ceil(tags.length / 64); i++) {
			...
			try {
				const res = await fetchRetryWithTimeout(
					`${this.cacheEndpoint}/v1/suspense-cache/revalidate?tags=${currentTags.map((tag) => encodeURIComponent(tag)).join(",")}`,
					{
						method: "POST",
		...
		if (this.cacheEndpoint && (!data || !hasFetchKindAndMatchingTags)) {
			try {
				...
				const res = await fetch(
					`${this.cacheEndpoint}/v1/suspense-cache/${key}`,
					{
						...
						headers: {
							...
							[CACHE_TAGS_HEADER]:
								(tags == null ? void 0 : tags.join(",")) || "",
							[_constants.NEXT_CACHE_SOFT_TAGS_HEADER]:
								(softTags == null ? void 0 : softTags.join(",")) || "",
						},
						next: fetchParams,
					},
				);
				...
				data = {
					...
					lastModified:
						cacheState !== "fresh"
							? Date.now() - _constants.CACHE_ONE_YEAR
							: Date.now() - parseInt(age || "0", 10) * 1000,
				};
				if (DEBUG) {
					console.log(
						`got fetch cache entry for ${key}, duration: ${Date.now() - start}ms, size: ${Object.keys(cached).length}, cache-state: ${cacheState} tags: ${tags == null ? void 0 : tags.join(",")} softTags: ${softTags == null ? void 0 : softTags.join(",")}`,
					);
				}
				if (data) {
	...
	async set(...args) {
		...
		if (this.cacheEndpoint) {
			try {
				...
				if (DEBUG) {
					console.log(
						`successfully set to fetch-cache for ${key}, duration: ${Date.now() - start}ms, size: ${body.length}`,
					);
				}
			} catch (err) {
