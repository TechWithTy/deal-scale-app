...
class FileSystemCache {
	constructor(ctx) {
		...
		if (ctx.maxMemoryCacheSize && !memoryCache) {
			...
			memoryCache = new _lrucache.default({
				...
				length({ value }) {
					...
					return (
						value.html.length +
						(((_JSON_stringify = JSON.stringify(value.pageData)) == null
							? void 0
		...
		if (this.serverDistDir && this.fs) {
			this.tagsManifestPath = _path.default.join(
				this.serverDistDir,
				"..",
				"cache",
				"fetch-cache",
	...
	async get(...args) {
		...
		if (!data && process.env.NEXT_RUNTIME !== "edge") {
			try {
				...
				const meta = JSON.parse(
					await this.fs.readFile(
						filePath.replace(/\.body$/, _constants.NEXT_META_SUFFIX),
						"utf8",
					),
				);
				const cacheEntry = {
			...
			try {
				...
				if (kind === "fetch" && this.flushToDisk) {
					...
					if (
						((_data_value2 = data.value) == null
							? void 0
							: _data_value2.kind) === "FETCH"
					) {
						var _data_value3;
						const storedTags =
							(_data_value3 = data.value) == null ? void 0 : _data_value3.tags;
						...
						if (
							!(tags == null
								? void 0
								: tags.every((tag) =>
										storedTags == null ? void 0 : storedTags.includes(tag),
									))
						) {
							if (this.debug) {
					...
					const pageData = isAppPath
						? await this.fs.readFile(
								this.getFilePath(
									`${key}${this.experimental.ppr ? _constants.RSC_PREFETCH_SUFFIX : _constants.RSC_SUFFIX}`,
									"app",
								),
								"utf8",
							)
						: JSON.parse(
								await this.fs.readFile(
									this.getFilePath(
										`${key}${_constants.NEXT_DATA_SUFFIX}`,
										"pages",
									),
									"utf8",
								),
					...
					if (isAppPath) {
						try {
							meta = JSON.parse(
								await this.fs.readFile(
									filePath.replace(/\.html$/, _constants.NEXT_META_SUFFIX),
									"utf8",
								),
							);
						} catch {}
		...
		if (
			(data == null
				? void 0
				: (_data_value = data.value) == null
					? void 0
					: _data_value.kind) === "PAGE"
		) {
			var _data_value_headers;
			let cacheTags;
			const tagsHeader =
				(_data_value_headers = data.value.headers) == null
					? void 0
					: _data_value_headers[_constants.NEXT_CACHE_TAGS_HEADER];
			...
			if (cacheTags == null ? void 0 : cacheTags.length) {
				...
				const isStale = cacheTags.some((tag) => {
					...
					return (
						(tagsManifest == null
							? void 0
							: (_tagsManifest_items_tag = tagsManifest.items[tag]) == null
								? void 0
								: _tagsManifest_items_tag.revalidatedAt) &&
						(tagsManifest == null
							? void 0
		...
		if (
			...
			(data == null
				? void 0
				: (_data_value1 = data.value) == null
					? void 0
					: _data_value1.kind) === "FETCH"
		) {
			this.loadTagsManifest();
	...
	async set(...args) {
		...
		if ((data == null ? void 0 : data.kind) === "PAGE") {
			...
			await this.fs.writeFile(
				this.getFilePath(
					`${key}${isAppPath ? (this.experimental.ppr ? _constants.RSC_PREFETCH_SUFFIX : _constants.RSC_SUFFIX) : _constants.NEXT_DATA_SUFFIX}`,
					isAppPath ? "app" : "pages",
				),
			...
			if (data.headers || data.status) {
				...
				await this.fs.writeFile(
					htmlPath.replace(/\.html$/, _constants.NEXT_META_SUFFIX),
					JSON.stringify(meta),
				);
			}
