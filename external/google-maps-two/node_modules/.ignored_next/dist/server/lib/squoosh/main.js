...
const getWorker = (0, _utils.execOnce)(
	() =>
		new _jestworker.Worker(_path.resolve(__dirname, "impl"), {
...
async function processBuffer(buffer, operations, encoding, quality) {
	...
	switch (encoding) {
		case "jpeg":
			return Buffer.from(
				await worker.encodeJpeg(imageData, {
					quality,
				}),
			);
		case "webp":
			return Buffer.from(
				await worker.encodeWebp(imageData, {
					quality,
				}),
			);
		case "avif":
