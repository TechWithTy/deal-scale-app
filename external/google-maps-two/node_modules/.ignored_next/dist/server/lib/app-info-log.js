...
function logStartInfo({
	...
	_log.bootstrap(
		`${(0, _picocolors.bold)((0, _picocolors.purple)(`${_log.prefixes.ready} Next.js ${"14.2.32"}`))}${process.env.TURBOPACK ? " (turbo)" : ""}`,
	);
	if (appUrl) {
	...
	if (expFeatureInfo == null ? void 0 : expFeatureInfo.length) {
		...
		/* indicate if there are more than the maximum shown no. flags */ if (
			expFeatureInfo.length > maxExperimentalFeatures
		) {
			_log.bootstrap(`  · ...`);
		}
	}
	...
	await (0, _config.default)(
		...
		dir,
		{
			onLoadUserConfig(userConfig) {
				const userNextConfigExperimental = (0,
				_config.getEnabledExperimentalFeatures)(userConfig.experimental);
