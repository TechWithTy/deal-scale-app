...
const _serverbrowser = /*#__PURE__*/ _interop_require_default(
...
const _loadablesharedruntime = /*#__PURE__*/ _interop_require_default(
...
async function renderToString(element) {
	const renderStream =
		await _serverbrowser.default.renderToReadableStream(element);
...
function checkRedirectValues(redirect, req, method) {
	...
	if (hasPermanent && hasStatusCode) {
		...
		hasStatusCode &&
		!_redirectstatus.allowedStatusCodes.has(statusCode)
	) {
		errors.push(
...
async function renderToHTMLImpl(req, res, pathname, query, renderOpts, extra) {
	...
	(0, _apiutils.setLazyProp)(
		...
		"cookies",
		(0, _getcookieparser.getCookieParser)(req.headers),
	);
	const metadata = {};
	...
	const hasPageGetInitialProps = !!(Component == null
		? void 0
		: Component.getInitialProps);
	const hasPageScripts =
		Component == null ? void 0 : Component.unstable_scriptLoader;
	...
	if (hasPageGetInitialProps && getServerSideProps) {
		throw new Error(
			_constants.SERVER_PROPS_GET_INIT_PROPS_CONFLICT + ` ${pathname}`,
		);
	}
	...
	if (dev) {
		...
		if (
			_constants1.STATIC_STATUS_PAGES.includes(pathname) &&
			(hasPageGetInitialProps || getServerSideProps)
		) {
			throw new Error(
				`\`pages${pathname}\` ${_constants.STATIC_STATUS_PAGE_GET_INITIAL_PROPS_ERROR}`,
			);
		}
	}
	for (const methodName of [
		...
		if (Component == null ? void 0 : Component[methodName]) {
			throw new Error(
				`page ${pathname} ${methodName} ${_constants.GSSP_COMPONENT_MEMBER_ERROR}`,
			);
		}
	}
	...
	const router = new ServerRouter(
		...
		isPreview,
		(0, _requestmeta.getRequestMeta)(req, "isLocaleDomain"),
	);
	...
	const inAmpMode =
		process.env.NEXT_RUNTIME !== "edge" && (0, _ampmode.isInAmpMode)(ampState);
									...
									_hooksclientcontextsharedruntime.PathParamsContext.Provider,
									{
										...
										children: /*#__PURE__*/ (0, _jsxruntime.jsx)(
											_routercontextsharedruntime.RouterContext.Provider,
											{
												value: router,
												children: /*#__PURE__*/ (0, _jsxruntime.jsx)(
													_ampcontextsharedruntime.AmpStateContext.Provider,
													{
														value: ampState,
														children: /*#__PURE__*/ (0, _jsxruntime.jsx)(
																...
																.HeadManagerContext.Provider,
															{
																...
																children: /*#__PURE__*/ (0, _jsxruntime.jsx)(
																		...
																		.Provider,
																	{
																		...
																		children: /*#__PURE__*/ (0,
																		_jsxruntime.jsx)(_styledjsx.StyleRegistry, {
	...
	const ctx = {
		...
		defaultGetInitialProps: async (docCtx, options = {}) => {
			const enhanceApp = (AppComp) => {
				return (props) =>
					/*#__PURE__*/ (0, _jsxruntime.jsx)(AppComp, {
						...props,
					});
			};
	...
	if (isSSG && !isFallback) {
		let data;
		try {
			data = await (0, _tracer.getTracer)().trace(
				_constants2.RenderSpan.getStaticProps,
				{
		...
		if (process.env.NODE_ENV !== "production") {
			if (
				...
				throw new Error(
					`\`redirect\` and \`notFound\` can not both be returned from ${isSSG ? "getStaticProps" : "getServerSideProps"} at the same time. Page: ${pathname}\nSee more info here: https://nextjs.org/docs/messages/gssp-mixed-not-found-redirect`,
				);
			}
		}
		...
		if (
			...
			data.props = {
				...
				__N_REDIRECT_STATUS: (0, _redirectstatus.getRedirectStatus)(
					data.redirect,
				),
			};
		...
		if (
			(dev || isBuildTimeSSG) &&
			!metadata.isNotFound &&
			!(0, _isserializableprops.isSerializableProps)(
				pathname,
				"getStaticProps",
				data.props,
			)
	...
	if (getServerSideProps && !isFallback) {
		...
		try {
			data = await (0, _tracer.getTracer)().trace(
				_constants2.RenderSpan.getServerSideProps,
				{
			...
			// 404 rendering
			if (
				(0, _iserror.default)(serverSidePropsError) &&
	...
	if (
		...
		Object.keys((props == null ? void 0 : props.pageProps) || {}).includes(
			"url",
		)
	) {
		console.warn(
	...
	if ((0, _utils.isResSent)(res) && !isSSG)
		return new _renderresult.default(null, {
			metadata,
		});
	...
	if (isAutoExport && pageIsDynamic) {
		const page = (0, _denormalizepagepath.denormalizePagePath)(
			(0, _normalizepagepath.normalizePagePath)(pathname),
		);
	...
	const renderDocument = async () => {
		...
		const BuiltinFunctionalDocument =
			Document[_constants1.NEXT_BUILTIN_DOCUMENT];
		...
		async function loadDocumentInitialProps(renderShell) {
				...
				if (renderShell) {
					return renderShell(EnhancedApp, EnhancedComponent).then(
						async (stream) => {
							...
							const html = await (0, _nodewebstreamshelper.streamToString)(
								stream,
							);
							return {
								html,
			...
			const docProps = await (0, _utils.loadGetInitialProps)(
				Document,
				documentCtx,
			);
			// the response might be finished on the getInitialProps call
		...
		const createBodyResult = (0, _tracer.getTracer)().wrap(
			_constants2.RenderSpan.createBodyResult,
			(initialStream, suffix) => {
		...
		if (hasDocumentGetInitialProps) {
			...
			bodyResult = (suffix) =>
				createBodyResult(
					(0, _nodewebstreamshelper.streamFromString)(docProps.html + suffix),
				);
		} else {
	...
	_tracer.getTracer)().getRootSpanAttributes()) == null
		? void 0
		: _getTracer_getRootSpanAttributes.set("next.route", renderOpts.page);
	const documentResult = await (0, _tracer.getTracer)().trace(
		_constants2.RenderSpan.renderDocument,
		{
	...
	const htmlProps = {
		...
		canonicalBase:
			!renderOpts.ampPath &&
			(0, _requestmeta.getRequestMeta)(req, "didStripLocale")
	...
	const document = /*#__PURE__*/ (0, _jsxruntime.jsx)(
		_ampcontextsharedruntime.AmpStateContext.Provider,
		{
			value: ampState,
			children: /*#__PURE__*/ (0, _jsxruntime.jsx)(
				_htmlcontextsharedruntime.HtmlContext.Provider,
				{
					value: htmlProps,
	...
	const documentHTML = await (0, _tracer.getTracer)().trace(
		_constants2.RenderSpan.renderToString,
	...
	const content = await (0, _nodewebstreamshelper.streamToString)(
		(0, _nodewebstreamshelper.chainStreams)(
