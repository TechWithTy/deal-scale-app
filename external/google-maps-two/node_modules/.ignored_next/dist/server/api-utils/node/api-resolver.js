...
const _bytes = /*#__PURE__*/ _interop_require_default(
	require("next/dist/compiled/bytes"),
);
...
async function revalidate(urlPath, opts, req, context) {
	...
	try {
		if (context.trustHostHeader) {
			...
			if (
				(cacheHeader == null ? void 0 : cacheHeader.toUpperCase()) !==
					"REVALIDATED" &&
		...
		throw new Error(
			`Failed to revalidate ${urlPath}: ${((0, _iserror.default))(err) ? err.message : err}`,
		);
	}
}
async function apiResolver(
	...
	try {
		...
		const bodyParser =
			((_config_api = config.api) == null ? void 0 : _config_api.bodyParser) !==
			false;
		const responseLimit =
			((_config_api1 = config.api) == null
				? void 0
				: _config_api1.responseLimit) ?? true;
		const externalResolver =
			((_config_api2 = config.api) == null
				? void 0
				: _config_api2.externalResolver) || false;
		// Parsing of cookies
		...
		(0, _index.setLazyProp)(
			...
			"previewData",
			() =>
				(0, _trygetpreviewdata.tryGetPreviewData)(
					req,
					res,
					apiContext,
		...
		if (bodyParser && !apiReq.body) {
			apiReq.body = await (0, _parsebody.parseBody)(
				apiReq,
		...
		apiRes.end = (...args) => {
			...
			if (responseLimit && contentLength >= maxContentLength) {
				console.warn(
					`API response for ${req.url} exceeds ${_bytes.default.format(maxContentLength)}. API Routes are meant to respond quickly. https://nextjs.org/docs/messages/api-routes-response-size-limit`,
				);
			}
			return endResponse.apply(apiRes, args);
		};
		apiRes.status = (statusCode) =>
			(0, _index.sendStatusCode)(apiRes, statusCode);
		apiRes.send = (data) => sendData(apiReq, apiRes, data);
		apiRes.json = (data) => sendJson(apiRes, data);
		apiRes.redirect = (statusOrUrl, url) =>
			(0, _index.redirect)(apiRes, statusOrUrl, url);
		apiRes.setDraftMode = (
			options = {
				enable: true,
			},
		...
		apiRes.clearPreviewData = (options = {}) =>
			(0, _index.clearPreviewData)(apiRes, options);
		apiRes.revalidate = (urlPath, opts) =>
