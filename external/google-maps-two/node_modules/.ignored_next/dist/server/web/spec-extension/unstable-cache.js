...
async function cacheNewResult(
	...
	await incrementalCache.set(
		cacheKey,
		{
			...
			revalidate:
				typeof revalidate !== "number" ? _constants.CACHE_ONE_YEAR : revalidate,
		},
		{
			revalidate,
			fetchCache: true,
...
function unstable_cache(cb, keyParts, options = {}) {
	...
	const cachedCb = async (...args) => {
		...
		const maybeIncrementalCache =
			(store == null ? void 0 : store.incrementalCache) ||
		...
		const { pathname, searchParams } = new URL(
			(store == null ? void 0 : store.urlPathname) || "/",
			"http://n",
		);
		...
		if (store) {
			...
			const result =
				await _staticgenerationasyncstorageexternal.staticGenerationAsyncStorage.run(
					{
						...store,
						// force any nested fetches to bypass cache so they revalidate
						// when the unstable_cache call is revalidated
