...
const _nodebuffer = /*#__PURE__*/ _interop_require_default(
	require("node:buffer"),
);
const _nodeevents = /*#__PURE__*/ _interop_require_default(
	require("node:events"),
);
			...
			context.__next_eval__ = function __next_eval__(fn) {
				...
				if (!warnedEvals.has(key)) {
					const warning = getServerError(
						new Error(`Dynamic Code Evaluation (e. g. 'eval', 'new Function') not allowed in Edge Runtime
...
Learn More: https://nextjs.org/docs/messages/edge-dynamic-code-evaluation`),
			...
			context.setInterval = (...args) =>
				_resourcemanagers.intervalsManager.add(args);
			// @ts-ignore the timeouts have weird types in the edge runtime
			context.clearInterval = (interval) =>
				_resourcemanagers.intervalsManager.remove(interval);
			// @ts-ignore the timeouts have weird types in the edge runtime
			context.setTimeout = (...args) =>
				_resourcemanagers.timeoutsManager.add(args);
			// @ts-ignore the timeouts have weird types in the edge runtime
			context.clearTimeout = (timeout) =>
				_resourcemanagers.timeoutsManager.remove(timeout);
			return context;
		},
	});
