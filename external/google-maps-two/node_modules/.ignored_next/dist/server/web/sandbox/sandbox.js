 ...
 */ function withTaggedErrors(fn) {
	if (process.env.NODE_ENV === "development") {
		const {
			getServerError,
		} = require("../../../client/components/react-dev-overlay/server/middleware");
		return (params) =>
			fn(params)
				.then((result) => {
					...
					return {
						...
						waitUntil:
							result == null
								? void 0
								: (_result_waitUntil = result.waitUntil) == null
									? void 0
									: _result_waitUntil.catch((error) => {
...
const run = withTaggedErrors(async function runWithTaggedErrors(params) {
	...
	const cloned = !["HEAD", "GET"].includes(params.request.method)
		? (_params_request_body = params.request.body) == null
			? void 0
			: _params_request_body.cloneBodyStream()
		: undefined;
	...
	try {
			...
			async () => {
				result = await edgeFunction({
					request: {
						...
						body:
							cloned &&
							(0, _bodystreams.requestToBodyStream)(
								runtime.context,
								KUint8Array,
								cloned,
							),
		...
		await ((_params_request_body1 = params.request.body) == null
			? void 0
			: _params_request_body1.finalize());
	}
});
