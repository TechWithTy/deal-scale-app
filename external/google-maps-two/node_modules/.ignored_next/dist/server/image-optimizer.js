...
const _contentdisposition = /*#__PURE__*/ _interop_require_default(
	require("next/dist/compiled/content-disposition"),
);
const _getorientation = require("next/dist/compiled/get-orientation");
const _imagesize = /*#__PURE__*/ _interop_require_default(
	require("next/dist/compiled/image-size"),
);
const _isanimated = /*#__PURE__*/ _interop_require_default(
	require("next/dist/compiled/is-animated"),
);
...
try {
	...
	if (sharp && sharp.concurrency() > 1) {
		...
		sharp.concurrency(
			Math.floor(Math.max((0, _os.cpus)().length / divisor, 1)),
		);
	}
} catch (e) {
...
class ImageOptimizerCache {
	static validateParams(req, query, nextConfig, isDev) {
		...
		const remotePatterns =
			((_nextConfig_images = nextConfig.images) == null
				? void 0
				: _nextConfig_images.remotePatterns) || [];
		const localPatterns =
			(_nextConfig_images1 = nextConfig.images) == null
				? void 0
				: _nextConfig_images1.localPatterns;
		const qualities =
			(_nextConfig_images2 = nextConfig.images) == null
				? void 0
				: _nextConfig_images2.qualities;
		const { url, w, q } = query;
		let href;
		if (domains.length > 0) {
			_log.warnOnce(
		...
		if (url.startsWith("/")) {
			...
			if (
				/\/_next\/image($|\/)/.test(
					decodeURIComponent(
						((_parseUrl = (0, _url1.parseUrl)(url)) == null
							? void 0
							: _parseUrl.pathname) ?? "",
					),
				)
			...
			if (
				!(0, _matchremotepattern.hasRemoteMatch)(
					domains,
					remotePatterns,
					hrefParsed,
				)
	...
	async get(cacheKey) {
		try {
			...
			for (const file of files) {
				...
				const buffer = await _fs.promises.readFile(
					(0, _path.join)(cacheDir, file),
				);
	...
	async set(cacheKey, value, { revalidate }) {
		...
		try {
			await writeToCacheDir(
				(0, _path.join)(this.cacheDir, cacheKey),
				value.extension,
				revalidate,
				expireAt,
				value.buffer,
...
async function imageOptimizer(imageUpstream, paramsResult, nextConfig, isDev) {
	...
	if (
		...
		(0, _isanimated.default)(upstreamBuffer)
	) {
		_log.warnOnce(
	...
	if (mimeType) {
		...
		(0, _servestatic.getExtension)(upstreamType) &&
		upstreamType !== WEBP &&
		upstreamType !== AVIF
	) {
		contentType = upstreamType;
	...
	try {
		...
		if (optimizedBuffer) {
			if (isDev && width <= BLUR_IMG_SIZE && quality === BLUR_QUALITY) {
				...
				optimizedBuffer = Buffer.from(
					unescape((0, _imageblursvg.getImageBlurSvg)(opts)),
				);
...
function setResponseHeaders(
	...
	res.setHeader(
		"Content-Disposition",
		(0, _contentdisposition.default)(fileName, {
...
async function getImageSize(
	buffer, // Should match VALID_BLUR_EXT
	extension,
) {
