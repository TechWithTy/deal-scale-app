	...
	getRequestHandler() {
		return async (req, res, parsedUrl) => {
			return (0, _tracer.getTracer)().trace(
				_constants2.NextServerSpan.getRequestHandler,
				async () => {
	...
	async [SYMBOL_LOAD_CONFIG]() {
		...
		const config =
			...
			(await (0, _config.default)(
				this.options.dev
					? _constants1.PHASE_DEVELOPMENT_SERVER
					: _constants1.PHASE_PRODUCTION_SERVER,
				dir,
				{
		...
		if (process.env.NODE_ENV === "production") {
			try {
				const serializedConfig = require(
					_path.default.join(dir, ".next", _constants1.SERVER_FILES_MANIFEST),
				).config;
	...
	async getServerRequestHandler() {
		...
		if (!this.reqHandlerPromise) {
			this.reqHandlerPromise = this.getServer().then((server) => {
				this.reqHandler = (0, _tracer.getTracer)().wrap(
					_constants2.NextServerSpan.getServerRequestHandler,
