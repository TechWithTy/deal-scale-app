...
const _sharedmodules = /*#__PURE__*/ _interop_require_wildcard(
...
class AppRouteRouteModule extends _routemodule.RouteModule {
	...
	constructor({ userland, definition, resolvedPagePath, nextConfigOutput }) {
		 ...
		 */ this.requestAsyncStorage =
			_requestasyncstorageexternal.requestAsyncStorage;
		/**
		 * A reference to the static generation async storage.
		 */ this.staticGenerationAsyncStorage =
			_staticgenerationasyncstorageexternal.staticGenerationAsyncStorage;
		/**
		 ...
		 */ this.actionAsyncStorage =
			_actionasyncstorageexternal.actionAsyncStorage;
		...
		if (process.env.NODE_ENV === "development") {
			...
			const lowercased = _http.HTTP_METHODS.map((method) =>
				method.toLowerCase(),
			);
	 ...
	 */ resolve(method) {
		...
		if (!(0, _http.isHTTPMethod)(method))
			return _responsehandlers.handleBadRequestResponse;
		// Return the handler.
	 ...
	 */ async execute(rawRequest, context) {
		...
		const response = await this.actionAsyncStorage.run(
			...
			() =>
				_requestasyncstoragewrapper.RequestAsyncStorageWrapper.wrap(
					this.requestAsyncStorage,
					requestContext,
					() =>
						_staticgenerationasyncstoragewrapper.StaticGenerationAsyncStorageWrapper.wrap(
							...
							(staticGenerationStore) => {
								...
								const route = (0,
								_getpathnamefromabsolutepath.getPathnameFromAbsolutePath)(
									this.resolvedPagePath,
								);
								(_getTracer_getRootSpanAttributes = (0,
								_tracer.getTracer)().getRootSpanAttributes()) == null
									? void 0
									: _getTracer_getRootSpanAttributes.set("next.route", route);
								return (0, _tracer.getTracer)().trace(
									_constants.AppRouteRouteHandlersSpan.runHandler,
									{
									...
									async () => {
										...
										const res = await handler(request, {
											params: context.params
												? (0, _parsedurlquerytoparams.parsedUrlQueryToParams)(
														context.params,
													)
												: undefined,
										});
		...
		switch (prop) {
			case "headers":
				return (
					...
					(target[headersSymbol] = _headers.HeadersAdapter.seal(
						new Headers({}),
					))
				);
			case "cookies":
				return (
					...
					(target[cookiesSymbol] = _requestcookies.RequestCookiesAdapter.seal(
	...
	get(target, prop, receiver) {
		switch (prop) {
			...
			case "href":
				return (
					target[hrefSymbol] ||
					(target[hrefSymbol] = (0, _cleanurl.cleanURL)(target.href).href)
				);
			case "toJSON":
			case "toString":
				return (
	...
	const nextUrlHandlers = {
		get(target, prop, receiver) {
			switch (prop) {
				...
				case "toString":
				case "origin": {
					(0, _dynamicrendering.trackDynamicDataAccessed)(
						staticGenerationStore,
						`nextUrl.${prop}`,
					);
					return _reflect.ReflectAdapter.get(target, prop, receiver);
