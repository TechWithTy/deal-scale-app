...
async function createComponentTreeInternal({
	...
	const { page, layoutOrPagePath, segment, components, parallelRoutes } = (0,
	...
	const [layoutOrPageMod] = await (0, _tracer.getTracer)().trace(
	...
	if (
		...
		staticGenerationStore.fetchCache =
	...
	if (
		...
		if (
			...
			staticGenerationStore.isStaticGeneration &&
			ctx.defaultRevalidate === 0 && // If the postpone API isn't available, we can't postpone the render and
			// therefore we can't use the dynamic API.
			!staticGenerationStore.prerenderState
		) {
	...
	const LayoutOrPage = layoutOrPageMod
		? await (0, _interopdefault.interopDefault)(layoutOrPageMod)
		: undefined;
	/**
		...
		Object.keys(parallelRoutes).map(async (parallelRouteKey) => {
			...
			if (
				...
				if (process.env.NODE_ENV === "development" && missingSlots) {
					...
					if (
						(_parsedTree_layoutOrPagePath = parsedTree.layoutOrPagePath) == null
							? void 0
							: _parsedTree_layoutOrPagePath.endsWith(
	...
	if (!Component) {
		return [
			...
			// caution, for now.
			/*#__PURE__*/ (0, _jsxruntime.jsxs)(_jsxruntime.Fragment, {
				children: [layerAssets, parallelRouteProps.children],
			}),
			loadingData,
		];
	}
	...
	const isClientComponent = (0, _clientreference.isClientReference)(
		layoutOrPageMod,
	);
	...
	if (
		NotFound &&
		asNotFound && // In development, it could hit the parallel-route-default not found, so we only need to check the segment.
	...
	if (isPage) {
		...
		if (isClientComponent) {
			...
			segmentElement = /*#__PURE__*/ (0, _jsxruntime.jsxs)(
