...
async function addRevalidationHeader(
	res,
	{ staticGenerationStore, requestStore },
) {
	var _staticGenerationStore_incrementalCache,
		_staticGenerationStore_revalidatedTags;
	await Promise.all([
	...
	const isTagRevalidated = (
		(_staticGenerationStore_revalidatedTags =
			staticGenerationStore.revalidatedTags) == null
			? void 0
			: _staticGenerationStore_revalidatedTags.length
	)
		? 1
		: 0;
 ...
 */ async function createForwardedActionResponse(
	...
	const proto =
		((_staticGenerationStore_incrementalCache =
			staticGenerationStore.incrementalCache) == null
			? void 0
			: _staticGenerationStore_incrementalCache.requestProtocol) || "https";
...
async function createRedirectRenderResult(
	...
	if (isAppRelativeRedirect) {
		...
		if (staticGenerationStore.revalidatedTags) {
			var _staticGenerationStore_incrementalCache_prerenderManifest_preview,
				_staticGenerationStore_incrementalCache_prerenderManifest,
				_staticGenerationStore_incrementalCache1;
			...
			forwardedHeaders.set(
				...
				((_staticGenerationStore_incrementalCache1 =
					...
					? void 0
					: (_staticGenerationStore_incrementalCache_prerenderManifest =
								_staticGenerationStore_incrementalCache1.prerenderManifest) ==
							null
						? void 0
						: (_staticGenerationStore_incrementalCache_prerenderManifest_preview =
									_staticGenerationStore_incrementalCache_prerenderManifest.preview) ==
								null
							? void 0
							: _staticGenerationStore_incrementalCache_prerenderManifest_preview.previewModeId) ||
					"",
			);
		}
...
async function handleAction({
	...
	if (!originDomain) {
		...
		if (
			...
			if (host) {
				...
				console.error(
					`\`${host.type}\` header with value \`${limitUntrustedHeaderValueForLogs(host.value)}\` does not match \`origin\` header with value \`${limitUntrustedHeaderValueForLogs(originDomain)}\` from a forwarded Server Actions request. Aborting the action.`,
				);
			} else {
	...
	if (actionId) {
		const forwardedWorker = (0, _actionutils.selectWorkerForForwarding)(
			actionId,
			page,
...
To configure the body size limit for Server Actions, see: https://nextjs.org/docs/app/api-reference/next-config-js/serverActions#bodysizelimit`,
				...
				)[actionId]; // `actionId` must exist if we got here, as otherwise we would have thrown an error above
		...
		if ((0, _redirect.isRedirectError)(err)) {
			...
			if (err.mutableCookies) {
				...
				// response.
				if (
					(0, _requestcookies.appendMutableCookies)(headers, err.mutableCookies)
				) {
		...
		if (isFetchAction) {
			...
			await Promise.all([
				(_staticGenerationStore_incrementalCache1 =
					staticGenerationStore.incrementalCache) == null
					? void 0
					: _staticGenerationStore_incrementalCache1.revalidateTag(
							staticGenerationStore.revalidatedTags || [],
						),
				...Object.values(staticGenerationStore.pendingRevalidates || {}),
			]);
 ...
 */ function getActionModIdOrError(actionId, serverModuleMap) {
	try {
		...
		throw new Error(
			`Failed to find Server Action "${actionId}". This request might be from an older or newer deployment. ${err instanceof Error ? `Original error: ${err.message}` : ""}`,
		);
	}
}
