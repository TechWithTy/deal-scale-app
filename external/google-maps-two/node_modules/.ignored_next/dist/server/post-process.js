		...
		this.mutate = async (markup, fontDefinitions, options) => {
			...
			fontDefinitions.forEach((fontDef) => {
				...
				if (!fontContent) {
					...
					const provider = _constants.OPTIMIZED_FONT_PROVIDERS.find((p) =>
		...
		process.env.NEXT_RUNTIME !== "edge" && renderOpts.optimizeFonts
			? async (html) => {
					const getFontDefinition = (url) => {
						...
						return (
							((_renderOpts_fontManifest_find = renderOpts.fontManifest.find(
								(font) => {
								...
								? void 0
								: _renderOpts_fontManifest_find.content) || ""
						);
					};
...
registerPostProcessor(
	"Inline-Fonts",
	new FontOptimizerMiddleware(), // Using process.env because passing Experimental flag through loader is not possible.
	// @ts-ignore
