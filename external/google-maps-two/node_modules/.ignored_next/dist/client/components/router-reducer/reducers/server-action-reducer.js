...
async function fetchServerAction(state, nextUrl, param) {
	...
	const redirectLocation = location
		? new URL(
				(0, _addbasepath.addBasePath)(location), // Ensure relative redirects in Server Actions work, e.g. redirect('./somewhere-else')
...
function serverActionReducer(state, action) {
	...
	const nextUrl =
		state.nextUrl &&
		(0, _hasinterceptionrouteincurrenttree.hasInterceptionRouteInCurrentTree)(
			state.tree,
		)
			? state.nextUrl
			: null;
	...
	return mutable.inFlightServerAction.then(
		async (param) => {
			...
			if (redirectLocation) {
				const newHref = (0, _createhreffromurl.createHrefFromUrl)(
					redirectLocation,
					false,
				);
			...
			for (const flightDataPath of flightData) {
				...
				_applyrouterstatepatchtotree.applyRouterStatePatchToTree)(
					...
					treePatch,
					redirectLocation
						? (0, _createhreffromurl.createHrefFromUrl)(redirectLocation)
						: state.canonicalUrl,
				);
				if (newTree === null) {
				...
				if (rsc !== null) {
					...
					(0, _filllazyitemstillleafwithhead.fillLazyItemsTillLeafWithHead)(
						cache, // Existing cache is not passed in as `router.refresh()` has to invalidate the entire cache.
						undefined,
						treePatch,
						cacheNodeSeedData,
						head,
