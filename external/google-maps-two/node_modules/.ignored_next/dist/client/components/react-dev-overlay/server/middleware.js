...
function getModuleSource(compilation, module1) {
	...
	return (_compilation_codeGenerationResults_get_sources_get =
		(_compilation_codeGenerationResults_get =
			compilation.codeGenerationResults.get(module1)) == null
			? void 0
			: _compilation_codeGenerationResults_get.sources.get("javascript")) !=
		null
		? _compilation_codeGenerationResults_get_sources_get
		: null;
}
...
async function findOriginalSourcePositionAndContent(webpackSource, position) {
	const consumer = await new _sourcemap08.SourceMapConsumer(
		webpackSource.map(),
	);
	try {
		...
		const sourcePosition = consumer.originalPositionFor({
			line: position.line,
			column:
				(_position_column = position.column) != null ? _position_column : 0,
		});
		...
		const sourceContent =
			(_consumer_sourceContentFor = consumer.sourceContentFor(
				sourcePosition.source,
				/* returnNullOnMissing */ true,
			)) != null
				? _consumer_sourceContentFor
				: null;
		return {
			sourcePosition,
			sourceContent,
...
function findOriginalSourcePositionAndContentFromCompilation(
	...
	return (_module_buildInfo_importLocByPath_get =
		module1 == null
			? void 0
		...
		? _module_buildInfo_importLocByPath_get
		: null;
}
async function createOriginalStackFrame(param) {
	...
	const traced = {
		file: sourceContent
			? _path.default.relative(rootDirectory, filePath)
			: sourcePosition.source,
		lineNumber: sourcePosition.line,
		column:
			((_sourcePosition_column = sourcePosition.column) != null
				? _sourcePosition_column
				: 0) + 1,
		methodName:
			sourcePosition.name ||
...
async function getSourceById(isFile, id, compilation) {
	if (isFile) {
		const fileContent = await _fs.promises
			.readFile(id, "utf-8")
			.catch(() => null);
		if (fileContent == null) {
			return null;
...
function getOverlayMiddleware(options) {
	return async function (req, res, next) {
		...
		const frame = {
			...
			lineNumber:
				parseInt(
					(_searchParams_get = searchParams.get("lineNumber")) != null
						? _searchParams_get
						: "0",
					10,
				) || 0,
			column:
				parseInt(
					(_searchParams_get1 = searchParams.get("column")) != null
						? _searchParams_get1
						: "0",
					10,
				) || 0,
		...
		if (pathname === "/__nextjs_original-stack-frame") {
			...
			try {
				if (isClient || isAppDirectory) {
					var _options_stats;
					compilation =
						(_options_stats = options.stats()) == null
							? void 0
							: _options_stats.compilation;
				...
				if ((isServer || isAppDirectory) && source === null) {
					...
					compilation =
						(_options_serverStats = options.serverStats()) == null
							? void 0
							: _options_serverStats.compilation;
					source = await getSourceById(isFile, moduleId, compilation);
				}
				...
				if ((isEdgeServer || isAppDirectory) && source === null) {
					...
					compilation =
						(_options_edgeServerStats = options.edgeServerStats()) == null
							? void 0
							: _options_edgeServerStats.compilation;
					source = await getSourceById(isFile, moduleId, compilation);
				}
			} catch (err) {
			...
			const fileExists = await _fs.promises
				.access(filePath, _fs.constants.F_OK)
				.then(
					() => true,
					() => false,
				);
			...
			try {
				var _frame_column;
				await (0, _launchEditor.launchEditor)(
					filePath,
					frame.lineNumber,
					(_frame_column = frame.column) != null ? _frame_column : 1,
				);
			} catch (err) {
