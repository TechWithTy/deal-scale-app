...
const nextFontLocalFontLoader = async ({
	...
	const {
		...
		style: defaultStyle,
	} = (0, validate_local_font_function_call_1.validateLocalFontFunctionCall)(
		functionName,
		data[0],
	);
	...
	const fontFiles = await Promise.all(
		src.map(async ({ path, style, weight, ext, format }) => {
			...
			const fileBuffer = await (0, util_1.promisify)(loaderContext.fs.readFile)(
				resolved,
			);
			...
			try {
				fontMetadata =
					fontFromBuffer === null || fontFromBuffer === void 0
						? void 0
						: fontFromBuffer(fileBuffer);
			} catch (e) {
	...
	if (adjustFontFallback !== false) {
		const fallbackFontFile = (0,
		pick_font_file_for_fallback_generation_1.pickFontFileForFallbackGeneration)(
			fontFiles,
		);
		if (fallbackFontFile.fontMetadata) {
			adjustFontFallbackMetrics = (0,
			get_fallback_metrics_from_font_file_1.getFallbackMetricsFromFontFile)(
