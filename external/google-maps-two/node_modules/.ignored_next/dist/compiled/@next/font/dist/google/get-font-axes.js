...
function getFontAxes(fontFamily, weights, styles, selectedVariableAxes) {
	...
	if (weights[0] === "variable") {
		...
		const allAxes =
			google_fonts_metadata_1.googleFontsMetadata[fontFamily].axes;
		if (!allAxes) {
		...
		if (selectedVariableAxes) {
			...
			if (defineAbleAxes.length === 0) {
				(0, next_font_error_1.nextFontError)(
					`Font \`${fontFamily}\` has no definable \`axes\``,
				);
			}
		...
		for (const { tag, min, max } of allAxes) {
			if (tag === "wght") {
				...
				selectedVariableAxes === null || selectedVariableAxes === void 0
					? void 0
					: selectedVariableAxes.includes(tag)
			) {
				if (!variableAxes) {
					variableAxes = [];
				}
