(function () {
	...
	!(function () {
		t.d = function (g, b) {
			for (var C in b) {
				if (t.o(b, C) && !t.o(g, C)) {
					Object.defineProperty(g, C, { enumerable: true, get: b[C] });
				}
			}
		};
	})();
	...
	var b,
		...
		o = function (t) {
			...
			if (g) {
				...
				if (
					0 === g.domContentLoadedEventStart ||
					t < g.domContentLoadedEventStart
				)
					return "dom-interactive";
				if (0 === g.domComplete || t < g.domComplete)
				...
				for (; t && 9 !== t.nodeType; ) {
					var C = t,
						F = C.id
							? "#" + C.id
							: u(C) +
								(C.className && C.className.length
									? "." + C.className.replace(/\s+/g, ".")
									: "");
		...
		v = function (t, g, b) {
			try {
				if (PerformanceObserver.supportedEntryTypes.includes(t)) {
					...
					return (
						C.observe(Object.assign({ type: t, buffered: !0 }, b || {})), C
					);
				}
			} catch (t) {}
		},
		p = function (t, g) {
			var b = function n(b) {
				("pagehide" !== b.type && "hidden" !== document.visibilityState) ||
					(t(b),
					g &&
		...
		h = function (t, g, b, C) {
			...
			return function (A) {
				g.value >= 0 &&
					(A || C) &&
					((P = g.value - (F || 0)) || void 0 === F) &&
					((F = g.value),
					(g.delta = P),
					(g.rating = (function (t, g) {
				...
				o = function (t) {
					t.forEach(function (t) {
						"first-contentful-paint" === t.name &&
							(N && N.disconnect(),
							t.startTime < F.firstHiddenTime &&
								((P.value = t.startTime - d()), P.entries.push(t), b(!0)));
					});
				},
				A =
					window.performance &&
						...
						t.forEach(function (t) {
							if (!t.hadRecentInput) {
								var g = A[0],
									b = A[A.length - 1];
								P &&
								t.startTime - b.startTime < 1e3 &&
								t.startTime - g.startTime < 5e3
									? ((P += t.value), A.push(t))
									: ((P = t.value), (A = [t])),
		...
		L = function (t, g) {
			S(function (g) {
				!(function (t) {
					if (t.entries.length) {
						var g = a(),
							b = t.entries[t.entries.length - 1];
						if (g) {
							var C = g.activationStart || 0,
								F = Math.max(0, g.responseStart - C);
							t.attribution = {
								timeToFirstByte: F,
			...
			var F,
				...
				s = function (t) {
					t.startTime < N.firstHiddenTime &&
						((q.value = t.processingStart - t.startTime),
						q.entries.push(t),
						F(!0));
				},
				f = function (t) {
		...
		G = function (t, g) {
			...
			var C,
				...
				a = function (t) {
						...
						(t.interactionId && z(t), "first-input" === t.entryType) &&
							!te.some(function (g) {
								return g.entries.some(function (g) {
									return (
										t.duration === g.duration && t.startTime === g.startTime
									);
								});
							}) &&
							z(t);
					...
					var g,
						b = ((g = Math.min(te.length - 1, Math.floor(U() / 50))), te[g]);
					b &&
						b.latency !== F.value &&
						((F.value = b.latency), (F.entries = b.entries), C());
				},
				P = v("event", a, { durationThreshold: g.durationThreshold || 40 });
			(C = h(t, F, b, g.reportAllChanges)),
				P &&
					(P.observe({ type: "first-input", buffered: !0 }),
					p(function () {
						a(P.takeRecords()),
							F.value < 0 && U() > 0 && ((F.value = 0), (F.entries = [])),
							C(!0);
					}),
					l(function () {
						(te = []),
			...
			!(function (t, g) {
				...
				var b,
					...
					o = function (t) {
						...
						if (g) {
							var C = g.startTime - d();
							C < F.firstHiddenTime && ((P.value = C), (P.entries = [g]), b());
						}
					},
				...
				if (A) {
					...
					var c = function () {
						re[P.id] ||
							(o(A.takeRecords()), A.disconnect(), (re[P.id] = !0), b(!0));
					};
					["keydown", "click"].forEach(function (t) {
						addEventListener(t, c, { once: !0, capture: !0 });
					}),
						p(c, !0),
						l(function (F) {
							(P = m("LCP")),
								...
								requestAnimationFrame(function () {
									requestAnimationFrame(function () {
										(P.value = performance.now() - F.timeStamp),
											(re[P.id] = !0),
											b(!0);
									});
								});
				...
				!(function (t) {
					if (t.entries.length) {
						var g = a();
						if (g) {
							var b = g.activationStart || 0,
								C = t.entries[t.entries.length - 1],
								F =
									C.url &&
									performance.getEntriesByType("resource").filter(function (t) {
										return t.name === C.url;
									})[0],
								P = Math.max(0, g.responseStart - b),
								A = Math.max(P, F ? (F.requestStart || F.startTime) - b : 0),
								N = Math.max(A, F ? F.responseEnd - b : 0),
								q = Math.max(N, C ? C.startTime - b : 0),
								j = {
									element: c(C.element),
									timeToFirstByte: P,
		...
		X = function (t, g) {
			...
			var b = [800, 1800],
				C = m("TTFB"),
				F = h(t, C, b, g.reportAllChanges);
			ie(function () {
				var P = a();
				if (P) {
					if (
						((C.value = Math.max(P.responseStart - d(), 0)),
		...
		Y = function (t, g) {
			X(function (g) {
				!(function (t) {
					if (t.entries.length) {
						var g = t.entries[0],
							b = g.activationStart || 0,
							C = Math.max(g.domainLookupStart - b, 0),
							F = Math.max(g.connectStart - b, 0),
							P = Math.max(g.requestStart - b, 0);
						t.attribution = {
							waitingTime: C,
							dnsTime: F - C,
							connectionTime: P - F,
