...
async function verifyRootLayout({
	dir,
	appDir,
	tsconfigPath,
	pagePath,
	...
	try {
		...
		const isFileUnderAppDir = pagePath.startsWith(
		...
		if (typeof availableDir === "string") {
			...
			const hasTsConfig = await _fs.promises.access(resolvedTsConfigPath).then(
				() => true,
				() => false,
			);
			rootLayoutPath = _path.default.join(
				appDir,
				availableDir,
