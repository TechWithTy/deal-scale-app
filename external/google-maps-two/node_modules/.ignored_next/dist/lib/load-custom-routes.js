...
function checkRedirect(route) {
	...
	if (
		route.statusCode &&
		!_redirectstatus.allowedStatusCodes.has(route["statusCode"])
	) {
...
function checkCustomRoutes(routes, type) {
	...
	for (const route of routes) {
		if (!route || typeof route !== "object") {
			console.error(
				`The route ${JSON.stringify(route)} is not a valid object with \`source\`${type !== "middleware" ? ` and \`${type === "header" ? "headers" : "destination"}\`` : ""}`,
			);
			numInvalidRoutes++;
			continue;
		}
		if (
			...
			console.error(
				`The route ${route.source} rewrites urls outside of the basePath. Please use a destination that starts with \`http://\` or \`https://\` https://nextjs.org/docs/messages/invalid-external-rewrite`,
			);
			numInvalidRoutes++;
			continue;
		}
		...
		if (typeof route.source === "string" && route.source.startsWith("/")) {
			...
			const { tokens, error, regexStr } = (0, _trytoparsepath.tryToParsePath)(
				route.source,
			);
			if (error) {
		...
		if (hasInvalidKeys || hasInvalidParts) {
			console.error(
				`${invalidParts.join(", ")}${invalidKeys.length ? (hasInvalidParts ? "," : "") + ` invalid field${invalidKeys.length === 1 ? "" : "s"}: ` + invalidKeys.join(",") : ""} for route ${JSON.stringify(route)}`,
			);
			console.error();
			numInvalidRoutes++;
		}
...
function processRoutes(routes, config, type) {
	...
	if (config.i18n && type === "redirect") {
		var _config_i18n;
		for (const item of ((_config_i18n = config.i18n) == null
			? void 0
			: _config_i18n.domains) || []) {
			defaultLocales.push({
	...
	for (const r of _routes) {
		...
		const isExternal = !((_r_destination = r.destination) == null
			? void 0
			: _r_destination.startsWith("/"));
		...
		if (config.i18n && r.locale !== false) {
			...
			if (
				r.destination &&
				((_r_destination1 = r.destination) == null
					? void 0
					: _r_destination1.startsWith("/"))
			) {
