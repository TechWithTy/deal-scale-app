...
const cleanupWorkers = (worker) => {
	...
	for (const curWorker of ((_worker__workerPool = worker._workerPool) == null
		? void 0
		: _worker__workerPool._workers) || []) {
		var _curWorker__child;
		(_curWorker__child = curWorker._child) == null
			? void 0
			: _curWorker__child.kill("SIGINT");
	}
};
class Worker {
	constructor(workerPath, options) {
		...
		const createWorker = () => {
			...
			this._worker = new _jestworker.Worker(workerPath, {
				...
				forkOptions: {
					...
					env: {
						...(((_farmOptions_forkOptions = farmOptions.forkOptions) == null
							? void 0
							: _farmOptions_forkOptions.env) || {}),
						...process.env,
						IS_NEXT_WORKER: "true",
					},
				},
			 ...
			 */ if (!farmOptions.enableWorkerThreads) {
				...
				for (const worker of ((_this__worker__workerPool =
					this._worker._workerPool) == null
					? void 0
					: _this__worker__workerPool._workers) || []) {
					var _worker__child;
					(_worker__child = worker._child) == null
						? void 0
						: _worker__child.on("exit", (code, signal) => {
		...
		const onHanging = () => {
			...
			logger.warn(
				`Sending SIGTERM signal to Next.js build worker due to timeout${timeout ? ` of ${timeout / 1000} seconds` : ""}. Subsequent errors may be a result of the worker exiting.`,
			);
