...
const _tar = /*#__PURE__*/ _interop_require_default(
	require("next/dist/compiled/tar"),
);
...
async function extractBinary(outputDirectory, pkgName, tarFileName) {
	const cacheDirectory = (0, _getcachedirectory.getCacheDirectory)(
		"next-swc",
	...
	if (
		!_fs.default.existsSync(_path.default.join(cacheDirectory, tarFileName))
	) {
		_log.info(`Downloading swc package ${pkgName}...`);
		await _fs.default.promises.mkdir(cacheDirectory, {
			recursive: true,
		});
		const tempFile = _path.default.join(
			cacheDirectory,
		...
		await _fs.default.promises.rename(
			tempFile,
			_path.default.join(cacheDirectory, tarFileName),
		);
	}
	...
	if (cacheFiles.length > MAX_VERSIONS_TO_CACHE) {
		...
		for (let i = 0; i++; i < cacheFiles.length - MAX_VERSIONS_TO_CACHE) {
			await _fs.default.promises
				.unlink(_path.default.join(cacheDirectory, cacheFiles[i]))
				.catch(() => {});
		}
	}
}
