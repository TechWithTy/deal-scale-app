...
async function createSelfSignedCertificate(host, certDir = "certificates") {
	try {
		...
		const resolvedCertDir = _path.default.resolve(
			process.cwd(),
			`./${certDir}`,
		);
		...
		const caLocation = (0, _child_process.execSync)(`"${binaryPath}" -CAROOT`)
			.toString()
			.trim();
		...
		if (_fs.default.existsSync(gitignorePath)) {
			const gitignore = await _fs.default.promises.readFile(
				gitignorePath,
				"utf8",
			);
