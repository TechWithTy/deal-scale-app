...
async function verifyTypeScriptSetup({
	...
	try {
		...
		const intent = await (0, _getTypeScriptIntent.getTypeScriptIntent)(
			dir,
			intentDirs,
			tsconfigPath,
		);
		...
		let deps = await (0, _hasnecessarydependencies.hasNecessaryDependencies)(
			dir,
			requiredPackages,
		);
		if (
			((_deps_missing = deps.missing) == null ? void 0 : _deps_missing.length) >
			0
		) {
			if (_ciinfo.isCI) {
		...
		await (0, _writeConfigurationDefaults.writeConfigurationDefaults)(
			ts,
