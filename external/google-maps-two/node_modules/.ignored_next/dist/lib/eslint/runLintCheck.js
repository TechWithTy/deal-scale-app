...
async function cliPrompt(cwd) {
	console.log(
		(0, _picocolors.bold)(
			`${(0, _picocolors.cyan)("?")} How would you like to configure ESLint? https://nextjs.org/docs/basic-features/eslint`,
		),
	);
	try {
		const cliSelect = (
		...
		const { value } = await cliSelect({
			...
			valueRenderer: ({ title, recommended }, selected) => {
				const name = selected
					? (0, _picocolors.bold)(
							(0, _picocolors.underline)((0, _picocolors.cyan)(title)),
						)
					: title;
				return (
					name +
					(recommended
						? (0, _picocolors.bold)((0, _picocolors.yellow)(" (recommended)"))
						: "")
				);
			},
...
async function lint(
	...
	try {
		...
		const deps = await (0, _hasnecessarydependencies.hasNecessaryDependencies)(
			baseDir,
			requiredPackages,
		);
		...
		if (deps.missing.some((dep) => dep.pkg === "eslint")) {
			_log.error(
				`ESLint must be installed${lintDuringBuild ? " in order to run during builds:" : ":"} ${(0, _picocolors.bold)((0, _picocolors.cyan)((packageManager === "yarn" ? "yarn add --dev" : packageManager === "pnpm" ? "pnpm install --save-dev" : "npm install --save-dev") + " eslint"))}`,
			);
			return null;
		}
		...
		const { ESLint } = mod;
		let eslintVersion =
			(ESLint == null ? void 0 : ESLint.version) ??
			((_mod_CLIEngine = mod.CLIEngine) == null
				? void 0
				: _mod_CLIEngine.version);
		...
		for (const configFile of [eslintrcFile, pkgJsonPath]) {
			...
			if (
				(_completeConfig_plugins = completeConfig.plugins) == null
					? void 0
					: _completeConfig_plugins.includes("@next/next")
			) {
		...
		if (nextEslintPluginIsEnabled) {
			...
			for (const rule of pagesDirRules) {
				...
				if (
					!((_options_baseConfig_rules = options.baseConfig.rules) == null
						? void 0
						: _options_baseConfig_rules[rule]) &&
					!((_options_baseConfig_rules1 = options.baseConfig.rules) == null
						? void 0
						: _options_baseConfig_rules1[
		...
		const formattedResult = (0, _customFormatter.formatResults)(
			baseDir,
			results,
			selectedFormatter == null ? void 0 : selectedFormatter.format,
		);
		...
		if (outputFile)
			await (0, _writeOutputFile.writeOutputFile)(
				outputFile,
		...
		return {
			...
			isError:
				((_ESLint_getErrorResults = ESLint.getErrorResults(results)) == null
					? void 0
					: _ESLint_getErrorResults.length) > 0 ||
						...
						? require(
								_path.default.join(
									_path.default.dirname(
										deps.resolved.get("eslint-config-next"),
									),
									"package.json",
								),
							).version
		...
		if (lintDuringBuild) {
			_log.error(
				`ESLint: ${(0, _iserror.default)(err) && err.message ? err.message.replace(/\n/g, " ") : err}`,
			);
			return null;
		} else {
...
async function runLintCheck(baseDir, lintDirs, opts) {
	...
	try {
		...
		const pkgJsonPath =
			(await (0, _findup.default)("package.json", {
				cwd: baseDir,
			})) ?? null;
		...
		const config = await (0, _hasEslintConfiguration.hasEslintConfiguration)(
			eslintrcFile,
			packageJsonConfig,
		);
		let deps;
		if (config.exists) {
			...
			if (lintDuringBuild) {
				if (config.emptyPkgJsonConfig || config.emptyEslintrc) {
					_log.warn(
						`No ESLint configuration detected. Run ${(0, _picocolors.bold)((0, _picocolors.cyan)("next lint"))} to begin setup`,
					);
				}
				return null;
			} else {
				...
				const { config: selectedConfig } = strict
					? await (0, _getESLintPromptValues.getESLintStrictValue)(baseDir)
				...
				if (selectedConfig == null) {
					...
					deps = await (0, _hasnecessarydependencies.hasNecessaryDependencies)(
						baseDir,
						requiredPackages,
					);
					if (deps.missing.length > 0) {
						...
						await (0, _installdependencies.installDependencies)(
							baseDir,
							deps.missing,
							true,
						);
					...
					if (
						["app", "src/app", "pages", "src/pages"].some((dir) =>
							(0, _fs.existsSync)(_path.default.join(baseDir, dir)),
						)
					) {
						await (0, _writeDefaultConfig.writeDefaultConfig)(
							baseDir,
							config,
							selectedConfig,
							eslintrcFile,
				...
				_log.ready(
					`ESLint has successfully been configured. Run ${(0, _picocolors.bold)((0, _picocolors.cyan)("next lint"))} again to view warnings and errors.`,
				);
				return null;
			}
		}
