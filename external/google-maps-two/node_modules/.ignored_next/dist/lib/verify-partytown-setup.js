...
async function missingDependencyError(dir) {
	...
	throw new _fatalerror.FatalError(
		(0, _picocolors.bold)(
			...
			"\n\n" +
			(0, _picocolors.bold)(
				`If you are not trying to use Partytown, please disable the experimental ${(0, _picocolors.cyan)('"nextScriptWorkers"')} flag in next.config.js.`,
			) +
			"\n",
	);
}
async function copyPartytownStaticFiles(deps, staticDir) {
	...
	const hasPartytownLibDir = await (0, _fileexists.fileExists)(
		partytownLibDir,
		_fileexists.FileType.Directory,
	);
...
async function verifyPartytownSetup(dir, targetDir) {
	try {
		...
		const partytownDeps = await (0,
		_hasnecessarydependencies.hasNecessaryDependencies)(dir, [
			{
		...
		if (
			((_partytownDeps_missing = partytownDeps.missing) == null
				? void 0
				: _partytownDeps_missing.length) > 0
		) {
			...
			try {
				...
				_log.warn(
					`Partytown library files could not be copied to the static directory. Please ensure that ${(0, _picocolors.bold)((0, _picocolors.cyan)("@builder.io/partytown"))} is installed as a dependency.`,
				);
			}
		}
	} catch (err) {
