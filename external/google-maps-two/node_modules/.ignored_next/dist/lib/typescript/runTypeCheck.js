...
async function runTypeCheck(
	...
	_getTypeScriptConfiguration.getTypeScriptConfiguration)(ts, tsConfigPath);
	...
	_writeConfigurationDefaults.getRequiredConfiguration)(ts);
	const options = {
		...requiredConfig,
	...
	const firstError =
		allDiagnostics.find(
			(d) =>
				d.category === _diagnosticFormatter.DiagnosticCategory.Error &&
				Boolean(d.file),
		) ??
		allDiagnostics.find(
			(d) => d.category === _diagnosticFormatter.DiagnosticCategory.Error,
		);
	...
	const warnings = allDiagnostics
		.filter(
			(d) => d.category === _diagnosticFormatter.DiagnosticCategory.Warning,
		)
		.map((d) =>
			(0, _diagnosticFormatter.getFormattedDiagnostic)(
				ts,
