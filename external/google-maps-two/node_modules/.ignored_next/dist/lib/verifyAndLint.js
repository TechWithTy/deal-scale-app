...
async function verifyAndLint(
	...
	try {
		...
		const lintDirs = (configLintDirs ?? _constants.ESLINT_DEFAULT_DIRS).reduce(
			(res, d) => {
		...
		const lintResults = await (lintWorkers == null
			? void 0
			: lintWorkers.runLintCheck(dir, lintDirs, {
		...
		if ((0, _iserror.default)(err)) {
			if (
				...
				err instanceof _compileerror.CompileError
			) {
