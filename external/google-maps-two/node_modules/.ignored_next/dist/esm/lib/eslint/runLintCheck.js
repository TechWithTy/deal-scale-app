...
async function cliPrompt(cwd) {
	console.log(
		bold(
			`${cyan("?")} How would you like to configure ESLint? https://nextjs.org/docs/basic-features/eslint`,
		),
	);
	try {
		const cliSelect = (
...
async function lint(
	...
	try {
		...
		if (deps.missing.some((dep) => dep.pkg === "eslint")) {
			Log.error(
				`ESLint must be installed${lintDuringBuild ? " in order to run during builds:" : ":"} ${bold(cyan((packageManager === "yarn" ? "yarn add --dev" : packageManager === "pnpm" ? "pnpm install --save-dev" : "npm install --save-dev") + " eslint"))}`,
			);
			return null;
		}
		...
		if (lintDuringBuild) {
			Log.error(
				`ESLint: ${isError(err) && err.message ? err.message.replace(/\n/g, " ") : err}`,
			);
			return null;
		} else {
		...
		if (config.exists) {
			...
			if (lintDuringBuild) {
				if (config.emptyPkgJsonConfig || config.emptyEslintrc) {
					Log.warn(
						`No ESLint configuration detected. Run ${bold(cyan("next lint"))} to begin setup`,
					);
				}
				return null;
			} else {
				...
				Log.ready(
					`ESLint has successfully been configured. Run ${bold(cyan("next lint"))} again to view warnings and errors.`,
				);
				return null;
			}
		}
