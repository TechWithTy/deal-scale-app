...
export async function writeDefaultConfig(
	...
	if (!exists && emptyEslintrc && eslintrcFile) {
		...
		Log.info(
			`We detected an empty ESLint configuration file (${bold(path.basename(eslintrcFile))}) and updated it for you!`,
		);
		...
		Log.info(
			`We detected an empty ${bold("eslintConfig")} field in package.json and updated it for you!`,
		);
	} else if (!exists) {
		await fs.writeFile(
		...
		console.log(
			green(
				`We created the ${bold(".eslintrc.json")} file for you and included your selected configuration.`,
			),
		);
	}
}
