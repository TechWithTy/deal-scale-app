...
async function missingDependencyError(dir) {
	...
	throw new FatalError(
		bold(
			...
			"\n\n" +
			bold(
				`If you are not trying to use Partytown, please disable the experimental ${cyan('"nextScriptWorkers"')} flag in next.config.js.`,
			) +
			"\n",
	);
}
	...
	try {
		...
		if (
			...
			try {
				...
				Log.warn(
					`Partytown library files could not be copied to the static directory. Please ensure that ${bold(cyan("@builder.io/partytown"))} is installed as a dependency.`,
				);
			}
		}
	} catch (err) {
