...
export default class FileSystemCache {
			...
			try {
				...
				if (kind === "fetch" && this.flushToDisk) {
					...
					const pageData = isAppPath
						? await this.fs.readFile(
								this.getFilePath(
									`${key}${this.experimental.ppr ? RSC_PREFETCH_SUFFIX : RSC_SUFFIX}`,
									"app",
								),
								"utf8",
							)
		...
		if (
			...
			const tagsHeader =
				(_data_value_headers = data.value.headers) == null
					? void 0
					: _data_value_headers[NEXT_CACHE_TAGS_HEADER];
	...
	async set(...args) {
		...
		if ((data == null ? void 0 : data.kind) === "PAGE") {
			...
			await this.fs.writeFile(
				this.getFilePath(
					`${key}${isAppPath ? (this.experimental.ppr ? RSC_PREFETCH_SUFFIX : RSC_SUFFIX) : NEXT_DATA_SUFFIX}`,
					isAppPath ? "app" : "pages",
				),
