...
export default class HotReloaderWebpack {
	...
	async start() {
							...
							onServer: () => {
								...
								if (isInstrumentation) {
									...
									value = getAppEntry({
										...
										pagePath: posix.join(
											APP_DIR_ALIAS,
											relative(this.appDir, entryData.absolutePagePath).replace(
												/\\/g,
												"/",
											),
										),
		...
		this.multiCompiler.compilers[1].hooks.done.tap(
			...
			(stats) => {
				...
				if (this.appDir) {
					...
					const diffChunkNames = difference(
		...
		this.multiCompiler.hooks.done.tap("NextjsHotReloaderForServer", () => {
			...
			this.reloadAfterInvalidation = false;
			const serverOnlyChanges = difference(
				changedServerPages,
				changedClientPages,
			);
			const edgeServerOnlyChanges = difference(
				changedEdgeServerPages,
				changedClientPages,
			);
	...
	invalidate(
		...
		if (outputPath) {
			var _getInvalidator;
			(_getInvalidator = getInvalidator(outputPath)) == null
				? void 0
