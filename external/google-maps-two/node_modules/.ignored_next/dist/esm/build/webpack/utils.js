	...
	compilation.chunkGroups.forEach((chunkGroup) => {
		...
		chunkGroup.chunks.forEach((chunk) => {
			...
			for (const mod of chunkModules) {
				...
				const modId =
					(_compilation_chunkGraph_getModuleId =
						compilation.chunkGraph.getModuleId(mod)) == null
						? void 0
						: _compilation_chunkGraph_getModuleId.toString();
...
export function forEachEntryModule(compilation, callback) {
	for (const [name, entry] of compilation.entries.entries()) {
		...
		if (
			name.startsWith("pages/") || // Skip for route.js entries
		...
		if (request.startsWith("next-edge-ssr-loader?")) {
			entryModule.dependencies.forEach((dependency) => {
				...
				if (
					modRequest == null ? void 0 : modRequest.includes("next-app-loader")
				) {
