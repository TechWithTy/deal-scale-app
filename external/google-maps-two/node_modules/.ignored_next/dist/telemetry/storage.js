...
const _conf = /*#__PURE__*/ _interop_require_default(
	require("next/dist/compiled/conf"),
);
const _crypto = require("crypto");
const _isdocker = /*#__PURE__*/ _interop_require_default(
...
class Telemetry {
	constructor({ distDir }) {
		this.notify = () => {
			...
			console.log(
				`${(0, _picocolors.magenta)((0, _picocolors.bold)("Attention"))}: Next.js now collects completely anonymous telemetry regarding usage.`,
			);
			console.log(
		...
		this.flushDetached = (mode, dir) => {
			...
			this.queue.forEach((item) => {
				try {
					...
					(_item__controller = item._controller) == null
						? void 0
						: _item__controller.abort();
			...
			_fs.default.writeFileSync(
				_path.default.join(this.distDir, "_events.json"),
			...
			spawn(
				process.execPath,
				[require.resolve("./detached-flush"), mode, dir],
				{
		...
		this.submitRecord = async (_events) => {
			...
			const res = (0, _postpayload._postPayload)(
	...
	async getProjectId() {
		this.loadProjectId =
			this.loadProjectId || (0, _projectid.getRawProjectId)();
