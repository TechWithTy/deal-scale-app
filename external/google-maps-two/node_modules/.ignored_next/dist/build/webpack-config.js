...
const _ReactRefreshWebpackPlugin = /*#__PURE__*/ _interop_require_default(
	require("next/dist/compiled/@next/react-refresh-utils/dist/ReactRefreshWebpackPlugin"),
);
...
const _middlewareplugin = /*#__PURE__*/ _interop_require_wildcard(
	require("./webpack/plugins/middleware-plugin"),
);
const _buildmanifestplugin = /*#__PURE__*/ _interop_require_default(
	require("./webpack/plugins/build-manifest-plugin"),
);
...
const _pagesmanifestplugin = /*#__PURE__*/ _interop_require_default(
	require("./webpack/plugins/pages-manifest-plugin"),
);
...
const _loadjsconfig = /*#__PURE__*/ _interop_require_default(
	require("./load-jsconfig"),
);
...
const NEXT_PROJECT_ROOT_DIST_CLIENT = _path.default.join(
...
const devtoolRevertWarning = (0, _utils1.execOnce)((devtool) => {
	console.warn(
		(0, _picocolors.yellow)((0, _picocolors.bold)("Warning: ")) +
			(0, _picocolors.bold)(`Reverting webpack devtool to '${devtool}'.\n`) +
...
function attachReactRefresh(webpackConfig, targetLoader) {
	...
	(_webpackConfig_module = webpackConfig.module) == null
		? void 0
		: (_webpackConfig_module_rules = _webpackConfig_module.rules) == null
			? void 0
			: _webpackConfig_module_rules.forEach((rule) => {
					if (rule && typeof rule === "object" && "use" in rule) {
						...
						if (curr === targetLoader) {
							...
							Array.isArray(curr) &&
							curr.some((r) => r === targetLoader) && // Check if loader already exists:
							!curr.some(
	...
	if (injections) {
		_log.info(
			`automatically enabled Fast Refresh for ${injections} custom loader${injections > 1 ? "s" : ""}`,
		);
	}
}
...
async function loadProjectInfo({ dir, config, dev }) {
	const { jsConfig, resolvedBaseUrl } = await (0, _loadjsconfig.default)(
		dir,
		config,
	);
...
async function getBaseWebpackConfig(
	...
	var _config_experimental_sri,
		...
		_config_compiler1,
		_config_compiler2,
		_jsConfig_compilerOptions,
		_config_compiler3,
		_jsConfig_compilerOptions1,
		_config_compiler4,
		_jsConfig_compilerOptions2, // always add JsConfigPathsPlugin to allow hot-reloading
		// if the config is added/removed
		_webpackConfig_resolve_plugins,
		_webpackConfig_resolve,
		...
		_config_compiler8,
		_config_compiler9,
		_webpackConfig_module,
		_webpackConfig_module_rules,
		_webpackConfig_module1;
	...
	const bundledReactChannel = (0,
	_needsexperimentalreact.needsExperimentalReact)(config)
		? "-experimental"
		: "";
	...
	if (useSWCLoader) {
		...
		const binaryTarget =
			(_require = require("./swc")) == null
				? void 0
				: (_require_getBinaryMetadata1 = _require.getBinaryMetadata) == null
					? void 0
					: (_require_getBinaryMetadata =
								_require_getBinaryMetadata1.call(_require)) == null
						? void 0
						: _require_getBinaryMetadata.target;
	...
	if (!loggedSwcDisabled && !useSWCLoader && babelConfigFile) {
		_log.info(
			`Disabled SWC as replacement for Babel because of custom Babel configuration "${_path.default.relative(dir, babelConfigFile)}" https://nextjs.org/docs/messages/swc-disabled`,
		);
	...
	const getSwcLoader = (extraOptions) => {
		...
		if (
			(config == null
				...
				: (_config_experimental = config.experimental) == null
					? void 0
					: _config_experimental.swcTraceProfiling) &&
			...
			(_require = require("./swc")) == null
				? void 0
				: (_require_initCustomTraceSubscriber =
							_require.initCustomTraceSubscriber) == null
					? void 0
					: _require_initCustomTraceSubscriber.call(
							_require,
							_path.default.join(
								distDir,
		...
		return {
			...
			options: {
				...
				supportedBrowsers,
				swcCacheDir: _path.default.join(
					dir,
					(config == null ? void 0 : config.distDir) ?? ".next",
					"cache",
					"swc",
				),
				...extraOptions,
		...
		? {
				...
				...(dev
					? {
							...
							[_constants1.CLIENT_STATIC_FILES_RUNTIME_AMP]:
								`./` +
								_path.default
									.relative(
										dir,
					...
					: {}),
				[_constants1.CLIENT_STATIC_FILES_RUNTIME_MAIN]:
					`./` +
					_path.default
						.relative(
							dir,
	...
	const resolveConfig = {
		...
		plugins: [
			isNodeServer
				? new _optionalpeerdependencyresolveplugin.OptionalPeerDependencyResolverPlugin()
				: undefined,
		].filter(Boolean),
	};
	const terserOptions = {
	...
	if (
		...
		const externalPackageConflicts = finalTranspilePackages.filter((pkg) => {
			...
			return (_config_experimental_serverComponentsExternalPackages =
				config.experimental.serverComponentsExternalPackages) == null
				? void 0
				: _config_experimental_serverComponentsExternalPackages.includes(pkg);
		});
	...
	const optOutBundlingPackages = EXTERNAL_PACKAGES.concat(
		...
		(pkg) =>
			!(finalTranspilePackages == null
				? void 0
				: finalTranspilePackages.includes(pkg)),
	);
	// For resolved request, such as `absolute path/package name/foo/bar.js`
	const optOutBundlingPackageRegex = new RegExp(
		`[/\\\\]node_modules[/\\\\](${optOutBundlingPackages.map((p) => p.replace(/\//g, "[/\\\\]")).join("|")})[/\\\\]`,
	);
	const transpilePackagesRegex = new RegExp(
		`[/\\\\]node_modules[/\\\\](${finalTranspilePackages == null ? void 0 : finalTranspilePackages.map((p) => p.replace(/\//g, "[/\\\\]")).join("|")})[/\\\\]`,
	);
	...
	const codeCondition = {
		test: {
			or: [/\.(tsx|ts|js|cjs|mjs|jsx)$/, /__barrel_optimize__/],
		},
		...
		exclude: (excludePath) => {
			...
			const shouldBeBundled = (0, _handleexternals.isResourceInPackages)(
				excludePath,
				finalTranspilePackages,
			);
	...
	let webpackConfig = {
											...
											resolveFunction(
												...
												(err, result, resolveData) => {
													...
													const isEsm = /\.js$/i.test(result)
														? (resolveData == null
																? void 0
																: (_resolveData_descriptionFileData =
																			resolveData.descriptionFileData) == null
																	? void 0
																	: _resolveData_descriptionFileData.type) ===
															"module"
														: /\.mjs$/i.test(result);
		...
		optimization: {
            ...
            */ const extractRootNodeModule = (modulePath) => {
				...
				const frameworkCacheGroup = {
					...
					test(module1) {
						const resource =
							module1.nameForCondition == null
								? void 0
								: module1.nameForCondition.call(module1);
						return resource
				...
				const libCacheGroup = {
					test(module1) {
						var _module_type;
						return (
							!((_module_type = module1.type) == null
								? void 0
								: _module_type.startsWith("css")) &&
							module1.size() > 160000 &&
							/node_modules[/\\]/.test(module1.nameForCondition() || "")
						);
					},
					name(module1) {
						...
						if (isModuleCSS(module1)) {
							...
							if (!module1.libIdent) {
								throw new Error(
									`Encountered unknown module type: ${module1.type}. Please open an issue.`,
								);
							}
							hash.update(
								module1.libIdent({
		...
		output: {
			...
			path:
				!dev && isNodeServer
					? _path.default.join(outputPath, "chunks")
					: outputPath,
		...
		resolveLoader: {
			...
			alias: [
				...
				alias[loader] = _path.default.join(
					__dirname,
					"webpack",
					"loaders",
					loader,
			...
			rules: [
				// Alias server-only and client-only to proper exports based on bundling layers
				{
					...
					resolve: {
						...
						alias: (0,
						_createcompileraliases.createServerOnlyClientOnlyAliases)(true),
					},
				},
				{
					issuerLayer: {
					...
					resolve: {
						...
						alias: (0,
						_createcompileraliases.createServerOnlyClientOnlyAliases)(false),
					},
				},
				...
				...(hasAppDir
					? [
							...
							},
							{
								...
								resolve: {
									alias: (0, _createcompileraliases.createAppRouterApiAliases)(
										true,
									),
								},
							},
					...
					? [
							{
								...
								resolve: {
									...
									alias: (0, _createcompileraliases.createRSCAliases)(
										bundledReactChannel,
										{
				...
				...(hasAppDir && isEdgeServer
					? [
							...
							{
								resourceQuery: new RegExp(
									_constants.WEBPACK_RESOURCE_QUERIES.edgeSSREntry,
								),
								layer: _constants.WEBPACK_LAYERS.reactServerComponents,
							},
						]
						...
						...(hasAppDir
							? [
									...
									},
									{
										...
										resolve: {
											mainFields: (0, _resolve.getMainField)(
												compilerType,
												true,
											),
										},
					...
					? [
							{
								...
								resourceQuery: {
									not: [
										...
										new RegExp(
											_constants.WEBPACK_RESOURCE_QUERIES.metadataImageMeta,
										),
									],
								},
								options: {
				...
				},
				{
					...
					use: ({ resourceQuery }) => {
						...
						const names = (
							((_resourceQuery_match =
								resourceQuery.match(/\?names=([^&]+)/)) == null
								? void 0
								: _resourceQuery_match[1]) || ""
						).split(",");
						return [
							{
		...
		plugins: [
			isNodeServer &&
				new _webpack.webpack.NormalModuleReplacementPlugin(
					/\.\/(.+)\.shared-runtime$/,
					function (resource) {
						const moduleName = _path.default.basename(
							resource.request,
							".shared-runtime",
						);
			...
			dev &&
				new _memorywithgccacheplugin.MemoryWithGcCachePlugin({
					maxGenerations: 5,
				}),
			dev &&
				isClient &&
			...
			(isClient || isEdgeServer) &&
				new _webpack.webpack.ProvidePlugin({
			...
			isClient &&
				new _reactloadableplugin.ReactLoadablePlugin({
			...
			(isClient || isEdgeServer) &&
				new _nextdropclientpageplugin.DropClientPage(),
			...
			config.excludeDefaultMomentLocales &&
				new _webpack.webpack.IgnorePlugin({
			...
			...(dev
				? (() => {
						...
						if (isClient || isEdgeServer) {
							devPlugins.push(
								new _webpack.webpack.HotModuleReplacementPlugin(),
							);
						}
						return devPlugins;
					})()
			...
			isNodeOrEdgeCompilation &&
				new _pagesmanifestplugin.default({
					dev,
					appDirEnabled: hasAppDir,
			...
			isEdgeServer &&
				new _middlewareplugin.default({
					dev,
					sriEnabled:
						!dev &&
						!!((_config_experimental_sri = config.experimental.sri) == null
							? void 0
							: _config_experimental_sri.algorithm),
					rewrites,
					edgeEnvironments: {
			...
			isClient &&
				new _buildmanifestplugin.default({
					buildId,
					rewrites,
					isDevFallback,
					exportRuntime: true,
			...
			isClient &&
				new _copyfileplugin.CopyFilePlugin({
			...
			hasAppDir &&
				isClient &&
				new _appbuildmanifestplugin.AppBuildManifestPlugin({
					dev,
				}),
			hasAppDir &&
				(isClient
					? new _flightmanifestplugin.ClientReferenceManifestPlugin({
							dev,
							appDir,
						})
			...
			hasAppDir &&
				!isClient &&
				new _nexttypesplugin.NextTypesPlugin({
					dir,
			...
			!dev &&
				isClient &&
				!!((_config_experimental_sri1 = config.experimental.sri) == null
					? void 0
					: _config_experimental_sri1.algorithm) &&
				new _subresourceintegrityplugin.SubresourceIntegrityPlugin(
					config.experimental.sri.algorithm,
				),
			isClient &&
				new _nextfontmanifestplugin.NextFontManifestPlugin({
					appDir,
				}),
			!dev &&
				isClient &&
				new _csschunkingplugin.CssChunkingPlugin(
				...
				new (require("./webpack/plugins/telemetry-plugin").TelemetryPlugin)(
					new Map(
						[
							...
							[
								"swcRelay",
								!!((_config_compiler = config.compiler) == null
									? void 0
									: _config_compiler.relay),
							],
							[
								"swcStyledComponents",
								!!((_config_compiler1 = config.compiler) == null
									? void 0
									: _config_compiler1.styledComponents),
							],
							[
								"swcReactRemoveProperties",
								!!((_config_compiler2 = config.compiler) == null
									? void 0
									: _config_compiler2.reactRemoveProperties),
							],
							[
								"swcExperimentalDecorators",
								!!(jsConfig == null
									? void 0
									: (_jsConfig_compilerOptions = jsConfig.compilerOptions) ==
											null
										? void 0
										: _jsConfig_compilerOptions.experimentalDecorators),
							],
							[
								"swcRemoveConsole",
								!!((_config_compiler3 = config.compiler) == null
									? void 0
									: _config_compiler3.removeConsole),
							],
							[
								"swcImportSource",
								!!(jsConfig == null
									? void 0
									: (_jsConfig_compilerOptions1 = jsConfig.compilerOptions) ==
											null
										? void 0
										: _jsConfig_compilerOptions1.jsxImportSource),
							],
							[
								"swcEmotion",
								!!((_config_compiler4 = config.compiler) == null
									? void 0
									: _config_compiler4.emotion),
							],
							["turbotrace", !!config.experimental.turbotrace],
	...
	if (resolvedBaseUrl && !resolvedBaseUrl.isImplicit) {
		...
		(_webpackConfig_resolve1 = webpackConfig.resolve) == null
			? void 0
			: (_webpackConfig_resolve_modules = _webpackConfig_resolve1.modules) ==
					null
				? void 0
				: _webpackConfig_resolve_modules.push(resolvedBaseUrl.baseUrl);
	}
	(_webpackConfig_resolve = webpackConfig.resolve) == null
		? void 0
		: (_webpackConfig_resolve_plugins = _webpackConfig_resolve.plugins) == null
			? void 0
			: _webpackConfig_resolve_plugins.unshift(
					new _jsconfigpathsplugin.JsConfigPathsPlugin(
						(jsConfig == null
							? void 0
							: (_jsConfig_compilerOptions2 = jsConfig.compilerOptions) == null
								? void 0
								: _jsConfig_compilerOptions2.paths) || {},
						resolvedBaseUrl,
					),
				);
	const webpack5Config = webpackConfig;
	if (isEdgeServer) {
		var _webpack5Config_module_rules,
			_webpack5Config_module,
			_webpack5Config_module_rules1,
			_webpack5Config_module1,
			_webpack5Config_module_rules2,
			_webpack5Config_module2;
		(_webpack5Config_module = webpack5Config.module) == null
			? void 0
			: (_webpack5Config_module_rules = _webpack5Config_module.rules) == null
				? void 0
				: _webpack5Config_module_rules.unshift({
						test: /\.wasm$/,
						loader: "next-middleware-wasm-loader",
						type: "javascript/auto",
						resourceQuery: /module/i,
					});
		(_webpack5Config_module1 = webpack5Config.module) == null
			? void 0
			: (_webpack5Config_module_rules1 = _webpack5Config_module1.rules) == null
				? void 0
				: _webpack5Config_module_rules1.unshift({
						dependency: "url",
						loader: "next-middleware-asset-loader",
						type: "javascript/auto",
						layer: _constants.WEBPACK_LAYERS.edgeAsset,
					});
		(_webpack5Config_module2 = webpack5Config.module) == null
			? void 0
			: (_webpack5Config_module_rules2 = _webpack5Config_module2.rules) == null
				? void 0
				: _webpack5Config_module_rules2.unshift({
	...
	const configVars = JSON.stringify({
		optimizePackageImports:
			config == null
				...
				: (_config_experimental = config.experimental) == null
					? void 0
					: _config_experimental.optimizePackageImports,
		...
		swcLoader: useSWCLoader,
		removeConsole:
			(_config_compiler5 = config.compiler) == null
				? void 0
				: _config_compiler5.removeConsole,
		reactRemoveProperties:
			(_config_compiler6 = config.compiler) == null
				? void 0
				: _config_compiler6.reactRemoveProperties,
		styledComponents:
			(_config_compiler7 = config.compiler) == null
				? void 0
				: _config_compiler7.styledComponents,
		relay:
			(_config_compiler8 = config.compiler) == null
				? void 0
				: _config_compiler8.relay,
		emotion:
			(_config_compiler9 = config.compiler) == null
				? void 0
				: _config_compiler9.emotion,
		modularizeImports: config.modularizeImports,
	...
	const cache = {
		...
		version: `${__dirname}|${"14.2.32"}|${configVars}`,
		...
		customAppFile: pagesDir
			? new RegExp(
					(0, _escaperegexp.escapeStringRegexp)(
						_path.default.join(pagesDir, `_app`),
					),
				)
			: undefined,
		hasAppDir,
	...
	if (dev) {
		if (webpackConfig.module) {
			webpackConfig.module.unsafeCache = (module1) =>
				!UNSAFE_CACHE_REGEX.test(module1.resource);
		} else {
	...
	if (typeof config.webpack === "function") {
		...
		if (
			((_webpack5Config_experiments = webpack5Config.experiments) == null
				? void 0
				: _webpack5Config_experiments.lazyCompilation) === true
		) {
			webpack5Config.experiments.lazyCompilation = {
				entries: false,
			};
		} else if (
			typeof ((_webpack5Config_experiments1 = webpack5Config.experiments) ==
			null
				? void 0
				: _webpack5Config_experiments1.lazyCompilation) === "object" &&
	...
	if (!config.images.disableStaticImages) {
		...
		const rules =
			((_webpackConfig_module2 = webpackConfig.module) == null
				? void 0
				: _webpackConfig_module2.rules) || [];
	...
	const hasUserCssConfig =
		((_webpackConfig_module1 = webpackConfig.module) == null
			? void 0
			: (_webpackConfig_module_rules = _webpackConfig_module1.rules) == null
				? void 0
				: _webpackConfig_module_rules.some(
						(rule) => canMatchCss(rule.test) || canMatchCss(rule.include),
					)) ?? false;
	if (hasUserCssConfig) {
		var _webpackConfig_module_rules1,
			_webpackConfig_module3,
			_webpackConfig_plugins,
			_webpackConfig_optimization_minimizer,
			_webpackConfig_optimization;
		...
		if (
			(_webpackConfig_module3 = webpackConfig.module) == null
				? void 0
				: (_webpackConfig_module_rules1 = _webpackConfig_module3.rules) == null
					? void 0
					: _webpackConfig_module_rules1.length
		) {
			...
			webpackConfig.module.rules.forEach((r) => {
				...
				if (Array.isArray(r.oneOf)) {
					r.oneOf = r.oneOf.filter(
						(o) => o[Symbol.for("__next_css_remove")] !== true,
					);
				}
			});
		}
		if (
			(_webpackConfig_plugins = webpackConfig.plugins) == null
				? void 0
				: _webpackConfig_plugins.length
		) {
			// Disable CSS Extraction Plugin
			webpackConfig.plugins = webpackConfig.plugins.filter(
				(p) => p.__next_css_remove !== true,
			);
		}
		if (
			(_webpackConfig_optimization = webpackConfig.optimization) == null
				? void 0
				: (_webpackConfig_optimization_minimizer =
							_webpackConfig_optimization.minimizer) == null
					? void 0
					: _webpackConfig_optimization_minimizer.length
		) {
			// Disable CSS Minifier
			webpackConfig.optimization.minimizer =
				webpackConfig.optimization.minimizer.filter(
					(e) => e.__next_css_remove !== true,
				);
		}
	}
	...
	if (typeof originalEntry !== "undefined") {
		const updatedEntry = async () => {
			...
			if (
				...
				const originalFile =
					clientEntries[_constants1.CLIENT_STATIC_FILES_RUNTIME_MAIN];
