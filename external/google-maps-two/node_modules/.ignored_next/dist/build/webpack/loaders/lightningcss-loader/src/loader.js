...
async function LightningCssLoader(source, prevMap) {
	...
	const replacements = [];
	if (
		((_options_modules = options.modules) == null
			? void 0
			: _options_modules.exportOnlyLocals) !== true
	) {
		imports.unshift({
			type: "api_import",
			importName: "___CSS_LOADER_API_IMPORT___",
			url: (0, _stringifyrequest.stringifyRequest)(
				this,
	...
	const transform =
		(implementation == null ? void 0 : implementation.transformCss) ??
	...
	const urlImportVisitor = createUrlAndImportVisitor(
		{
			urlHandler: (url) =>
				(0, _stringifyrequest.stringifyRequest)(
					this,
					(0, _utils1.getPreRequester)(this)(options.importLoaders ?? 0) + url,
				),
	...
	const icssVisitor = createIcssVisitor({
		...
		urlHandler: (url) =>
			(0, _stringifyrequest.stringifyRequest)(
				this,
				(0, _utils1.getPreRequester)(this)(options.importLoaders) + url,
			),
	});
	...
	try {
		...
		if (replacedUrls.size !== 0) {
			...
			for (const [index, url] of replacedUrls.entries()) {
				...
				const resolvedUrl = await (0, _utils1.resolveRequests)(
					urlResolver,
					this.context,
					[...new Set([request, url])],
				);
		...
		const exportCode = (0, _codegen.getExportCode)(
			exports1,
			replacements,
			options,
		);
