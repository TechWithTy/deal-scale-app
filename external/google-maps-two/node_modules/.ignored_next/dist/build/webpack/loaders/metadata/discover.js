...
async function enumMetadataFiles(
	...
	extensions,
	{
		metadataResolver, // When set to true, possible filename without extension could: icon, icon0, ..., icon9
		numericSuffix,
	},
) {
...
async function createStaticMetadataFromRoute(
	resolvedDir,
	{
		segment,
		metadataResolver,
	...
	async function collectIconModuleIfExists(type) {
		...
		const resolvedMetadataFiles = await enumMetadataFiles(
			resolvedDir,
			_ismetadataroute.STATIC_METADATA_IMAGES[type].filename,
			[
		...
		resolvedMetadataFiles
			...
			.forEach((filepath) => {
				const imageModuleImportSource = `next-metadata-image-loader?${(0,
