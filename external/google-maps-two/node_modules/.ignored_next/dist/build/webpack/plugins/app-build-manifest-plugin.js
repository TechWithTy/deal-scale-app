...
const _getapproutefromentrypoint = /*#__PURE__*/ _interop_require_default(
	require("../../../server/get-app-route-from-entrypoint"),
);
...
class AppBuildManifestPlugin {
	...
	createAsset(assets, compilation) {
		...
		const mainFiles = new Set(
			(0, _buildmanifestplugin.getEntrypointFiles)(
				compilation.entrypoints.get(
		...
		for (const entrypoint of compilation.entrypoints.values()) {
			...
			const filesForPage = (0, _buildmanifestplugin.getEntrypointFiles)(
				entrypoint,
			);
			manifest.pages[pagePath] = [...new Set([...mainFiles, ...filesForPage])];
		}
		const json = JSON.stringify(manifest, null, 2);
		assets[_constants.APP_BUILD_MANIFEST] = new _webpack.sources.RawSource(
			json,
		);
	}
}
