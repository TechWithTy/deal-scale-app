...
class FlightClientEntryPlugin {
		...
		compiler.hooks.afterCompile.tap(PLUGIN_NAME, (compilation) => {
			const recordModule = (modId, mod) => {
				...
				const modResource = modPath
					? modPath.startsWith(_constants1.BARREL_OPTIMIZATION_PREFIX)
						? (0, _utils1.formatBarrelOptimizedResource)(mod.resource, modPath)
						: modPath + modQuery
					: mod.resource;
				...
				if (typeof modId !== "undefined" && modResource) {
					...
					// module ID strategy.
					let ssrNamedModuleId = _path.default.relative(
						compiler.context,
						modResource,
					);
			...
			(0, _utils1.traverseModules)(
				compilation,
				(mod, _chunk, _chunkGroup, modId) => {
					if (
						mod &&
						mod.resource &&
						!(0, _utils2.isWebpackServerOnlyLayer)(mod.layer)
					) {
	...
	async createClientEntries(compiler, compilation) {
		...
		(0, _utils1.forEachEntryModule)(compilation, ({ name, entryModule }) => {
			...
			for (const connection of (0, _utils1.getModuleReferencesInOrder)(
				entryModule,
				...
				const relativeRequest = isAbsoluteRequest
					? _path.default.relative(compilation.options.context, entryRequest)
					: entryRequest;
				// Replace file suffix as `.js` will be added.
				const bundlePath = (0, _normalizepathsep.normalizePathSep)(
				...
				if (
					name === `app${_constants1.UNDERSCORE_NOT_FOUND_ROUTE_ENTRY}` &&
	...
	collectClientActionsFromDependencies({ compilation, dependencies }) {
		...
		const collectActions = ({ entryRequest, resolvedModule }) => {
			const collectActionsInDep = (mod) => {
				...
				(0, _utils1.getModuleReferencesInOrder)(
					mod,
					compilation.moduleGraph,
				).forEach((connection) => {
	...
	collectComponentInfoFromServerEntryDependency({
		...
		const filterClientComponents = (mod, importedIdentifiers) => {
			...
			const webpackRuntime = this.isEdgeServer
				? _constants1.EDGE_RUNTIME_WEBPACK
				: _constants1.DEFAULT_RUNTIME_WEBPACK;
			if (isCSS) {
				const sideEffectFree =
					mod.factoryMeta && mod.factoryMeta.sideEffectFree;
				if (sideEffectFree) {
	...
	injectClientEntryAndSSRModules({
		...
		const loaderOptions = {
			modules: Object.keys(clientImports)
				.sort((a, b) => (_utils.regexCSS.test(b) ? 1 : a.localeCompare(b)))
		...
		// replace them.
		const clientBrowserLoader = `next-flight-client-entry-loader?${(0,
		_querystring.stringify)({
			modules: (this.isEdgeServer
				? loaderOptions.modules.map(({ request, ids }) => ({
						request: request.replace(
							/[\\/]next[\\/]dist[\\/]esm[\\/]/,
							"/next/dist/".replace(/\//g, _path.default.sep),
						),
						ids,
					}))
		...
		const clientSSRLoader = `next-flight-client-entry-loader?${(0,
		_querystring.stringify)({
		...
		if (this.dev) {
			...
			const pageKey = (0, _ondemandentryhandler.getEntryKey)(
		...
		const clientComponentEntryDep =
			_webpack.webpack.EntryPlugin.createDependency(clientSSRLoader, {
				name: bundlePath,
			});
		return [
			shouldInvalidate,
	...
	injectActionEntry({
		...
		const actionLoader = `next-flight-action-entry-loader?${(0,
		_querystring.stringify)({
		...
		const actionEntryDep = _webpack.webpack.EntryPlugin.createDependency(
			actionLoader,
			{
				name: bundlePath,
			},
	...
	async createActionAssets(compilation, assets) {
			...
			for (let name in action.workers) {
				const modId =
					pluginState.actionModServerId[name][
						action.layer[name] === _constants.WEBPACK_LAYERS.actionBrowser
							? "client"
							: "server"
					];
		...
		assets[`${this.assetPrefix}${_constants1.SERVER_REFERENCE_MANIFEST}.js`] =
			new _webpack.sources.RawSource(
				`self.__RSC_SERVER_MANIFEST=${JSON.stringify(edgeJson)}`,
			);
		assets[`${this.assetPrefix}${_constants1.SERVER_REFERENCE_MANIFEST}.json`] =
			new _webpack.sources.RawSource(json);
	}
}
function addClientImport(
	...
	const clientEntryType =
		(_getModuleBuildInfo_rsc = (0, _getmodulebuildinfo.getModuleBuildInfo)(
			mod,
		).rsc) == null
			? void 0
			: _getModuleBuildInfo_rsc.clientEntryType;
	const isCjsModule = clientEntryType === "cjs";
	const assumedSourceType = (0, _nextflightloader.getAssumedSourceType)(
		mod,
...
function getModuleResource(mod) {
	...
	if (
		(_mod_matchResource = mod.matchResource) == null
			? void 0
			: _mod_matchResource.startsWith(_constants1.BARREL_OPTIMIZATION_PREFIX)
	) {
