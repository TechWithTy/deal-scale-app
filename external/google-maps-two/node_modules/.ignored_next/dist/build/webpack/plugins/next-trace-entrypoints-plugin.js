...
function getFilesMapFromReasons(fileList, reasons, ignoreFn) {
	...
	for (const file of fileList) {
		...
		const isInitial =
			(reason == null ? void 0 : reason.type.length) === 1 &&
		...
		await span.traceChild("create-trace-assets").traceAsyncFn(async () => {
			...
			this.buildTraceContext.chunksTrace = {
				action: {
					...
					input: [...chunksToTrace],
					contextDirectory:
						((_this_turbotrace = this.turbotrace) == null
							? void 0
							: _this_turbotrace.contextDirectory) ?? this.tracingRoot,
					processCwd:
						((_this_turbotrace1 = this.turbotrace) == null
							? void 0
							: _this_turbotrace1.processCwd) ?? this.rootDir,
					showAll:
						(_this_turbotrace2 = this.turbotrace) == null
							? void 0
							: _this_turbotrace2.logAll,
					logLevel:
						(_this_turbotrace3 = this.turbotrace) == null
							? void 0
							: _this_turbotrace3.logLevel,
				},
				outputPath,
			...
			for (const [entrypoint, entryFiles] of entryFilesMap) {
				...
				const traceOutputPath = _path.default.dirname(
					_path.default.join(outputPath, traceOutputName),
				);
				// don't include the entry itself in the trace
				entryFiles.delete(
					_path.default.join(outputPath, `../${entrypoint.name}.js`),
				);
				if (entrypoint.name.startsWith("app/")) {
					// include the client reference manifest
					const clientManifestsForPage =
						entrypoint.name.endsWith("/page") ||
						entrypoint.name === _constants.UNDERSCORE_NOT_FOUND_ROUTE_ENTRY
							? _path.default.join(
									outputPath,
									"..",
									entrypoint.name.replace(/%5F/g, "_") +
										"_" +
										_constants.CLIENT_REFERENCE_MANIFEST +
										".js",
								)
							: null;
				...
				for (const file of new Set([
					...
					if (file) {
						finalFiles.push(
							_path.default.relative(traceOutputPath, file).replace(/\\/g, "/"),
						);
					}
				}
				assets[traceOutputName] = new _webpack.sources.RawSource(
					JSON.stringify({
					...
					.traceAsyncFn(async () => {
						...
						finishModulesSpan.traceChild("get-entries").traceFn(() => {
							compilation.entries.forEach((entry, name) => {
								const normalizedName =
									name == null ? void 0 : name.replace(/\\/g, "/");
								...
								if (isApp || isPage) {
									for (const dep of entry.dependencies) {
										...
										if (entryMod && entryMod.resource === "") {
											const moduleBuildInfo = (0,
											_getmodulebuildinfo.getModuleBuildInfo)(entryMod);
						...
						const readFile = async (path) => {
							...
							const source =
								mod == null
									? void 0
									: (_mod_originalSource = mod.originalSource) == null
										? void 0
										: _mod_originalSource.call(mod);
							if (source) {
						...
						const collectDependencies = (mod) => {
							...
							for (const dep of mod.dependencies) {
								...
								if (
									(depMod == null ? void 0 : depMod.resource) &&
									!depModMap.get(depMod.resource)
								) {
						...
						this.buildTraceContext.entriesTrace = {
							action: {
								...
								logLevel:
									(_this_turbotrace2 = this.turbotrace) == null
										? void 0
										: _this_turbotrace2.logLevel,
								showAll:
									(_this_turbotrace3 = this.turbotrace) == null
										? void 0
										: _this_turbotrace3.logAll,
							},
							appDir: this.rootDir,
						...
						if (this.turbotrace) {
							...
							if (
								!(binding == null ? void 0 : binding.isWasm) &&
							...
							.traceAsyncFn(() => {
								const parentFilesMap = getFilesMapFromReasons(
									...
									(file) => {
										...
										const isAsset =
											(_reasons_get = reasons.get(
												_path.default.relative(this.tracingRoot, file),
											)) == null
												? void 0
												: _reasons_get.type.includes("asset");
										return (
											!isAsset &&
											Array.isArray(depMod == null ? void 0 : depMod.loaders) &&
								...
								entryPaths.forEach((entry) => {
									...
									const entryName = entryNameMap.get(entry);
									const normalizedEntry = _path.default.relative(
										this.tracingRoot,
										entry,
									);
									const curExtraEntries = additionalEntries.get(entryName);
									const finalDeps = new Set();
									(_parentFilesMap_get = parentFilesMap.get(normalizedEntry)) ==
									null
										? void 0
										: _parentFilesMap_get.forEach((dep) => {
												finalDeps.add(
													_path.default.join(this.tracingRoot, dep),
												);
											});
									if (curExtraEntries) {
										for (const extraEntry of curExtraEntries.keys()) {
											var _parentFilesMap_get1;
											const normalizedExtraEntry = _path.default.relative(
												this.tracingRoot,
												extraEntry,
											);
											finalDeps.add(extraEntry);
											(_parentFilesMap_get1 =
												parentFilesMap.get(normalizedExtraEntry)) == null
												? void 0
												: _parentFilesMap_get1.forEach((dep) => {
														finalDeps.add(
															_path.default.join(this.tracingRoot, dep),
														);
													});
	...
	apply(compiler) {
		compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
			...
			const compilationSpan =
				_profilingplugin.spans.get(compilation) ||
				_profilingplugin.spans.get(compiler);
			...
			traceEntrypointsPluginSpan.traceFn(() => {
							...
							curResolver.resolve(
								...
								async (err, result, resContext) => {
									...
									if (result.includes("?") || result.includes("!")) {
										result =
											(resContext == null ? void 0 : resContext.path) || result;
									}
									try {
										...
										if (result.includes("node_modules")) {
											let requestPath = result
												...
												.replace(/\0/g, "");
											if (
												!_path.default.isAbsolute(request) &&
												request.includes("/") &&
												(resContext == null
													? void 0
												...
												requestPath = (
													resContext.descriptionFileRoot +
													request.slice(
														((_getPkgName = getPkgName(request)) == null
															? void 0
															: _getPkgName.length) || 0,
													) +
													_path.default.sep +
													"package.json"
												)
				...
				const doResolve = async (request, parent, job, isEsmRequested) => {
					...
					const { res } = await (0, _handleexternals.resolveExternal)(
						this.rootDir,
						this.esmExternals,
						context,
						request,
