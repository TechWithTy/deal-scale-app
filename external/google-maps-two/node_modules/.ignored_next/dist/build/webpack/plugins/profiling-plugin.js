...
class ProfilingPlugin {
	...
	traceHookPair(
		...
		startHook,
		stopHook,
		{ parentSpan, attrs, onStart, onStop } = {},
	) {
		let span;
		startHook.tap(
			{
			...
			(compilation) => {
				compilation.hooks.buildModule.tap(pluginName, (module1) => {
					...
					const issuerModule =
						compilation == null
							? void 0
							: (_compilation_moduleGraph = compilation.moduleGraph) == null
								? void 0
								: _compilation_moduleGraph.getIssuer(module1);
					let span;
					const moduleSpans = moduleSpansByCompilation.get(compilation);
					const spanName = `build-module${moduleType ? `-${moduleType}` : ""}`;
					const issuerSpan =
						issuerModule &&
						(moduleSpans == null ? void 0 : moduleSpans.get(issuerModule));
					if (issuerSpan) {
				...
				const moduleHooks =
					_webpack.NormalModule.getCompilationHooks(compilation);
				...
				moduleHooks.loader.tap(pluginName, (loaderContext, module1) => {
					...
					const moduleSpan =
						(_moduleSpansByCompilation_get =
							moduleSpansByCompilation.get(compilation)) == null
							? void 0
							: _moduleSpansByCompilation_get.get(module1);
				...
				compilation.hooks.succeedModule.tap(pluginName, (module1) => {
					...
					moduleSpansByCompilation == null
						? void 0
						: (_moduleSpansByCompilation_get =
									moduleSpansByCompilation.get(compilation)) == null
							? void 0
							: (_moduleSpansByCompilation_get_get =
										_moduleSpansByCompilation_get.get(module1)) == null
								? void 0
								: _moduleSpansByCompilation_get_get.stop();
				});
