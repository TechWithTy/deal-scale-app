...
function getRSCModuleInformation(source, isReactServerLayer) {
	...
	const clientRefs =
		clientInfoMatch == null
			? void 0
			: (_clientInfoMatch_ = clientInfoMatch[1]) == null
				? void 0
				: _clientInfoMatch_.split(",");
	const clientEntryType = clientInfoMatch == null ? void 0 : clientInfoMatch[2];
	const type = clientRefs
		? _constants1.RSC_MODULE_TYPES.client
		: _constants1.RSC_MODULE_TYPES.server;
	return {
		type,
		actions,
		clientRefs,
	...
	if (Array.isArray(swcAST == null ? void 0 : swcAST.body)) {
		try {
			...
			for (const node of swcAST.body) {
				var _node_declaration,
					_node_declaration1,
					_node_declaration_identifier,
					_node_declaration2;
				...
				if (
					...
					((_node_declaration = node.declaration) == null
						? void 0
						: _node_declaration.type) === "VariableDeclaration"
				) {
					var _node_declaration3;
					for (const declaration of (_node_declaration3 = node.declaration) ==
					null
						? void 0
						: _node_declaration3.declarations) {
				...
				if (
					node.type === "ExportDeclaration" &&
					((_node_declaration1 = node.declaration) == null
						? void 0
						: _node_declaration1.type) === "FunctionDeclaration" &&
					exportsSet.has(
						(_node_declaration_identifier = node.declaration.identifier) == null
							? void 0
							: _node_declaration_identifier.value,
					)
				) {
				...
				if (
					node.type === "ExportDeclaration" &&
					((_node_declaration2 = node.declaration) == null
						? void 0
						: _node_declaration2.type) === "VariableDeclaration"
				) {
					var _node_declaration_declarations_, _node_declaration4;
					const id =
						(_node_declaration4 = node.declaration) == null
							? void 0
							: (_node_declaration_declarations_ =
										_node_declaration4.declarations[0]) == null
								? void 0
								: _node_declaration_declarations_.id.value;
				...
				if (node.type === "ExportNamedDeclaration") {
					const values = node.specifiers.map((specifier) => {
						...
						return (
							...
							((_specifier_orig = specifier.orig) == null
								? void 0
								: _specifier_orig.type) === "Identifier" &&
							((_specifier_orig1 = specifier.orig) == null
								? void 0
								: _specifier_orig1.value)
						);
					});
	...
	if (config.unstable_allowDynamic) {
		...
		for (const glob of result.unstable_allowDynamicGlobs ?? []) {
			try {
				...
				throw new Error(
					`${pageFilePath} exported 'config.unstable_allowDynamic' contains invalid pattern '${glob}': ${err.message}`,
				);
			}
		}
	}
...
async function isDynamicMetadataRoute(pageFilePath) {
	...
	if (/generateImageMetadata|generateSitemaps/.test(fileContent)) {
		const swcAST = await (0, _parsemodule.parseModule)(
			pageFilePath,
			fileContent,
		);
		const exportsInfo = checkExports(swcAST, pageFilePath);
		return !!(
			exportsInfo.generateImageMetadata || exportsInfo.generateSitemaps
		);
	}
	return false;
}
async function getPageStaticInfo(params) {
	...
	if (
		...
		try {
			config = (0, _extractconstvalue.extractExportedConstValue)(
				swcAST,
				"config",
			);
		} catch (e) {
		...
		if (extraProperties && pageType === _pagetypes.PAGE_TYPES.APP) {
			for (const prop of extraProperties) {
				...
				try {
					extraConfig[prop] = (0, _extractconstvalue.extractExportedConstValue)(
						swcAST,
						prop,
					);
				} catch (e) {
		...
		if (
			...
			!(0, _isedgeruntime.isEdgeRuntime)(resolvedRuntime)
		) {
		...
		const requiresServerRuntime =
			ssr || ssg || pageType === _pagetypes.PAGE_TYPES.APP;
		const isAnAPIRoute = (0, _isapiroute.isAPIRoute)(
			page == null ? void 0 : page.replace(/^(?:\/src)?\/pages\//, "/"),
		);
		resolvedRuntime =
			(0, _isedgeruntime.isEdgeRuntime)(resolvedRuntime) ||
		...
		if (
			resolvedRuntime === _constants.SERVER_RUNTIME.edge &&
			pageType === _pagetypes.PAGE_TYPES.PAGES &&
			page &&
			!isAnAPIRoute
		) {
		...
		if (
			...
			(directives == null ? void 0 : directives.has("client")) &&
			generateStaticParams
		) {
			throw new Error(
