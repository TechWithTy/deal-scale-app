...
const getSupportedArchTriples = () => {
	const { darwin, win32, linux, freebsd, android } =
...
const triples = (() => {
	...
	const targetTriple =
		(_supportedArchTriples_PlatformName = supportedArchTriples[PlatformName]) ==
		null
			? void 0
			: _supportedArchTriples_PlatformName[ArchName];
	...
	let rawTargetTriple =
		(_platformArchTriples_PlatformName =
			_triples.platformArchTriples[PlatformName]) == null
			? void 0
			: _platformArchTriples_PlatformName[ArchName];
	if (rawTargetTriple) {
		_log.warn(
...
async function loadBindings(useWasmBinary = false) {
	...
	if (process.stdout._handle != null) {
		// @ts-ignore
		process.stdout._handle.setBlocking == null
			? void 0
			: process.stdout._handle.setBlocking.call(process.stdout._handle, true);
	}
	if (process.stderr._handle != null) {
		// @ts-ignore
		process.stderr._handle.setBlocking == null
			? void 0
			: process.stderr._handle.setBlocking.call(process.stderr._handle, true);
	}
	pendingBindings = new Promise(async (resolve, _reject) => {
		if (!lockfilePatchPromise.cur) {
			// always run lockfile check once so that it gets patched
			// even if it doesn't fail to load locally
			lockfilePatchPromise.cur = (0,
			_patchincorrectlockfile.patchIncorrectLockfile)(process.cwd()).catch(
				console.error,
			);
		}
		let attempts = [];
		...
		const unsupportedPlatform = triples.some(
			(triple) =>
				!!(triple == null ? void 0 : triple.raw) &&
				knownDefaultWasmFallbackTriples.includes(triple.raw),
		);
...
async function tryLoadNativeWithFallback(attempts) {
	const nativeBindingsDirectory = _path.default.join(
		_path.default.dirname(require.resolve("next/package.json")),
		"next-swc-fallback",
	);
	if (!downloadNativeBindingsPromise) {
		downloadNativeBindingsPromise = (0, _downloadswc.downloadNativeNextSwc)(
			nextVersion,
			nativeBindingsDirectory,
			triples.map((triple) => triple.platformArchABI),
		);
	}
...
async function tryLoadWasmWithFallback(attempts) {
	...
	try {
		...
		const wasmDirectory = _path.default.join(
			_path.default.dirname(require.resolve("next/package.json")),
			"wasm",
		);
		if (!downloadWasmPromise) {
			downloadWasmPromise = (0, _downloadswc.downloadWasmSwc)(
				nextVersion,
				wasmDirectory,
			);
		}
...
function createDefineEnv({
	...
	for (const variant of Object.keys(defineEnv)) {
		defineEnv[variant] = rustifyEnv(
			(0, _defineenvplugin.getDefineEnv)({
				isTurbopack,
				clientRouterFilters,
				config,
				dev,
...
function bindingToApi(binding, _wasm) {
	...
	class ProjectImpl {
		...
		entrypointsSubscribe() {
			const subscription = subscribe(false, async (callback) =>
		...
		hmrIdentifiersSubscribe() {
			return subscribe(false, async (callback) =>
		...
		updateInfoSubscribe(aggregationMs) {
			const subscription = subscribe(true, async (callback) =>
	...
	class EndpointImpl {
		...
		async clientChanged() {
			const clientSubscription = subscribe(false, async (callback) =>
		...
		async serverChanged(includeIssues) {
			const serverSubscription = subscribe(false, async (callback) =>
	...
	async function serializeNextConfig(nextConfig, projectPath) {
		...
		nextConfigSerializable.generateBuildId =
			await (nextConfig.generateBuildId == null
				? void 0
				: nextConfig.generateBuildId.call(nextConfig));
		...
		if (
			(_nextConfig_experimental = nextConfig.experimental) == null
				? void 0
				: (_nextConfig_experimental_turbo = _nextConfig_experimental.turbo) ==
						null
					? void 0
					: _nextConfig_experimental_turbo.rules
		) {
			var _nextConfig_experimental_turbo1;
			ensureLoadersHaveSerializableOptions(
				(_nextConfig_experimental_turbo1 = nextConfig.experimental.turbo) ==
					null
					? void 0
					: _nextConfig_experimental_turbo1.rules,
			);
		}
		nextConfigSerializable.modularizeImports =
			nextConfigSerializable.modularizeImports
				? Object.fromEntries(
						Object.entries(nextConfigSerializable.modularizeImports).map(
							([mod, config]) => [
								mod,
								{
									...config,
		...
		if (nextConfigSerializable.images.loaderFile) {
			nextConfigSerializable.images = {
				...
				loaderFile:
					"./" +
					_path.default.relative(projectPath, nextConfig.images.loaderFile),
			};
		}
		...
		try {
			...
			wasmBindings = {
				...
				transform(src, options) {
					...
					return (bindings == null ? void 0 : bindings.transform)
				...
				minify(src, options) {
					return (bindings == null ? void 0 : bindings.minify)
				...
				parse(src, options) {
					return (bindings == null ? void 0 : bindings.parse)
...
function getBinaryMetadata() {
	...
	return {
		target:
			bindings == null
				? void 0
				: (_bindings_getTargetTriple = bindings.getTargetTriple) == null
					? void 0
					: _bindings_getTargetTriple.call(bindings),
	};
}
