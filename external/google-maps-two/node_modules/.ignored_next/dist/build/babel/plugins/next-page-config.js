...
Object.defineProperty(
	exports, // config to parsing pageConfig for client bundles
	"default",
	{
		enumerable: true,
		get: function () {
			...
			[
				t.variableDeclaration("const", [
					t.variableDeclarator(
						t.identifier(_constants.STRING_LITERAL_DROP_BUNDLE),
						t.stringLiteral(
							`${_constants.STRING_LITERAL_DROP_BUNDLE} ${Date.now()}`,
						),
					),
				]),
			],
					...
					path.traverse(
						{
							ExportDeclaration(exportPath, exportState) {
								...
								if (
									_core.types.isExportNamedDeclaration(exportPath.node) &&
									((_exportPath_node_specifiers = exportPath.node.specifiers) ==
									null
										? void 0
										: _exportPath_node_specifiers.some((specifier) => {
												return (
							...
							ExportNamedDeclaration(exportPath, exportState) {
								...
								const config = {};
								const declarations = [
									...(((_exportPath_node_declaration =
										exportPath.node.declaration) == null
										? void 0
										: _exportPath_node_declaration.declarations) || []),
									(_exportPath_scope_getBinding =
										exportPath.scope.getBinding(CONFIG_KEY)) == null
										? void 0
										: _exportPath_scope_getBinding.path.node,
								].filter(Boolean);
								for (const specifier of exportPath.node.specifiers) {
									if (
										...
										if (_core.types.isStringLiteral(exportPath.node.source)) {
											...
											if (
												_core.types.isImportSpecifier(
													(_exportPath_scope_getBinding1 =
														exportPath.scope.getBinding(
															specifier.local.name,
														)) == null
														? void 0
														: _exportPath_scope_getBinding1.path.node,
												)
											) {
												throw new Error(
													errorMessage(
								...
								for (const declaration of declarations) {
									if (
										!_core.types.isIdentifier(declaration.id, {
											name: CONFIG_KEY,
										})
									) {
										continue;
									...
									for (const prop of init.properties) {
										...
										const { name } = prop.key;
										if (
											_core.types.isIdentifier(prop.key, {
												name: "amp",
											})
										) {
											...
											if (
												!_core.types.isBooleanLiteral(prop.value) &&
												!_core.types.isStringLiteral(prop.value)
											) {
												throw new Error(
													errorMessage(
														exportState,
								...
								if (config.amp === true) {
									...
									if (
										!((_exportState_file = exportState.file) == null
											? void 0
											: (_exportState_file_opts = _exportState_file.opts) ==
													null
												? void 0
												: _exportState_file_opts.caller.isDev)
									) {
