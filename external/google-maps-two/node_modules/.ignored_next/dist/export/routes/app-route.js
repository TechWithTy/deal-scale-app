...
async function exportAppRoute(
	...
	const request = _nextrequest.NextRequestAdapter.fromNodeNextRequest(
		new _node.NodeNextRequest(req),
		(0, _nextrequest.signalFromNodeResponse)(res),
	);
	...
	try {
		...
		const revalidate =
			typeof ((_context_renderOpts_store = context.renderOpts.store) == null
				? void 0
				: _context_renderOpts_store.revalidate) === "undefined"
				? false
		...
		await fileWriter(
			"BODY",
			htmlFilepath.replace(/\.html$/, _constants.NEXT_BODY_SUFFIX),
			body,
			"utf8",
		);
		...
		await fileWriter(
			"META",
			htmlFilepath.replace(/\.html$/, _constants.NEXT_META_SUFFIX),
			JSON.stringify(meta),
		);
		return {
