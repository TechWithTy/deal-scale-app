...
const _renderresult = /*#__PURE__*/ _interop_require_default(
	require("../../server/render-result"),
);
const _path = require("path");
const _ampmode = require("../../shared/lib/amp-mode");
const _constants = require("../../lib/constants");
const _bailouttocsr = require("../../shared/lib/lazy-dynamic/bailout-to-csr");
const _amphtmlvalidator = /*#__PURE__*/ _interop_require_default(
...
async function exportPages(
	...
	const ampState = {
		ampFirst:
			((_components_pageConfig = components.pageConfig) == null
				? void 0
				: _components_pageConfig.amp) === true,
		hasQuery: Boolean(query.amp),
		hybrid:
			((_components_pageConfig1 = components.pageConfig) == null
				? void 0
				: _components_pageConfig1.amp) === "hybrid",
	};
	...
	if (typeof components.Component === "string") {
		...
		try {
			renderResult = await (0, _modulerender.lazyRenderPagesPage)(
				req,
				res,
				page,
				query,
	...
	const ssgNotFound =
		renderResult == null ? void 0 : renderResult.metadata.isNotFound;
	const ampValidations = [];
	const validateAmp = async (rawAmpHtml, ampPageName, validatorPath) => {
		const validator =
			await _amphtmlvalidator.default.getInstance(validatorPath);
	...
	if (inAmpMode && !renderOpts.ampSkipValidation) {
		...
		const ampHtmlFilename = subFolders
			? (0, _path.join)(ampPath, "index.html")
			: `${ampPath}.html`;
		const ampHtmlFilepath = (0, _path.join)(outDir, ampHtmlFilename);
		const exists = await (0, _fileexists.fileExists)(
			ampHtmlFilepath,
			_fileexists.FileType.File,
		);
		if (!exists) {
			try {
				ampRenderResult = await (0, _modulerender.lazyRenderPagesPage)(
					req,
					res,
					page,
					{
	...
	const metadata =
		(renderResult == null ? void 0 : renderResult.metadata) ||
		(ampRenderResult == null ? void 0 : ampRenderResult.metadata) ||
		{};
	if (metadata.pageData) {
		const dataFile = (0, _path.join)(
			pagesDataDir,
			htmlFilename.replace(/\.html$/, _constants.NEXT_DATA_SUFFIX),
		);
		await fileWriter(
			"DATA",
			dataFile,
