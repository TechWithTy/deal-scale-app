...
async function exportAppPage(
	...
	try {
		const result = await (0, _modulerender.lazyRenderAppPage)(
			req,
			res,
			pathname,
			query,
		...
		if (revalidate === 0) {
			...
			if (
				revalidate === 0 &&
				debugOutput &&
				(staticBailoutInfo == null ? void 0 : staticBailoutInfo.description)
			) {
			...
			await fileWriter(
				"PREFETCH_FLIGHT",
				htmlFilepath.replace(/\.html$/, _constants.RSC_PREFETCH_SUFFIX),
				flightData,
			);
		} else {
			...
			await fileWriter(
				"FLIGHT",
				htmlFilepath.replace(/\.html$/, _constants.RSC_SUFFIX),
				flightData,
			);
		}
		const headers = {
