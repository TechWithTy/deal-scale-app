...
function setupWorkers(options, nextConfig) {
	...
	const timeout =
		(nextConfig == null ? void 0 : nextConfig.staticPageGenerationTimeout) || 0;
...
async function exportAppImpl(dir, options, span) {
	...
	const nextConfig =
		...
		(await span
			...
			.traceAsyncFn(() => (0, _config.default)(_constants1.PHASE_EXPORT, dir)));
	...
	if (telemetry) {
		telemetry.record(
			(0, _events.eventCliSession)(distDir, nextConfig, {
				webpackVersion: null,
				cliCommand: "export",
				isSrcDir: null,
	...
	try {
		prerenderManifest = require(
			(0, _path.join)(distDir, _constants1.PRERENDER_MANIFEST),
		);
	} catch {}
	let appRoutePathManifest;
	try {
		appRoutePathManifest = require(
			(0, _path.join)(distDir, _constants1.APP_PATH_ROUTES_MANIFEST),
		);
	} catch (err) {
		if (
	...
	for (const page of pages) {
		...
		if (
			prerenderManifest == null ? void 0 : prerenderManifest.dynamicRoutes[page]
		) {
	...
	if (!options.buildExport && appRoutePathManifest) {
		for (const [pageName, routePath] of Object.entries(appRoutePathManifest)) {
			...
			if (
				(0, _isapppageroute.isAppPageRoute)(pageName) &&
	...
	await _fs.promises.writeFile(
		...
		(0, _formatmanifest.formatManifest)({
			version: 1,
			outDirectory: outDir,
			success: false,
		}),
	...
	if (!options.buildExport) {
		const { isNextImageImported } = await span
			...
			.traceAsyncFn(() =>
				_fs.promises
					.readFile((0, _path.join)(distDir, _constants1.EXPORT_MARKER), "utf8")
  ...
  Read more: https://nextjs.org/docs/messages/export-image-api`);
	...
	const renderOpts = {
		previewProps:
			prerenderManifest == null ? void 0 : prerenderManifest.preview,
		buildId,
		nextExport: true,
		...
		canonicalBase:
			((_nextConfig_amp = nextConfig.amp) == null
				? void 0
				: _nextConfig_amp.canonicalBase) || "",
		ampSkipValidation:
			((_nextConfig_experimental_amp = nextConfig.experimental.amp) == null
				? void 0
				: _nextConfig_experimental_amp.skipValidation) || false,
		ampOptimizerConfig:
			((_nextConfig_experimental_amp1 = nextConfig.experimental.amp) == null
				? void 0
				: _nextConfig_experimental_amp1.optimizer) || undefined,
	...
	const filteredPaths = exportPaths.filter(
		(route) =>
			exportPathMap[route]._isAppDir || // Remove API routes
		...
		filteredPaths.map(async (path) => {
			...
			const result = await pageExportSpan.traceAsyncFn(async () => {
				...
				return await exportPage({
					...
					renderOpts,
					ampValidatorPath:
						((_nextConfig_experimental_amp = nextConfig.experimental.amp) ==
						null
							? void 0
							: _nextConfig_experimental_amp.validator) || undefined,
					trailingSlash: nextConfig.trailingSlash,
					serverRuntimeConfig,
					subFolders,
					...
					enableExperimentalReact: (0,
					_needsexperimentalreact.needsExperimentalReact)(nextConfig),
					enabledDirectories,
				});
			});
	...
	for (const { result, path } of results) {
		...
		if (result.turborepoAccessTraceResult) {
			...
			(_collector_turborepoAccessTraceResults =
				collector.turborepoAccessTraceResults) == null
				? void 0
				: _collector_turborepoAccessTraceResults.set(
						path,
						_turborepoaccesstrace.TurborepoAccessTraceResult.fromSerialized(
		...
		if (options.buildExport) {
			...
			if (typeof result.revalidate !== "undefined") {
				info.revalidate = (0, _patchfetch.validateRevalidate)(
					result.revalidate,
					path,
				);
			}
		...
		await Promise.all(
			Object.keys(prerenderManifest.routes).map(async (route) => {
				...
				const isAppRouteHandler =
					appPageName && (0, _isapprouteroute.isAppRouteRoute)(appPageName);
				...
				const htmlDest = (0, _path.join)(
					outDir,
					`${route}${subFolders && route !== "/index" ? `${_path.sep}index` : ""}.html`,
				);
				...
				const jsonDest = isAppPath
					? (0, _path.join)(
							outDir,
							`${route}${subFolders && route !== "/index" ? `${_path.sep}index` : ""}.txt`,
						)
					: (0, _path.join)(pagesDataDir, `${route}.json`);
