...
async function exportPageImpl(input, fileWriter) {
	...
	const {
		page, // Check if this is an `app/` page.
	...
	try {
		...
		const outDir = isAppDir
			? (0, _path.join)(distDir, "server/app")
			: input.outDir;
		let params;
		...
		const { pathname: nonLocalizedPath } = (0,
		_normalizelocalepath.normalizeLocalePath)(path, input.renderOpts.locales);
		if (isDynamic && page !== nonLocalizedPath) {
			const normalizedPage = isAppDir
		...
		if (
			trailingSlash &&
			!((_req_url = req.url) == null ? void 0 : _req_url.endsWith("/"))
		) {
			req.url += "/";
		}
		if (
			...
			input.renderOpts.domainLocales.some((dl) => {
				var _dl_locales;
				return (
					dl.defaultLocale === locale ||
					((_dl_locales = dl.locales) == null
						? void 0
						: _dl_locales.includes(locale || ""))
				);
			})
		) {
		...
		const getHtmlFilename = (p) =>
			subFolders ? `${p}${_path.sep}index.html` : `${p}.html`;
		...
		if (isAppDir && (0, _isapprouteroute.isAppRouteRoute)(page)) {
			return await (0, _approute.exportAppRoute)(
				req,
				res,
				params,
				page,
		...
		const renderOpts = {
			...
			fontManifest: optimizeFonts
				? (0, _require.requireFontManifest)(distDir)
				: undefined,
			locale,
		...
		if (isAppDir) {
			...
			return await (0, _apppage.exportAppPage)(
				req,
				res,
				page,
				path,
		...
		return await (0, _pages.exportPages)(
			req,
			res,
			path,
			page,
...
async function exportPage(input) {
	...
	const exportPageSpan = (0, _trace.trace)(
		"export-page-worker",
		input.parentSpanId,
	);
