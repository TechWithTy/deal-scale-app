...
function isIgnoredPlugin(pluginPath) {
	...
	console.warn(
		`${yellow(bold("Warning"))}: Please remove the ${underline(plugin)} plugin from your PostCSS configuration. ` +
...
export async function getPostCssPlugins(
	...
	if (invalidKey) {
		console.warn(
			`${yellow(bold("Warning"))}: Your PostCSS configuration defines a field which is not supported (\`${invalidKey}\`). ` +
	...
	plugins.forEach((plugin) => {
		if (plugin == null) {
			console.warn(
				`${yellow(bold("Warning"))}: A ${bold("null")} PostCSS plugin was provided. This entry will be ignored.`,
			);
			...
			if (
				...
				if (typeof pluginName !== "string") {
					console.error(
						`${red(bold("Error"))}: A PostCSS Plugin must be provided as a ${bold("string")}. Instead, we got: '${pluginName}'.\n` +
							"Read more: https://nextjs.org/docs/messages/postcss-shape",
					);
				} else {
					console.error(
						`${red(bold("Error"))}: A PostCSS Plugin was passed as an array but did not provide its configuration ('${pluginName}').\n` +
							"Read more: https://nextjs.org/docs/messages/postcss-shape",
					);
				}
				throw new Error(genericErrorText);
			...
			console.error(
				`${red(bold("Error"))}: A PostCSS Plugin was passed as a function using require(), but it must be provided as a ${bold("string")}.\nRead more: https://nextjs.org/docs/messages/postcss-shape`,
			);
			throw new Error(genericErrorText);
		} else {
			console.error(
