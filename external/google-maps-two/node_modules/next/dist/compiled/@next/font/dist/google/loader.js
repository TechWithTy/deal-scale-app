...
const nextFontGoogleFontLoader = async ({
	...
	const {
		...
		variable,
		subsets,
	} = (0, validate_google_font_function_call_1.validateGoogleFontFunctionCall)(
		functionName,
		data[0],
	);
	// Validate and get the font axes required to generated the URL
	const fontAxes = (0, get_font_axes_1.getFontAxes)(
		fontFamily,
		weights,
		styles,
		selectedVariableAxes,
	...
	const url = (0, get_google_fonts_url_1.getGoogleFontsUrl)(
		fontFamily,
		fontAxes,
		display,
	);
	// Get precalculated fallback font metrics, used to generate the fallback font CSS
	const adjustFontFallbackMetrics = adjustFontFallback
		? (0, get_fallback_font_override_metrics_1.getFallbackFontOverrideMetrics)(
				fontFamily,
			)
		: undefined;
	const result = {
	...
	try {
		...
		let fontFaceDeclarations = hasCachedCSS
			? cssCache.get(url)
			: await (0, fetch_css_from_google_fonts_1.fetchCSSFromGoogleFonts)(
					url,
					fontFamily,
					isDev,
				).catch((err) => {
		...
		if (!hasCachedCSS) {
			cssCache.set(
				url,
				fontFaceDeclarations !== null && fontFaceDeclarations !== void 0
		...
		const fontFiles = (0, find_font_files_in_css_1.findFontFilesInCss)(
			fontFaceDeclarations,
		...
		const downloadedFiles = await Promise.all(
			fontFiles.map(async ({ googleFontFileUrl, preloadFontFile }) => {
				...
				const fontFileBuffer = hasCachedFont
					...
					: await (0, fetch_font_file_1.fetchFontFile)(
							googleFontFileUrl,
							isDev,
						).catch((err) => {
							console.error(err);
				...
				if (!hasCachedFont) {
					fontCache.set(
						googleFontFileUrl,
						fontFileBuffer !== null && fontFileBuffer !== void 0
							? fontFileBuffer
							: null,
					);
				} else {
