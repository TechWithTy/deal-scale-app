	...
	var __webpack_modules__ = {
		...
		655: (e, s, r) => {
			...
			function resolve(e, s) {
				return f(p, e).reduce(function (e, r, n) {
					if (r.not && n === 0) {
						throw new l(
			...
			function browserslist(e, s) {
				...
				var n = u.getStat(s, browserslist.data);
				if (n) {
					r.customUsage = {};
					for (var t in n) {
						fillUsage(r.customUsage, t, n[t]);
					}
				}
				var a = JSON.stringify([e, r]);
				if (m[a]) return m[a];
				var i = uniq(resolve(e, r)).sort(function (e, s) {
					e = e.split(" ");
					s = s.split(" ");
					if (e[0] === s[0]) {
						var r = e[1].split("-")[0];
			...
			browserslist.coverage = function (e, s) {
				...
				if (typeof s === "undefined") {
					...
					u.loadCountry(browserslist.usage, s, browserslist.data);
			...
			function coverQuery(e, s) {
				...
				if (s.place) {
					if (s.place.match(/^my\s+stats$/i)) {
						...
						u.loadCountry(browserslist.usage, t, browserslist.data);
			...
			var p = {
				...
				last_electron_major_versions: {
					...
					select: function (e, s) {
						var r = getMajorVersions(Object.keys(o), s.versions);
				...
				popularity_in_config_stats: {
					...
					select: function (e, s) {
						var r = parseFloat(s.popularity);
						var n = u.loadStat(e, s.config, browserslist.data);
						if (n) {
							e.customUsage = {};
							for (var t in n) {
				...
				popularity_in_place: {
					...
					select: function (e, s) {
						...
						u.loadCountry(browserslist.usage, n, browserslist.data);
				...
				supports: {
					...
					select: function (e, s) {
						u.loadFeature(browserslist.cache, s.feature);
						...
						for (var a in n) {
							...
							var o =
								e.mobileToDesktop &&
								a in browserslist.desktopNames &&
								isSupported(n[a][i.released.slice(-1)[0]], r);
				...
				electron_range: {
					...
					select: function (e, s) {
						...
						var a = parseFloat(s.to);
						if (!o[r]) {
							throw new l("Unknown version " + t + " of electron");
						}
						if (!o[n]) {
							throw new l("Unknown version " + a + " of electron");
						}
						return Object.keys(o)
							.filter(function (e) {
								var s = parseFloat(e);
				...
				current_node: {
					...
					select: function (e) {
						return [u.currentNode(resolve, e)];
					},
				},
				maintained_node: {
					matches: [],
					...
					select: function (e) {
						...
						var r = Object.keys(a)
							.filter(function (e) {
								return (
									s < Date.parse(a[e].end) &&
									s > Date.parse(a[e].start) &&
									isEolReleased(e)
								);
							})
							.map(function (e) {
				...
				extends: {
					...
					regexp: /^extends (.+)$/i,
					select: function (e, s) {
						return resolve(u.loadQueries(e, s.config), e);
					},
				},
				defaults: {
					matches: [],
			...
			module.exports = {
				...
				loadCountry: function loadCountry(e, s, r) {
					...
					if (!e[n]) {
						var t;
						try {
							t = require("caniuse-lite/data/regions/" + n + ".js");
						} catch (e) {
				...
				findConfig: function findConfig(e) {
					e = path.resolve(e);
					var s = [];
					var r = eachParent(e, function (e) {
						if (e in configCache) {
							return configCache[e];
						}
						s.push(e);
						var r = path.join(e, "browserslist");
						var n = path.join(e, "package.json");
						var t = path.join(e, ".browserslistrc");
						var a;
						if (isFile(n)) {
							try {
								a = parsePackage(n);
						...
						} else if (isFile(r) && isFile(t)) {
							throw new BrowserslistError(
								e + " contains both .browserslistrc and browserslist",
							);
						} else if (isFile(r)) {
							return module.exports.readConfig(r);
						} else if (isFile(t)) {
