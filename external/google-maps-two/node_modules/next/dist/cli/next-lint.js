...
const nextLint = async (options, directory) => {
	...
	if (!(0, _fs.existsSync)(baseDir)) {
		(0, _utils.printAndExit)(
			`> No such directory exists as the project root: ${baseDir}`,
		);
	}
	const nextConfig = await (0, _config.default)(
		_constants1.PHASE_PRODUCTION_BUILD,
		baseDir,
	);
	const files = options.file ?? [];
	const dirs =
		options.dir ??
		((_nextConfig_eslint = nextConfig.eslint) == null
			? void 0
			: _nextConfig_eslint.dirs);
	const filesToLint = [...(dirs ?? []), ...files];
	const pathsToLint = (
		filesToLint.length ? filesToLint : _constants.ESLINT_DEFAULT_DIRS
	).reduce((res, d) => {
	...
	(0, _runLintCheck.runLintCheck)(baseDir, pathsToLint, {
		...
		.then(async (lintResults) => {
			...
			if (
				typeof lintResults !== "string" &&
				(lintResults == null ? void 0 : lintResults.eventInfo)
			) {
				const telemetry = new _storage.Telemetry({
					distDir,
				});
				telemetry.record(
					(0, _events.eventLintCheckCompleted)({
			...
			if (
				typeof lintResults !== "string" &&
				(lintResults == null ? void 0 : lintResults.isError) &&
				lintOutput
			) {
			...
			if (lintOutput) {
				...
				(0, _utils.printAndExit)(
					(0, _picocolors.green)("✔ No ESLint warnings or errors"),
					0,
				);
			} else {
