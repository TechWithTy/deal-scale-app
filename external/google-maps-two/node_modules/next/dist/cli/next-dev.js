...
const _uploadtrace = /*#__PURE__*/ _interop_require_default(
	require("../trace/upload-trace"),
);
...
const handleSessionStop = async (signal) => {
	...
	try {
		...
		let telemetry =
			_shared.traceGlobals.get("telemetry") ||
		...
		if (
			typeof _shared.traceGlobals.get("pagesDir") === "undefined" ||
			typeof _shared.traceGlobals.get("appDir") === "undefined"
		) {
...
const nextDev = async (options, portSource, directory) => {
	dir = (0, _getprojectdir.getProjectDir)(
		process.env.NEXT_PRIVATE_DEV_DIR || directory,
	);
	// Check if pages dir exists and warn if not
	if (
		!(await (0, _fileexists.fileExists)(dir, _fileexists.FileType.Directory))
	) {
	...
	if ((0, _getreservedport.isPortIsReserved)(port)) {
		(0, _utils.printAndExit)(
			(0, _getreservedport.getReservedPortExplanation)(port),
			1,
		);
	}
