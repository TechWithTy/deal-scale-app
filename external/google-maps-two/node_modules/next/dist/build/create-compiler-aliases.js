...
function createWebpackAliases({
	...
	return {
		...
		...(isClient || isEdgeServer
			...
			: {}),
		"@swc/helpers/_": _path.default.join(
			_path.default.dirname(require.resolve("@swc/helpers/package.json")),
			"_",
		),
...
function createNextApiEsmAliases() {
	...
	for (const [key, value] of Object.entries(mapping)) {
		const nextApiFilePath = _path.default.join(
			_webpackconfig.NEXT_PROJECT_ROOT,
			key,
		);
...
function createRSCAliases(
	bundledReactChannel,
	{ layer, isEdgeServer, reactProductionProfiling },
) {
	let alias = {
		...
		try {
			...
			for (const field of mainFields) {
				if (descriptionFileData.hasOwnProperty(field)) {
					aliases[pkg + "$"] = _path.default.join(
						_path.default.dirname(descriptionFilePath),
