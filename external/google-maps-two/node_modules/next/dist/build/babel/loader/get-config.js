...
const _json5 = /*#__PURE__*/ _interop_require_default(
	require("next/dist/compiled/json5"),
);
const _core = require("next/dist/compiled/babel/core");
const _corelibconfig = /*#__PURE__*/ _interop_require_default(
	require("next/dist/compiled/babel/core-lib-config"),
);
...
function getCacheCharacteristics(loaderOptions, source, filename) {
	...
	const fileExt =
		((_fileExtensionRegex_exec = fileExtensionRegex.exec(filename)) == null
			? void 0
			: _fileExtensionRegex_exec[1]) || "unknown";
	return {
		isServer,
		isPageFile,
		isNextDist,
 ...
 */ function getPlugins(loaderOptions, cacheCharacteristics) {
	...
	const applyCommonJsItem = hasModuleExports
		? (0, _core.createConfigItem)(require("../plugins/commonjs"), {
				type: "plugin",
			})
		: null;
	...
	const pageConfigItem =
		!isServer && isPageFile
			? (0, _core.createConfigItem)([require("../plugins/next-page-config")], {
					type: "plugin",
				})
			: null;
 ...
 */ function checkCustomBabelConfigDeprecation(config) {
	...
	if (isPresetReadyToDeprecate && unsupportedPlugins.length === 0) {
		_log.warn(
			`It looks like there is a custom Babel configuration that can be removed${pluginReasons.length > 0 ? ":" : "."}`,
		);
 ...
 */ function getFreshConfig(
	...
	const config = (0, _util.consumeIterator)(
		(0, _corelibconfig.default)(loadedOptions),
	);
	return config;
}
/**
