...
const _gzipsize = /*#__PURE__*/ _interop_require_default(
	require("next/dist/compiled/gzip-size"),
);
const _texttable = /*#__PURE__*/ _interop_require_default(
	require("next/dist/compiled/text-table"),
);
const _path = /*#__PURE__*/ _interop_require_default(require("path"));
const _fs = require("fs");
const _reactis = require("next/dist/compiled/react-is");
const _stripansi = /*#__PURE__*/ _interop_require_default(
	require("next/dist/compiled/strip-ansi"),
);
const _browserslist = /*#__PURE__*/ _interop_require_default(
	require("next/dist/compiled/browserslist"),
);
const _constants = require("../lib/constants");
const _constants1 = require("../shared/lib/constants");
const _prettybytes = /*#__PURE__*/ _interop_require_default(
	require("../lib/pretty-bytes"),
);
...
async function computeFromManifest(
	...
	if (
		(_manifests_app = manifests.app) == null ? void 0 : _manifests_app.pages
	) {
		files.app = {
			each: new Map(),
			expected: 0,
	...
	await Promise.all(
		[
			...new Set([
				...
				...(((_files_app = files.app) == null
					? void 0
					: _files_app.each.keys()) ?? []),
			]),
		].map(async (f) => {
			try {
...
function isMiddlewareFilename(file) {
	return (
		file === _constants.MIDDLEWARE_FILENAME ||
		file === `src/${_constants.MIDDLEWARE_FILENAME}`
	);
}
function isInstrumentationHookFilename(file) {
	return (
		file === _constants.INSTRUMENTATION_HOOK_FILENAME ||
...
async function printTreeView(
	...
	const hasCustomApp = !!(
		pagesDir &&
		(await (0, _findpagefile.findPageFile)(
			pagesDir,
			"/_app",
			pageExtensions,
			false,
	...
	const printFileTree = async ({ list, routerType }) => {
		...
		filteredPages.forEach((item, i, arr) => {
			var _pageInfo_ssgPageDurations,
				_buildManifest_pages_item,
				_pageInfo_ssgPageRoutes;
			const border =
				i === 0
					? arr.length === 1
						? "─"
			...
			const totalDuration =
				((pageInfo == null ? void 0 : pageInfo.pageDuration) || 0) +
				((pageInfo == null
					? void 0
					: (_pageInfo_ssgPageDurations = pageInfo.ssgPageDurations) == null
						? void 0
						: _pageInfo_ssgPageDurations.reduce((a, b) => a + (b || 0), 0)) ||
					0);
			let symbol;
			if (item === "/_app" || item === "/_app.server") {
				symbol = " ";
			} else if (
				(0, _isedgeruntime.isEdgeRuntime)(
					pageInfo == null ? void 0 : pageInfo.runtime,
				)
			) {
				symbol = "ƒ";
			...
			const uniqueCssFiles =
				((_buildManifest_pages_item = buildManifest.pages[item]) == null
					? void 0
					: _buildManifest_pages_item.filter((file) => {
							var _stats_router_routerType;
							return (
								file.endsWith(".css") &&
								((_stats_router_routerType = stats.router[routerType]) == null
									? void 0
									: _stats_router_routerType.unique.files.includes(file))
							);
						})) || [];
			...
			if (
				pageInfo == null
					? void 0
					: (_pageInfo_ssgPageRoutes = pageInfo.ssgPageRoutes) == null
						? void 0
						: _pageInfo_ssgPageRoutes.length
			) {
		...
		const sharedFilesSize =
			(_stats_router_routerType = stats.router[routerType]) == null
				? void 0
				: _stats_router_routerType.common.size.total;
		const sharedFiles =
			((_stats_router_routerType1 = stats.router[routerType]) == null
				? void 0
				: _stats_router_routerType1.common.files) ?? [];
		messages.push([
	...
	if (
		...
		!((_lists_app = lists.app) == null
			? void 0
			: _lists_app.includes(_constants1.UNDERSCORE_NOT_FOUND_ROUTE))
	) {
	...
	const middlewareInfo =
		(_middlewareManifest_middleware = middlewareManifest.middleware) == null
			? void 0
			: _middlewareManifest_middleware["/"];
...
async function getJsPageSizeInKb(
	routerType,
	page,
	distPath,
	buildManifest,
	...
	const pagePath =
		routerType === "pages"
			? (0, _denormalizepagepath.denormalizePagePath)(page)
			: (0, _denormalizeapppath.denormalizeAppPagePath)(page);
		...
		if (typeof entry === "string") {
			...
			prerenderPaths.add(
				entry
					.split("/")
					.map((segment) =>
						(0, _escapepathdelimiters.default)(
			...
			_validParamKeys.forEach((validParamKey) => {
				...
				if (
					...
					throw new Error(
						`A required parameter (${validParamKey}) was not provided as ${repeat ? "an array" : "a string"} received ${typeof paramValue} in ${appDir ? "generateStaticParams" : "getStaticPaths"} for ${page}`,
					);
				}
				...
				builtPage = builtPage
					.replace(
						...
						repeat
							? paramValue
									.map((segment) =>
										(0, _escapepathdelimiters.default)(segment, true),
									)
									.join("/")
							: (0, _escapepathdelimiters.default)(paramValue, true),
					)
					.replace(/\\/g, "/")
					.replace(/(?!^)\/$/, "");
			...
			if (
				entry.locale &&
				!(locales == null ? void 0 : locales.includes(entry.locale))
			) {
				throw new Error(
...
async function collectGenerateParams(tree) {
	...
	while (currentLoaderTree) {
		...
		const mod = await (isLayout
			? (_components_layout = components.layout) == null
				? void 0
				: (_components_layout_ = _components_layout[0]) == null
					? void 0
					: _components_layout_.call(_components_layout)
			: (_components_page = components.page) == null
				? void 0
				: (_components_page_ = _components_page[0]) == null
					? void 0
					: _components_page_.call(_components_page));
		if (page) {
...
async function buildAppStaticPaths({
	dir,
	page,
	distDir,
	configFileName,
	...
	if (cacheHandler) {
		CacheHandler = (0, _interopdefault.interopDefault)(
			await import(
				(0, _formatdynamicimportpath.formatDynamicImportPath)(dir, cacheHandler)
	...
	return _staticgenerationasyncstoragewrapper.StaticGenerationAsyncStorageWrapper.wrap(
		...
		async () => {
			...
			if (
				typeof (pageEntry == null ? void 0 : pageEntry.getStaticPaths) ===
				"function"
			) {
				return buildStaticPaths({
					page,
				...
				const fallback = !generateParams.some(
					...
					(generate) => {
						...
						return (
							((_generate_config = generate.config) == null
								? void 0
								: _generate_config.dynamicParams) === false
						);
					},
				);
...
async function isPageStatic({
	...
	return isPageStaticSpan
		.traceAsyncFn(async () => {
			...
			const routeModule =
				(_componentsResult_ComponentMod = componentsResult.ComponentMod) == null
					? void 0
					: _componentsResult_ComponentMod.routeModule;
			let supportsPPR = false;
			if (pageType === "app") {
				if (
					ppr &&
					routeModule.definition.kind === _routekind.RouteKind.APP_PAGE
				) {
					supportsPPR = true;
				}
				const ComponentMod = componentsResult.ComponentMod;
				isClientComponent = (0, _clientreference.isClientReference)(
					componentsResult.ComponentMod,
				);
				const { tree } = ComponentMod;
				const generateParams =
					routeModule && (0, _checks.isAppRouteRouteModule)(routeModule)
						? [
								{
									config: {
				...
				appConfig = generateParams.reduce((builtConfig, curGenParams) => {
					...
					} = (curGenParams == null ? void 0 : curGenParams.config) || {};
				...
				if (
					!Comp ||
					!(0, _reactis.isValidElementType)(Comp) ||
					typeof Comp === "string"
				) {
					throw new Error("INVALID_DEFAULT_EXPORT");
				}
			}
			const hasGetInitialProps = !!(Comp == null
				? void 0
...
async function hasCustomGetInitialProps({
	page,
	distDir,
	runtimeEnvConfig,
	checkingApp,
...
function detectConflictingPaths(combinedPages, ssgPages, additionalSsgPaths) {
	...
	const dynamicSsgPages = [...ssgPages].filter((page) =>
		(0, _isdynamic.isDynamicRoute)(page),
	);
...
async function copyTracedFiles(
	dir,
	distDir,
	pageKeys,
	appPageKeys,
	...
	try {
		...
		const packageJson = JSON.parse(
			await _fs.promises.readFile(packageJsonPath, "utf8"),
		);
	...
	async function handleTraceFiles(traceFilePath) {
		const traceData = JSON.parse(
			await _fs.promises.readFile(traceFilePath, "utf8"),
		);
		...
		await Promise.all(
			traceData.files.map(async (relativeFile) => {
				...
				const fileOutputPath = _path.default.join(
					outputPath,
					_path.default.relative(tracingRoot, tracedFilePath),
				);
				if (!copiedFiles.has(fileOutputPath)) {
					...
					const symlink = await _fs.promises
	...
	async function handleEdgeFunction(page) {
		...
		async function handleFile(file) {
			...
			const fileOutputPath = _path.default.join(
				outputPath,
				_path.default.relative(tracingRoot, distDir),
				file,
			);
			await _fs.promises.mkdir(_path.default.dirname(fileOutputPath), {
				recursive: true,
		...
		await Promise.all([
			...
			(_page_wasm = page.wasm) == null
				? void 0
				: _page_wasm.map((file) => handleFile(file.filePath)),
			(_page_assets = page.assets) == null
				? void 0
				: _page_assets.map((file) => handleFile(file.filePath)),
		]);
	}
	...
	for (const page of pageKeys) {
		...
		const pageFile = _path.default.join(
			distDir,
			"server",
			"pages",
			`${(0, _normalizepagepath.normalizePagePath)(page)}.js`,
		);
	...
	if (hasInstrumentationHook) {
		await handleTraceFiles(
			_path.default.join(distDir, "server", "instrumentation.js.nft.json"),
		);
	}
	await handleTraceFiles(
		_path.default.join(distDir, "next-server.js.nft.json"),
	);
	const serverOutputPath = _path.default.join(
		outputPath,
		_path.default.relative(tracingRoot, dir),
		"server.js",
	);
...
function getPossibleInstrumentationHookFilenames(folder, extensions) {
	...
	for (const extension of extensions) {
		files.push(
			_path.default.join(
				folder,
				`${_constants.INSTRUMENTATION_HOOK_FILENAME}.${extension}`,
			),
			_path.default.join(
				folder,
				`src`,
...
function getPossibleMiddlewareFilenames(folder, extensions) {
	return extensions.map((extension) =>
		_path.default.join(
			folder,
			`${_constants.MIDDLEWARE_FILENAME}.${extension}`,
		),
	);
}
...
function isWebpackServerOnlyLayer(layer) {
	return Boolean(
		layer && _constants.WEBPACK_LAYERS.GROUP.serverOnly.includes(layer),
	);
}
function isWebpackClientOnlyLayer(layer) {
	return Boolean(
		layer && _constants.WEBPACK_LAYERS.GROUP.clientOnly.includes(layer),
	);
}
