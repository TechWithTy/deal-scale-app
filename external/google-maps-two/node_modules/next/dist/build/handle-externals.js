...
function isResourceInPackages(resource, packageNames, packageDirMapping) {
	...
	return packageNames.some((p) =>
		packageDirMapping && packageDirMapping.has(p)
			? resource.startsWith(packageDirMapping.get(p) + _path.default.sep)
			: resource.includes(
					_path.default.sep +
						_path.default.join(
							"node_modules",
							p.replace(/\//g, _path.default.sep),
						) +
						_path.default.sep,
				),
	);
...
async function resolveExternal(
	...
	request,
	isEsmRequested,
	_optOutBundlingPackages,
	getResolve,
	isLocalCallback,
	baseResolveCheck = true,
	esmResolveOptions = _webpackconfig.NODE_ESM_RESOLVE_OPTIONS,
	nodeResolveOptions = _webpackconfig.NODE_RESOLVE_OPTIONS,
	baseEsmResolveOptions = _webpackconfig.NODE_BASE_ESM_RESOLVE_OPTIONS,
	baseResolveOptions = _webpackconfig.NODE_BASE_RESOLVE_OPTIONS,
) {
...
function makeExternalHandler({
	...
	const looseEsmExternals =
		((_config_experimental = config.experimental) == null
			? void 0
			: _config_experimental.esmExternals) === "loose";
	return async function handleExternals(
		...
		// not be bundled.
		const isLocal =
			request.startsWith(".") || // Always check for unix-style path, as webpack sometimes
			// normalizes as posix.
		...
		if (
			(0, _utils.isWebpackServerOnlyLayer)(layer) &&
		...
		if (config.transpilePackages && !resolvedExternalPackageDirs) {
			...
			for (const pkg of config.transpilePackages) {
				...
				if (pkgRes.res) {
					resolvedExternalPackageDirs.set(
						pkg,
						_path.default.dirname(pkgRes.res),
					);
				}
			}
		}
