...
const _devalue = /*#__PURE__*/ _interop_require_default(
	require("next/dist/compiled/devalue"),
);
...
const _loadcustomroutes = /*#__PURE__*/ _interop_require_wildcard(
...
const _ciinfo = /*#__PURE__*/ _interop_require_wildcard(
	require("../telemetry/ci-info"),
);
...
function getCacheDir(distDir) {
	...
	if (_ciinfo.isCI && !_ciinfo.hasNextSupport) {
		...
		if (!hasCache) {
			...
			// stderr is detected.
			console.log(
				`${_log.prefixes.warn} No build cache found. Please configure build caching for faster rebuilds. Read more: https://nextjs.org/docs/messages/no-cache`,
			);
		}
	}
	return cacheDir;
...
async function writePrerenderManifest(distDir, manifest) {
	await writeManifest(
		_path.default.join(distDir, _constants1.PRERENDER_MANIFEST),
		manifest,
	);
}
async function writeClientSsgManifest(
	prerenderManifest,
	{ buildId, distDir, locales },
) {
	const ssgPages = new Set(
		[
			...Object.entries(prerenderManifest.routes) // Filter out dynamic routes
				...
				.map(
					([route]) =>
						(0, _normalizelocalepath.normalizeLocalePath)(route, locales)
							.pathname,
				),
...
async function writeFunctionsConfigManifest(distDir, manifest) {
	await writeManifest(
		_path.default.join(
			...
			_constants1.FUNCTIONS_CONFIG_MANIFEST,
		),
		manifest,
	);
}
async function writeRequiredServerFilesManifest(distDir, requiredServerFiles) {
	await writeManifest(
		_path.default.join(distDir, _constants1.SERVER_FILES_MANIFEST),
		requiredServerFiles,
	);
}
async function writeImagesManifest(distDir, config) {
	...
	images.remotePatterns = (
		(config == null
			? void 0
			: (_config_images = config.images) == null
				? void 0
				: _config_images.remotePatterns) || []
	).map((p) => ({
	...
	if (
		config == null
			? void 0
			: (_config_images1 = config.images) == null
				? void 0
				: _config_images1.localPatterns
	) {
	...
	await writeManifest(
		_path.default.join(distDir, _constants1.IMAGES_MANIFEST),
		{
			version: 1,
			images,
		},
...
async function writeStandaloneDirectory(
	...
	await nextBuildSpan
		...
		.traceAsyncFn(async () => {
			await (0, _utils1.copyTracedFiles)(
			...
			for (const file of [
				...
				const outputPath = _path.default.join(
					distDir,
					STANDALONE_DIRECTORY,
					_path.default.relative(outputFileTracingRoot, filePath),
				);
			...
			await (0, _recursivecopy.recursiveCopy)(
				_path.default.join(distDir, _constants1.SERVER_DIRECTORY, "pages"),
				_path.default.join(
					distDir,
					STANDALONE_DIRECTORY,
					_path.default.relative(outputFileTracingRoot, distDir),
					_constants1.SERVER_DIRECTORY,
					"pages",
				),
				{
			...
			if (appDir) {
				const originalServerApp = _path.default.join(
					distDir,
					_constants1.SERVER_DIRECTORY,
					"app",
				);
...
function getNumberOfWorkers(config) {
	if (
		...
		config.experimental.cpus !== _configshared.defaultConfig.experimental.cpus
	) {
	...
	if (config.experimental.memoryBasedWorkersCount) {
		return Math.max(
			Math.min(
				...
				Math.floor(_os.default.freemem() / 1e9),
...
async function getBuildId(isGenerateMode, distDir, nextBuildSpan, config) {
	if (isGenerateMode) {
		return await _fs.promises.readFile(
			_path.default.join(distDir, "BUILD_ID"),
			"utf8",
		);
	}
	return await nextBuildSpan
		...
		.traceAsyncFn(() =>
			(0, _generatebuildid.generateBuildId)(
...
async function build(
	...
	try {
		...
		_buildcontext.NextBuildContext.reactProductionProfiling =
		...
		await nextBuildSpan.traceAsyncFn(async () => {
			...
			const { loadedEnvFiles } = nextBuildSpan
				...
				.traceFn(() => (0, _env.loadEnvConfig)(dir, false, _log));
			_buildcontext.NextBuildContext.loadedEnvFiles = loadedEnvFiles;
			const turborepoAccessTraceResult =
				new _turborepoaccesstrace.TurborepoAccessTraceResult();
			const config = await nextBuildSpan
				.traceChild("load-next-config")
				.traceAsyncFn(() =>
					(0, _turborepoaccesstrace.turborepoTraceAccess)(
						() =>
							(0, _config.default)(_constants1.PHASE_PRODUCTION_BUILD, dir, {
			...
			const customRoutes = await nextBuildSpan
				...
				.traceAsyncFn(() => (0, _loadcustomroutes.default)(config));
			...
			_buildcontext.NextBuildContext.originalRewrites =
				config._originalRewrites;
			_buildcontext.NextBuildContext.originalRedirects =
				config._originalRedirects;
			...
			_encryptionutils.generateEncryptionKeyBase64)();
			_buildcontext.NextBuildContext.encryptionKey = encryptionKey;
			const isSrcDir = _path.default
				.relative(dir, pagesDir || appDir || "")
				.startsWith("src");
			const hasPublicDir = (0, _fs.existsSync)(publicDir);
			telemetry.record(
				(0, _events.eventCliSession)(dir, config, {
					webpackVersion: 5,
					cliCommand: "build",
					isSrcDir,
			...
			(0, _events.eventNextPlugins)(_path.default.resolve(dir)).then((events) =>
				telemetry.record(events),
			);
			(0, _swcplugins.eventSwcPlugins)(_path.default.resolve(dir), config).then(
				(events) => telemetry.record(events),
			);
			// Always log next version first then start rest jobs
			const { envInfo, expFeatureInfo } = await (0,
			_appinfolog.getStartServerInfo)(dir, false);
			...
			const validFileMatcher = (0, _findpagefile.createValidFileMatcher)(
			...
			const pagesPaths =
				!appDirOnly && pagesDir
					? await nextBuildSpan.traceChild("collect-pages").traceAsyncFn(() =>
							(0, _recursivereaddir.recursiveReadDir)(pagesDir, {
								pathnameFilter: validFileMatcher.isPageFile,
							}),
						)
					: [];
			const middlewareDetectionRegExp = new RegExp(
				`^${_constants.MIDDLEWARE_FILENAME}\\.(?:${config.pageExtensions.join("|")})$`,
			);
			const instrumentationHookDetectionRegExp = new RegExp(
				`^${_constants.INSTRUMENTATION_HOOK_FILENAME}\\.(?:${config.pageExtensions.join("|")})$`,
			);
			...
			const rootPaths = (await (0, _getfilesindir.getFilesInDir)(rootDir))
				.filter((file) => includes.some((include) => include.test(file)))
				.sort((0, _entries.sortByPageExts)(config.pageExtensions))
				.map((file) => _path.default.join(rootDir, file).replace(dir, ""));
			const hasInstrumentationHook = rootPaths.some((p) =>
				p.includes(_constants.INSTRUMENTATION_HOOK_FILENAME),
			);
			const hasMiddlewareFile = rootPaths.some((p) =>
				p.includes(_constants.MIDDLEWARE_FILENAME),
			);
			_buildcontext.NextBuildContext.hasInstrumentationHook =
				hasInstrumentationHook;
			const previewProps = {
				...
				previewModeEncryptionKey: _crypto.default
					.randomBytes(32)
					.toString("hex"),
			};
			...
			if (appDir) {
				const appPaths = await nextBuildSpan
					...
					.traceAsyncFn(() =>
						(0, _recursivereaddir.recursiveReadDir)(appDir, {
							pathnameFilter: (absolutePath) =>
								validFileMatcher.isAppRouterPage(absolutePath) || // For now we only collect the root /not-found page in the app
				...
				for (const [pageKey, pagePath] of Object.entries(mappedAppPages)) {
					if (pageKey.includes("[[...__metadata_id__]]")) {
						...
						const isDynamic = await (0,
						_getpagestaticinfo.isDynamicMetadataRoute)(pageFilePath);
						if (!isDynamic) {
			...
			if (!IS_TURBOPACK_BUILD) {
				...
				if (mappedAppPages && numConflictingAppPaths > 0) {
					_log.error(
						`Conflicting app and page file${numConflictingAppPaths === 1 ? " was" : "s were"} found, please remove the conflicting files to continue:`,
					);
			...
			const hasPages404 =
				(_mappedPages_404 = mappedPages["/404"]) == null
					? void 0
					: _mappedPages_404.startsWith(_constants.PAGES_DIR_ALIAS);
			const hasApp404 = !!(mappedAppPages == null
				? void 0
				: mappedAppPages[_constants1.UNDERSCORE_NOT_FOUND_ROUTE_ENTRY]);
			const hasCustomErrorPage = mappedPages["/_error"].startsWith(
				_constants.PAGES_DIR_ALIAS,
			);
			if (hasPublicDir) {
				const hasPublicUnderScoreNextDir = (0, _fs.existsSync)(
					_path.default.join(publicDir, "_next"),
				);
			...
			await nextBuildSpan
				...
				.traceAsyncFn(async () => {
					...
					for (const page in mappedPages) {
						const hasPublicPageFile = await (0, _fileexists.fileExists)(
							_path.default.join(publicDir, page === "/" ? "/index" : page),
							_fileexists.FileType.File,
						);
					...
					if (numConflicting) {
						throw new Error(
							`Conflicting public and page file${numConflicting === 1 ? " was" : "s were"} found. https://nextjs.org/docs/messages/conflicting-public-file-page\n${conflictingPublicFiles.join("\n")}`,
						);
					}
				});
			const nestedReservedPages = pageKeys.pages.filter((page) => {
				return (
					page.match(/\/(_app|_document|_error)$/) &&
					_path.default.dirname(page) !== "/"
				);
			});
			...
			const routesManifestPath = _path.default.join(
				distDir,
				...
				.traceFn(() => {
					...
					return {
						...
						redirects: redirects.map((r) =>
							(0, _buildcustomroute.buildCustomRoute)(
								"redirect",
								r,
						...
						headers: headers.map((r) =>
							(0, _buildcustomroute.buildCustomRoute)("header", r),
						),
						dynamicRoutes,
						staticRoutes,
						dataRoutes: [],
			...
			if (rewrites.beforeFiles.length === 0 && rewrites.fallback.length === 0) {
				routesManifest.rewrites = rewrites.afterFiles.map((r) =>
					(0, _buildcustomroute.buildCustomRoute)("rewrite", r),
				);
			} else {
			...
			if (config.experimental.clientRouterFilter) {
				...
				_buildcontext.NextBuildContext.clientRouterFilters =
					clientRouterFilters;
			}
			...
			await writeFileUtf8(
				_path.default.join(distDir, "package.json"),
				'{"type": "commonjs"}',
			);
			...
			const pagesManifestPath = _path.default.join(
				distDir,
				_constants1.SERVER_DIRECTORY,
					...
					const serverFilesManifest = {
						...
						config: {
							...
							cacheHandler: cacheHandler
								? _path.default.relative(distDir, cacheHandler)
								: config.cacheHandler,
							experimental: {
								...config.experimental,
						...
						files: [
							...
							_path.default.join(
								_constants1.SERVER_DIRECTORY,
								_constants1.MIDDLEWARE_MANIFEST,
							),
							_path.default.join(
								_constants1.SERVER_DIRECTORY,
								_constants1.MIDDLEWARE_BUILD_MANIFEST + ".js",
							),
							_path.default.join(
								_constants1.SERVER_DIRECTORY,
								_constants1.MIDDLEWARE_REACT_LOADABLE_MANIFEST + ".js",
							),
							...(appDir
								? [
										...(config.experimental.sri
											? [
													_path.default.join(
														_constants1.SERVER_DIRECTORY,
														_constants1.SUBRESOURCE_INTEGRITY_MANIFEST + ".js",
													),
													_path.default.join(
														_constants1.SERVER_DIRECTORY,
														_constants1.SUBRESOURCE_INTEGRITY_MANIFEST +
															".json",
													),
												]
											: []),
										_path.default.join(
											_constants1.SERVER_DIRECTORY,
											_constants1.APP_PATHS_MANIFEST,
										),
										_path.default.join(_constants1.APP_PATH_ROUTES_MANIFEST),
										_constants1.APP_BUILD_MANIFEST,
										_path.default.join(
											_constants1.SERVER_DIRECTORY,
											_constants1.SERVER_REFERENCE_MANIFEST + ".js",
										),
										_path.default.join(
											_constants1.SERVER_DIRECTORY,
											_constants1.SERVER_REFERENCE_MANIFEST + ".json",
										),
									]
								: []),
							...
							config.optimizeFonts
								? _path.default.join(
										_constants1.SERVER_DIRECTORY,
										_constants1.AUTOMATIC_FONT_OPTIMIZATION_MANIFEST,
									)
								: null,
							_constants1.BUILD_ID_FILE,
							_path.default.join(
								_constants1.SERVER_DIRECTORY,
								_constants1.NEXT_FONT_MANIFEST + ".js",
							),
							_path.default.join(
								_constants1.SERVER_DIRECTORY,
								_constants1.NEXT_FONT_MANIFEST + ".json",
							),
							...(hasInstrumentationHook
								? [
										_path.default.join(
											_constants1.SERVER_DIRECTORY,
											`${_constants.INSTRUMENTATION_HOOK_FILENAME}.js`,
										),
										_path.default.join(
											_constants1.SERVER_DIRECTORY,
											`edge-${_constants.INSTRUMENTATION_HOOK_FILENAME}.js`,
										),
									]
								: []),
						]
							.filter(_nonnullable.nonNullable)
							.map((file) => _path.default.join(config.distDir, file)),
						ignore: [],
					};
			...
			async function turbopackBuild() {
				...
				const bindings = await (0, _swc.loadBindings)(
					config == null
						? void 0
						: (_config_experimental = config.experimental) == null
							? void 0
							: _config_experimental.useWasmBinary,
				);
				const dev = false;
				const project = await bindings.turbo.createProject({
					...
					nextConfig: config,
					jsConfig: await (0, _turbopackutils.getTurbopackJsConfig)(
						dir,
						config,
					),
					watch: false,
				...
				await _fs.promises.mkdir(
					_path.default.join(distDir, "static", buildId),
					{
						recursive: true,
					},
				);
				...
				entrypointsSubscription.return == null
					? void 0
					: entrypointsSubscription.return
							.call(entrypointsSubscription)
							.catch(() => {});
				...
				if (topLevelErrors.length > 0) {
					throw new Error(
						`Turbopack build failed with ${topLevelErrors.length} issues:\n${topLevelErrors.map((e) => e.message).join("\n")}`,
					);
				}
				...
				const progress = (0, _progress.createProgress)(
				...
				for (const [page, route] of currentEntrypoints.page) {
					enqueue(() =>
						(0, _turbopackutils.handleRouteType)({
							dev,
							page,
							pathname: page,
							route,
				...
				enqueue(() =>
					(0, _turbopackutils.handlePagesErrorRoute)({
						currentEntryIssues,
						entrypoints: currentEntrypoints,
						manifestLoader,
						rewrites: emptyRewritesObjToBeImplemented,
			...
			if (!isGenerateMode) {
				if (runServerAndEdgeInParallel || collectServerBuildTracesInParallel) {
					...
					const serverBuildPromise = (0, _webpackbuild.webpackBuild)(
						useBuildWorker,
						["server"],
					).then((res) => {
						(0, _trace1.traceMemoryUsage)(
							"Finished server compilation",
							nextBuildSpan,
						);
						buildTraceContext = res.buildTraceContext;
						durationInSeconds += res.duration;
						if (collectServerBuildTracesInParallel) {
							const buildTraceWorker = new _worker.Worker(
					...
					const edgeBuildPromise = (0, _webpackbuild.webpackBuild)(
						useBuildWorker,
						["edge-server"],
					).then((res) => {
						durationInSeconds += res.duration;
					...
					telemetry.record(
						(0, _events.eventBuildCompleted)(pagesPaths, {
							durationInSeconds,
							totalAppPagesCount,
						}),
					);
					...
					const { duration: compilerDuration, ...rest } = turboNextBuild
						...
						: await (0, _webpackbuild.webpackBuild)(useBuildWorker, null);
			...
			const buildManifestPath = _path.default.join(
				distDir,
				_constants1.BUILD_MANIFEST,
			);
			const appBuildManifestPath = _path.default.join(
				distDir,
				_constants1.APP_BUILD_MANIFEST,
			);
			...
			if (appDir) {
				...
				await writeManifest(
					_path.default.join(distDir, _constants1.APP_PATH_ROUTES_MANIFEST),
					appPathRoutes,
				);
			}
			...
			if (config.experimental.staticWorkerRequestDeduping) {
				...
				const cacheInitialization = await (0,
				_incrementalcacheserver.initialize)({
			...
			const {
					...
					async () => {
						...
						return (
							...
							pagesStaticWorkers.isPageStatic({
								...
								locales:
									(_config_i18n = config.i18n) == null
										? void 0
										: _config_i18n.locales,
								defaultLocale:
									(_config_i18n1 = config.i18n) == null
										? void 0
										: _config_i18n1.defaultLocale,
				...
				for (const key of Object.keys(
					middlewareManifest == null ? void 0 : middlewareManifest.functions,
				)) {
					...
					Object.entries(pageKeys)
						...
						.map(({ pageType, page }) => {
							...
							return checkPageSpan.traceAsyncFn(async () => {
								const actualPage = (0, _normalizepagepath.normalizePagePath)(
									page,
								);
								const [size, totalSize] = await (0, _utils1.getJsPageSizeInKb)(
									pageType,
									actualPage,
									distDir,
									buildManifest,
								...
								const pageFilePath = (0, _utils1.isAppBuiltinNotFoundPage)(
									pagePath,
								)
									? require.resolve(
								...
								const staticInfo = pagePath
									? await (0, _getpagestaticinfo.getPageStaticInfo)({
											pageFilePath,
											nextConfig: config,
										...
										try {
											...
											let workerResult = await isPageStaticSpan.traceAsyncFn(
												() => {
													...
													return (
														...
														isrFlushToDisk: _ciinfo.hasNextSupport
															? false
															: config.experimental.isrFlushToDisk,
														maxMemoryCacheSize: config.cacheMaxMemorySize,
														nextConfigOutput: config.output,
											...
											if (pageType === "app" && originalAppPath) {
												...
												if ((0, _isedgeruntime.isEdgeRuntime)(pageRuntime)) {
													...
													const isInterceptionRoute = (0,
													_interceptionroutes.isInterceptionRouteAppPath)(page);
													if (appConfig.revalidate !== 0) {
														var _workerResult_prerenderRoutes;
														const isDynamic = (0, _utils.isDynamicRoute)(page);
														const hasGenerateStaticParams =
															!!((_workerResult_prerenderRoutes =
																workerResult.prerenderRoutes) == null
																? void 0
																: _workerResult_prerenderRoutes.length);
														if (
													...
													if (
														!isStatic &&
														!(0, _isapprouteroute.isAppRouteRoute)(
															originalAppPath,
														) &&
														!(0, _utils.isDynamicRoute)(originalAppPath) &&
														!isPPR &&
														!isInterceptionRoute
													) {
												...
												if (hasPages404 && page === "/404") {
													if (
														...
														throw new Error(
															`\`pages/404\` ${_constants.STATIC_STATUS_PAGE_GET_INITIAL_PROPS_ERROR}`,
														);
													}
												...
												if (
													_constants1.STATIC_STATUS_PAGES.includes(page) &&
													!workerResult.isStatic &&
													!workerResult.hasStaticProps
												) {
													throw new Error(
														`\`pages${page}\` ${_constants.STATIC_STATUS_PAGE_GET_INITIAL_PROPS_ERROR}`,
													);
												}
											}
										} catch (err) {
			...
			if (customAppGetInitialProps) {
				console.warn(
					(0, _picocolors.bold)((0, _picocolors.yellow)(`Warning: `)) +
			...
			if (invalidPages.size > 0) {
				const err = new Error(
					`Build optimization failed: found page${invalidPages.size === 1 ? "" : "s"} without a React Component as default export in \n${[
						...invalidPages,
					]
						.map((pg) => `pages${pg}`)
						.join(
			...
			if (config.experimental.optimizeCss) {
				...
				requiredServerFilesManifest.files.push(
					...cssFilePaths.map((filePath) =>
						_path.default.join(config.distDir, "static", filePath),
					),
				);
			}
			const features = [
			...
			const middlewareManifest = await readManifest(
				_path.default.join(
					distDir,
					_constants1.SERVER_DIRECTORY,
					_constants1.MIDDLEWARE_MANIFEST,
			...
			const { i18n } = config;
			const usedStaticStatusPages = _constants1.STATIC_STATUS_PAGES.filter(
				(page) =>
					mappedPages[page] &&
			...
			const isApp404Static = appStaticPaths.has(
				_constants1.UNDERSCORE_NOT_FOUND_ROUTE_ENTRY,
			);
			...
			if (
				...
				await staticGenerationSpan.traceAsyncFn(async () => {
								...
								for (const page of [
									...
									for (const locale of i18n.locales) {
										...
										defaultMap[outputPath] = {
											page:
												((_defaultMap_page = defaultMap[page]) == null
													? void 0
													: _defaultMap_page.page) || page,
											query: {
												__nextLocale: locale,
					...
					const exportOptions = {
						...
						// exposed methods.
						exportAppPageWorker:
							appStaticWorkers == null ? void 0 : appStaticWorkers.exportPage,
						exportPageWorker:
							pagesStaticWorkers == null
								? void 0
								: pagesStaticWorkers.exportPage,
						endWorker: async () => {
							await pagesStaticWorkers.end();
							await (appStaticWorkers == null
								? void 0
					...
					for (const page of staticPages) {
						const serverBundle = (0, _require.getPagePath)(
							page,
							distDir,
							undefined,
							false,
					...
					for (const [originalAppPath, routes] of appStaticPaths) {
						...
						let hasDynamicData =
							...
							((_exportResult_byPath_get = exportResult.byPath.get(page)) ==
							null
								? void 0
								: _exportResult_byPath_get.revalidate) === 0;
						if (
							hasDynamicData &&
							((_pageInfos_get = pageInfos.get(page)) == null
								? void 0
								: _pageInfos_get.isStatic)
						) {
						...
						const isRouteHandler = (0, _isapprouteroute.isAppRouteRoute)(
							originalAppPath,
						);
						...
						(0, _utils.getSortedRoutes)(routes).forEach((route) => {
							...
							if (revalidate !== 0) {
								...
								_normalizepagepath.normalizePagePath)(route);
								let dataRoute;
								if (isRouteHandler) {
									dataRoute = null;
								} else {
									dataRoute = _path.default.posix.join(
										`${normalizedRoute}${_constants.RSC_SUFFIX}`,
									);
								}
								let prefetchDataRoute;
								if (experimentalPPR) {
									prefetchDataRoute = _path.default.posix.join(
										`${normalizedRoute}${_constants.RSC_PREFETCH_SUFFIX}`,
									);
								}
								const routeMeta = {};
						...
						if (
							...
							const normalizedRoute = (0, _normalizepagepath.normalizePagePath)(
								page,
							);
							...
							finalDynamicRoutes[page] = {
								experimentalPPR,
								experimentalBypassFor: bypassFor,
								routeRegex: (0, _loadcustomroutes.normalizeRouteRegex)(
									(0, _routeregex.getNamedRouteRegex)(page, false).re.source,
								),
								dataRoute,
								...
								dataRouteRegex: isRouteHandler
									? null
									: (0, _loadcustomroutes.normalizeRouteRegex)(
											(0, _routeregex.getNamedRouteRegex)(
					...
					const moveExportedPage = async (
						...
						return staticGenerationSpan
							...
							.traceAsyncFn(async () => {
								...
								const relativeDest = _path.default
									.relative(
										_path.default.join(distDir, _constants1.SERVER_DIRECTORY),
										_path.default.join(
											_path.default.join(
								...
								if (i18n) {
									...
									const localeExt =
										page === "/" ? _path.default.extname(file) : "";
									...
									for (const locale of i18n.locales) {
										...
										const updatedRelativeDest = _path.default
											.join(
												"pages",
												locale + localeExt, // if it's the top-most index page we want it to be locale.EXT
												// instead of locale/index.html
												page === "/" ? "" : relativeDestNoPages,
											)
											.replace(/\\/g, "/");
										const updatedOrig = _path.default.join(
											exportOptions.outdir,
											locale + localeExt,
											page === "/" ? "" : file,
										);
										const updatedDest = _path.default.join(
											distDir,
											_constants1.SERVER_DIRECTORY,
											updatedRelativeDest,
										);
										...
										await _fs.promises.mkdir(
											_path.default.dirname(updatedDest),
											{
												recursive: true,
											},
										);
						...
						return staticGenerationSpan
							...
							.traceAsyncFn(async () => {
								...
								if ((0, _fs.existsSync)(orig)) {
									await _fs.promises.copyFile(
										orig,
										_path.default.join(distDir, "server", updatedRelativeDest),
									);
							...
							if (!isDynamic) {
								...
								if (i18n) {
									...
									for (const locale of i18n.locales) {
										...
										finalPrerenderRoutes[localePage] = {
											initialRevalidateSeconds:
												((_exportResult_byPath_get1 =
													exportResult.byPath.get(localePage)) == null
													? void 0
													: _exportResult_byPath_get1.revalidate) ?? false,
									...
									finalPrerenderRoutes[page] = {
										initialRevalidateSeconds:
											((_exportResult_byPath_get2 =
												exportResult.byPath.get(page)) == null
												? void 0
												: _exportResult_byPath_get2.revalidate) ?? false,
										experimentalPPR: undefined,
										srcRoute: null,
										dataRoute: _path.default.posix.join(
											"/_next/data",
								...
								if (pageInfo) {
									...
									pageInfo.initialRevalidateSeconds =
										((_exportResult_byPath_get3 =
											exportResult.byPath.get(page)) == null
											? void 0
											: _exportResult_byPath_get3.revalidate) ?? false;
								}
							} else {
								...
								for (const route of extraRoutes) {
									...
									const pageFile = (0, _normalizepagepath.normalizePagePath)(
										route,
									);
									...
									const initialRevalidateSeconds =
										((_exportResult_byPath_get4 =
											exportResult.byPath.get(route)) == null
											? void 0
											: _exportResult_byPath_get4.revalidate) ?? false;
			...
			telemetry.record(
				(0, _events.eventBuildOptimize)(pagesPaths, {
			...
			if (_buildcontext.NextBuildContext.telemetryState) {
				const events = (0, _events.eventBuildFeatureUsage)(
					_buildcontext.NextBuildContext.telemetryState.usages,
				);
				telemetry.record(events);
				telemetry.record(
					(0, _events.eventPackageUsedInGetServerSideProps)(
			...
			if (ssgPages.size > 0 || appDir) {
				...
				tbdPrerenderRoutes.forEach((tbdRoute) => {
					...
					finalDynamicRoutes[tbdRoute] = {
						routeRegex: (0, _loadcustomroutes.normalizeRouteRegex)(
							(0, _routeregex.getNamedRouteRegex)(tbdRoute, false).re.source,
						),
						experimentalPPR: undefined,
						dataRoute,
						fallback: ssgBlockingFallbackPages.has(tbdRoute)
							...
							: ssgStaticFallbackPages.has(tbdRoute)
								...
								: false,
						dataRouteRegex: (0, _loadcustomroutes.normalizeRouteRegex)(
							(0, _routeregex.getNamedRouteRegex)(
				...
				_buildcontext.NextBuildContext.previewModeId =
					previewProps.previewModeId;
				_buildcontext.NextBuildContext.fetchCacheKeyPrefix =
					config.experimental.fetchCacheKeyPrefix;
				_buildcontext.NextBuildContext.allowedRevalidateHeaderKeys =
					config.experimental.allowedRevalidateHeaderKeys;
			...
			await writeManifest(
				_path.default.join(distDir, _constants1.EXPORT_MARKER),
				{
					version: 1,
			...
			await _fs.promises
				.unlink(_path.default.join(distDir, _constants1.EXPORT_DETAIL))
				.catch((err) => {
					if (err.code === "ENOENT") {
			...
			if (Boolean(config.experimental.nextScriptWorkers)) {
				await nextBuildSpan
					...
					.traceAsyncFn(async () => {
						await (0, _verifypartytownsetup.verifyPartytownSetup)(
							dir,
							_path.default.join(distDir, _constants1.CLIENT_STATIC_FILES_PATH),
						);
					});
			}
			...
			console.log();
			if (debugOutput) {
				nextBuildSpan.traceChild("print-custom-routes").traceFn(() =>
					(0, _utils1.printCustomRoutes)({
						redirects,
						rewrites,
						headers,
					}),
			...
			await nextBuildSpan.traceChild("print-tree-view").traceAsyncFn(() =>
				(0, _utils1.printTreeView)(pageKeys, pageInfos, {
					distPath: distDir,
					buildId: buildId,
					pagesDir,
					useStaticPages404,
