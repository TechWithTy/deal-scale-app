...
function transformSource() {
	...
	const code = modules
		.map((x) => JSON.parse(x)) // Filter out CSS files in the SSR compilation
		.filter(({ request }) => (isServer ? !_utils.regexCSS.test(request) : true))
		.map(({ request, ids }) => {
			const importPath = JSON.stringify(
				request.startsWith(_constants.BARREL_OPTIMIZATION_PREFIX)
					? request.replace(":", "!=!")
					: request,
			);
