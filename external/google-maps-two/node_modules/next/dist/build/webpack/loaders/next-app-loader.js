...
async function createAppRouteCode({
	...
	if ((0, _ismetadataroute.isMetadataRoute)(name) && filename !== "route") {
		const { ext } = (0, _nextmetadatarouteloader.getFilenameAndExtension)(
			resolvedPagePath,
		);
		const isDynamic = pageExtensions.includes(ext);
		resolvedPagePath = `next-metadata-route-loader?${(0,
	...
	const pathname = new _apppathnamenormalizer.AppPathnameNormalizer().normalize(
		page,
	);
	const bundlePath =
		new _appbundlepathnormalizer.AppBundlePathNormalizer().normalize(page);
	return await (0, _loadentrypoint.loadEntrypoint)(
		"app-route",
		{
...
const normalizeParallelKey = (key) =>
...
async function createTreeCodeFromPath(
	...
	const appDirPrefix = isDefaultNotFound
		? _constants1.APP_DIR_ALIAS
		: splittedPath[0];
	...
	async function createSubtreePropsFromSegmentPath(
		...
		if (resolvedRouteDir) {
			metadata = await (0, _discover.createStaticMetadataFromRoute)(
				resolvedRouteDir,
				{
					basePath,
					segment: segmentPath,
		...
		for (const [parallelKey, parallelSegment] of parallelSegments) {
          ...
          ${(0, _discover.createMetadataExportsCode)(metadata)}
			...
			const isFirstLayerGroupRoute =
				segments.length === 1 &&
				subSegmentPath.filter((seg) => (0, _segment.isGroupSegment)(seg))
					.length === 1;
			...
			if (!rootLayout) {
				...
				const layoutPath =
					(_definedFilePaths_find = definedFilePaths.find(
						([type]) => type === "layout",
					)) == null
						? void 0
						: _definedFilePaths_find[1];
			...
			if (isNotFoundRoute && normalizedParallelKey === "children") {
				...
				const notFoundPath =
					((_definedFilePaths_find1 = definedFilePaths.find(
						([type]) => type === "not-found",
					)) == null
						? void 0
						: _definedFilePaths_find1[1]) ?? defaultNotFoundPath;
		...
		for (const adjacentParallelSegment of adjacentParallelSegments) {
			if (!props[normalizeParallelKey(adjacentParallelSegment)]) {
				...
				const defaultPath =
					...
					)) ?? _parallelroutedefault.PARALLEL_ROUTE_DEFAULT_PATH;
...
const nextAppLoader = async function nextAppLoader() {
	...
	const middlewareConfig = JSON.parse(
	...
	if (!treeCodeResult.rootLayout) {
		if (!isDev) {
			...
			_log.error(
				`${(0, _picocolors.bold)(pagePath.replace(`${_constants1.APP_DIR_ALIAS}/`, ""))} doesn't have a root layout. To fix this error, make sure every page has a root layout.`,
			);
			process.exit(1);
		} else {
			// In dev we'll try to create a root layout
			const [createdRootLayout, rootLayoutPath] = await (0,
	...
	const code = await (0, _loadentrypoint.loadEntrypoint)(
		"app-page",
		{
			VAR_DEFINITION_PAGE: page,
			VAR_DEFINITION_PATHNAME: pathname,
