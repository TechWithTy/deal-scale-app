...
async function nextMetadataImageLoader(content) {
	...
	const contentHash =
		type === "favicon"
			? ""
			: _loaderutils3.default.interpolateName(this, "[contenthash]", opts);
	const interpolatedName = _loaderutils3.default.interpolateName(
		this,
		"[name].[ext]",
		opts,
	);
	...
	const pathnamePrefix = (0, _normalizepathsep.normalizePathSep)(
		_path.default.join(basePath, segment),
	);
	if (isDynamicResource) {
		const exportedFieldsExcludingDefault = (
			await (0, _utils.getLoaderModuleNamedExports)(resourcePath, this)
	...
	const imageSize = await (0, _imageoptimizer.getImageSize)(
		content,
		extension,
	).catch((err) => err);
	...
	if (type === "openGraph" || type === "twitter") {
		const altPath = _path.default.join(
