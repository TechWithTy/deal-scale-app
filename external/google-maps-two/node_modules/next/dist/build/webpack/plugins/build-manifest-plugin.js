...
const _getroutefromentrypoint = /*#__PURE__*/ _interop_require_default(
	require("../../../server/get-route-from-entrypoint"),
);
...
function normalizeRewritesForBuildManifest(rewrites) {
	...
	return {
		afterFiles:
			(_rewrites_afterFiles = rewrites.afterFiles) == null
				? void 0
				: _rewrites_afterFiles.map((item) => normalizeRewrite(item)),
		beforeFiles:
			(_rewrites_beforeFiles = rewrites.beforeFiles) == null
				? void 0
				: _rewrites_beforeFiles.map((item) => normalizeRewrite(item)),
		fallback:
			(_rewrites_fallback = rewrites.fallback) == null
				? void 0
				: _rewrites_fallback.map((item) => normalizeRewrite(item)),
	};
}
...
function generateClientManifest(compiler, compilation, assetMap, rewrites) {
	...
	const genClientManifestSpan =
		compilationSpan == null
			? void 0
			: compilationSpan.traceChild(
					"NextJsBuildManifest-generateClientManifest",
				);
	return genClientManifestSpan == null
		? void 0
		: genClientManifestSpan.traceFn(() => {
				const clientManifest = {
					__rewrites: normalizeRewritesForBuildManifest(rewrites),
				};
				const appDependencies = new Set(assetMap.pages["/_app"]);
				const sortedPageKeys = (0, _utils.getSortedRoutes)(
					Object.keys(assetMap.pages),
				);
...
class BuildManifestPlugin {
	...
	createAssets(compiler, compilation, assets) {
		...
		const createAssetsSpan =
			compilationSpan == null
				? void 0
				: compilationSpan.traceChild("NextJsBuildManifest-createassets");
		return createAssetsSpan == null
			? void 0
			: createAssetsSpan.traceFn(() => {
					...
					const ampFirstEntryNames =
						_nextdropclientpageplugin.ampFirstEntryNamesMap.get(compilation);
					...
					const mainFiles = new Set(
						getEntrypointFiles(
							entrypoints.get(_constants.CLIENT_STATIC_FILES_RUNTIME_MAIN),
						),
					);
					...
					assetMap.polyfillFiles = compilationAssets
						.filter((p) => {
							...
							return (
								p.info &&
								_constants.CLIENT_STATIC_FILES_RUNTIME_POLYFILLS_SYMBOL in
									p.info
							);
						})
						.map((v) => v.name);
					...
					assetMap.ampDevFiles = getEntrypointFiles(
						entrypoints.get(_constants.CLIENT_STATIC_FILES_RUNTIME_AMP),
					);
					for (const entrypoint of compilation.entrypoints.values()) {
						if (_constants.SYSTEM_ENTRYPOINTS.has(entrypoint.name)) continue;
						const pagePath = (0, _getroutefromentrypoint.default)(
							entrypoint.name,
						);
						if (!pagePath) {
							continue;
					...
					if (!this.isDevFallback) {
						...
						assets[ssgManifestPath] = new _webpack.sources.RawSource(
							srcEmptySsgManifest,
						);
					}
					...
					assets[buildManifestName] = new _webpack.sources.RawSource(
						JSON.stringify(assetMap, null, 2),
					);
					assets[`server/${_constants.MIDDLEWARE_BUILD_MANIFEST}.js`] =
					...
					if (!this.isDevFallback) {
						...
						assets[clientManifestPath] = new _webpack.sources.RawSource(
							`self.__BUILD_MANIFEST = ${generateClientManifest(compiler, compilation, assetMap, this.rewrites)};self.__BUILD_MANIFEST_CB && self.__BUILD_MANIFEST_CB()`,
						);
					}
					return assets;
				});
