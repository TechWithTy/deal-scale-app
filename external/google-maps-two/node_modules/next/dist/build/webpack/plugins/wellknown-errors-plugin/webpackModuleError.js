...
function getFileData(compilation, m) {
	...
	let resolved;
	let ctx =
		((_compilation_compiler = compilation.compiler) == null
			? void 0
			: _compilation_compiler.context) ?? null;
	...
	if (resolved) {
		...
		try {
			content = (0, _fs.readFileSync)(
				ctx ? _path.resolve(ctx, resolved) : resolved,
				"utf8",
			);
		} catch {}
...
async function getModuleBuildError(compiler, compilation, input) {
	if (
		!(
			...
			(0, _iserror.default)(input.error)
		)
	) {
		return false;
	}
	...
	const imageError = await (0, _parseNotFoundError.getImageError)(
		compilation,
		input,
		err,
	);
	...
	const rsc = (0, _parseRSC.getRscError)(
		sourceFilename,
		err,
		input.module,
		compilation,
	...
	const nextAppLoader = (0, _parseNextAppLoaderError.getNextAppLoaderError)(
		err,
		input.module,
		compiler,
	);
	...
	const invalidImportError = (0,
	_parseNextInvalidImportError.getNextInvalidImportError)(
		err,
		input.module,
		compilation,
		compiler,
