...
function getEntryFiles(entryFiles, meta, hasInstrumentationHook, opts) {
	...
	if (meta.edgeSSR) {
				...
				...entryFiles
					...
					.map(
						(file) =>
							...
							file.replace(
								".js",
								"_" + _constants.CLIENT_REFERENCE_MANIFEST + ".js",
							),
					),
			);
		}
		files.push(
			...
			`server/${_constants.NEXT_FONT_MANIFEST}.js`,
			`server/${_constants.INTERCEPTION_ROUTE_REWRITE_MANIFEST}.js`,
		);
	}
	...
	return (assets) => {
		...
		for (const entrypoint of compilation.entrypoints.values()) {
			...
			const page =
				(metadata == null
					? void 0
					: (_metadata_edgeMiddleware = metadata.edgeMiddleware) == null
						? void 0
						: _metadata_edgeMiddleware.page) ||
				(metadata == null
					? void 0
					: (_metadata_edgeSSR = metadata.edgeSSR) == null
						? void 0
						: _metadata_edgeSSR.page) ||
				(metadata == null
					? void 0
					: (_metadata_edgeApiFunction = metadata.edgeApiFunction) == null
						? void 0
						: _metadata_edgeApiFunction.page);
			if (!page) {
				continue;
			}
			const matcherSource = (
				(_metadata_edgeSSR1 = metadata.edgeSSR) == null
					? void 0
					: _metadata_edgeSSR1.isAppDir
			)
				? (0, _apppaths.normalizeAppPath)(page)
				: page;
			const catchAll = !metadata.edgeSSR && !metadata.edgeApiFunction;
			const { namedRegex } = (0, _routeregex.getNamedMiddlewareRegex)(
				matcherSource,
				{
					catchAll,
				},
			);
			const matchers = (metadata == null
				? void 0
				: (_metadata_edgeMiddleware1 = metadata.edgeMiddleware) == null
					? void 0
					: _metadata_edgeMiddleware1.matchers) ?? [
				{
					regexp: namedRegex,
		...
		middlewareManifest.sortedMiddleware = (0, _utils.getSortedRoutes)(
			Object.keys(middlewareManifest.middleware),
		);
		assets[_constants.MIDDLEWARE_MANIFEST] = new _webpack.sources.RawSource(
...
function buildWebpackError({ message, loc, compilation, entryModule, parser }) {
	...
	error.name = NAME;
	const module1 =
		entryModule ?? (parser == null ? void 0 : parser.state.current);
	if (module1) {
...
function isInMiddlewareLayer(parser) {
	...
	return (
		((_parser_state_module = parser.state.module) == null
			? void 0
			: _parser_state_module.layer) === "middleware"
	);
}
...
function isDynamicCodeEvaluationAllowed(fileName, middlewareConfig, rootDir) {
	...
	if (
		KNOWN_SAFE_DYNAMIC_PACKAGES.some((pkg) =>
			fileName.includes(
				`/node_modules/${pkg}/`.replace(/\//g, _path.default.sep),
			),
		)
	) {
		return true;
	...
	return (0, _picomatch.default)(
		(middlewareConfig == null
			? void 0
			: middlewareConfig.unstable_allowDynamicGlobs) ?? [],
		{
			dot: true,
		},
	)(name);
	...
	return (parser) => {
		 ...
		 */ const handleExpression = () => {
			...
			wp.optimize.InnerGraph.onUsage(parser.state, (used = true) => {
				const buildInfo = (0, _getmodulebuildinfo.getModuleBuildInfo)(
					parser.state.module,
				);
		 ...
		 */ const handleImport = (node) => {
			var _node_source;
			if (
				isInMiddlewareLayer(parser) &&
				((_node_source = node.source) == null ? void 0 : _node_source.value) &&
				(node == null ? void 0 : node.loc)
			) {
				...
				const importedModule =
					(_node_source_value = node.source.value) == null
						? void 0
						: _node_source_value.toString();
	...
	return async () => {
		...
		for (const [entryName, entry] of compilation.entries) {
			...
			const entryDependency =
				(_entry_dependencies = entry.dependencies) == null
					? void 0
					: _entry_dependencies[0];
			const resolvedModule =
				compilation.moduleGraph.getResolvedModule(entryDependency);
			if (!resolvedModule) {
				continue;
			}
			const { rootDir, route } = (0, _getmodulebuildinfo.getModuleBuildInfo)(
				resolvedModule,
			);
			...
			if (
				route == null
					? void 0
					: (_route_middlewareConfig = route.middlewareConfig) == null
						? void 0
						: _route_middlewareConfig.regions
			) {
			...
			for (const module1 of modules) {
				 ...
				 */ if (
					...
					if (
						route == null
							? void 0
							: (_route_middlewareConfig1 = route.middlewareConfig) == null
								? void 0
								: _route_middlewareConfig1.unstable_allowDynamicGlobs
					) {
						telemetry == null
							? void 0
							: telemetry.record({
									...
									payload: {
										file:
											route == null
												? void 0
												: route.absolutePagePath.replace(rootDir ?? "", ""),
										config: route == null ? void 0 : route.middlewareConfig,
										fileWithDynamicCode: module1.userRequest.replace(
											rootDir ?? "",
											"",
										),
									},
					...
					if (
						!isDynamicCodeEvaluationAllowed(
							module1.userRequest,
							route == null ? void 0 : route.middlewareConfig,
							rootDir,
						)
					) {
						...
						compilation.errors.push(
							(0,
							_parsedynamiccodeevaluationerror.getDynamicCodeEvaluationError)(
								message,
								module1,
								compilation,
								compiler,
				 ...
				 */ if (
					buildInfo == null ? void 0 : buildInfo.nextWasmMiddlewareBinding
				) {
				 ...
				 */ for (const conn of (0, _utils1.getModuleReferencesInOrder)(
					module1,
					moduleGraph,
				)) {
					if (conn.module) {
