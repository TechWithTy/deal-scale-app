...
const PACKAGE_ROOT = _path.default.normalize(
	_path.default.join(__dirname, "../../.."),
);
const TEMPLATE_FOLDER = _path.default.join(__dirname, "templates");
const TEMPLATES_ESM_FOLDER = _path.default.normalize(
	_path.default.join(__dirname, "../../dist/esm/build/templates"),
);
async function loadEntrypoint(entrypoint, replacements, injections, imports) {
	const filepath = _path.default.resolve(
		_path.default.join(TEMPLATES_ESM_FOLDER, `${entrypoint}.js`),
	);
	...
	file = file.replaceAll(
		...
		function (_, fromRequest, importRequest) {
			...
			const relative = _path.default
				.relative(
					PACKAGE_ROOT,
					_path.default.resolve(TEMPLATE_FOLDER, fromRequest ?? importRequest),
	...
	if (replaced.size !== Object.keys(replacements).length) {
		...
		throw new Error(
			`Invariant: Expected to replace all template variables, missing ${difference.join(", ")} in template`,
		);
	}
	...
	if (injected.size !== Object.keys(injections ?? {}).length) {
		...
		throw new Error(
			`Invariant: Expected to inject all injections, missing ${difference.join(", ")} in template`,
		);
	}
	...
	if (importsAdded.size !== Object.keys(imports ?? {}).length) {
		...
		throw new Error(
			`Invariant: Expected to inject all imports, missing ${difference.join(", ")} in template`,
		);
	}
	return file;
}
