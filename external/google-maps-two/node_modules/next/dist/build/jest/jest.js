...
/** Loads dotenv files and sets environment variables based on next config. */ function setUpEnv(
	dir,
) {
	const dev = false;
	...
	return (customJestConfig) => {
		...
		return async () => {
			...
			if (options.dir) {
				...
				const result = await (0, _loadjsconfig.default)(
					resolvedDir,
					nextConfig,
				);
			...
			const transpiled = (
				(nextConfig == null ? void 0 : nextConfig.transpilePackages) ?? []
			).join("|");
