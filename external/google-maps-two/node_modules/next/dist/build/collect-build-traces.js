...
const _picomatch = /*#__PURE__*/ _interop_require_default(
...
async function collectBuildTraces({
	dir,
	config,
	distDir,
	pageInfos,
	staticPages,
	nextBuildSpan = new _trace.Span({
		name: "build",
	}),
	hasSsrAmpPages,
	buildTraceContext,
	...
	const runTurbotrace = async function () {
		...
		if (
			!(bindings == null ? void 0 : bindings.isWasm) &&
			typeof bindings.turbo.startTrace === "function"
		) {
			var _config_experimental_turbotrace;
			let turbotraceOutputPath;
			let turbotraceFiles;
			turboTasksForTrace = bindings.turbo.createTurboTasks(
				(((_config_experimental_turbotrace = config.experimental.turbotrace) ==
				null
					? void 0
					: _config_experimental_turbotrace.memoryLimit) ??
					_constants.TURBO_TRACE_DEFAULT_MEMORY_LIMIT) *
					1024 *
					1024,
			);
			...
			if (entriesTrace) {
				...
				const filesTracedInEntries = await bindings.turbo.startTrace(
					action,
					turboTasksForTrace,
				);
				...
				const filesTracedFromEntries = filesTracedInEntries
					.map((f) => _path.default.join(contextDirectory, f))
					.filter(
						(f) =>
				...
				if (filesTracedFromEntries.length) {
					...
					const traceOutputPath = _path.default.join(
						outputPath,
					...
					turbotraceFiles = filesTracedFromEntries.map((file) =>
						_path.default.relative(traceOutputDir, file),
					);
				}
			}
			if (chunksTrace) {
				...
				if (turbotraceOutputPath && turbotraceFiles) {
					const existedNftFile = await _promises.default
	...
	await nextBuildSpan
		...
		.traceAsyncFn(async () => {
			...
			const nextServerTraceOutput = _path.default.join(
				distDir,
				"next-server.js.nft.json",
			);
			const nextMinimalTraceOutput = _path.default.join(
				distDir,
			...
			const root =
				((_config_experimental = config.experimental) == null
					...
					: (_config_experimental_turbotrace =
								_config_experimental.turbotrace) == null
						? void 0
						: _config_experimental_turbotrace.contextDirectory) ??
				outputFileTracingRoot;
			...
			const sharedEntriesSet = [
				...(config.experimental.turbotrace
					? []
					: Object.keys(_requirehook.defaultOverrides).map((value) =>
			...
			if (cacheHandler) {
				sharedEntriesSet.push(
					require.resolve(
						_path.default.isAbsolute(cacheHandler)
							? cacheHandler
							: _path.default.join(dir, cacheHandler),
					),
				);
			}
			const serverEntries = [
			...
			const makeIgnoreFn = (ignores) => {
				...
				return (pathname) => {
					if (
						_path.default.isAbsolute(pathname) &&
			...
			function addToTracedFiles(base, file, dest) {
				dest.add(
					_path.default
						.relative(distDir, _path.default.join(base, file))
						.replace(/\\/g, "/"),
				);
			}
			if (isStandalone) {
			...
			if (config.experimental.turbotrace) {
				...
				const makeTrace = async (entries) => {
					var _config_experimental_turbotrace,
						_config_experimental_turbotrace1,
						_config_experimental_turbotrace2,
						_config_experimental_turbotrace3;
					return startTrace(
						{
							action: "print",
							input: entries,
							...
							logLevel:
								(_config_experimental_turbotrace =
									config.experimental.turbotrace) == null
									? void 0
									: _config_experimental_turbotrace.logLevel,
							processCwd:
								(_config_experimental_turbotrace1 =
									config.experimental.turbotrace) == null
									? void 0
									: _config_experimental_turbotrace1.processCwd,
							logDetail:
								(_config_experimental_turbotrace2 =
									config.experimental.turbotrace) == null
									? void 0
									: _config_experimental_turbotrace2.logDetail,
							showAll:
								(_config_experimental_turbotrace3 =
									config.experimental.turbotrace) == null
									? void 0
									: _config_experimental_turbotrace3.logAll,
						},
						turboTasksForTrace,
					);
				};
						...
						if (
							!(
								set === minimalServerTracedFiles
									...
									: serverIgnoreFn
							)(_path.default.join(traceContext, file))
						) {
				...
				const chunksToTrace = [
					...((buildTraceContext == null
						? void 0
						: (_buildTraceContext_chunksTrace =
									buildTraceContext.chunksTrace) == null
							? void 0
							: _buildTraceContext_chunksTrace.action.input) || []),
					...serverEntries,
				...
				const result = await (0, _nft.nodeFileTrace)(chunksToTrace, {
					...
					ignore(p) {
						...
						if (
							...
							!chunksToTrace.includes(
								_path.default.join(outputFileTracingRoot, p),
							)
						) {
							return true;
						}
				...
				const parentFilesMap = (0,
				_nexttraceentrypointsplugin.getFilesMapFromReasons)(fileList, reasons);
				...
				for (const [entries, tracedFiles] of [
					...
					for (const file of entries) {
						const curFiles = parentFilesMap.get(
							_path.default.relative(outputFileTracingRoot, file),
						);
						tracedFiles.add(
							_path.default.relative(distDir, file).replace(/\\/g, "/"),
						);
						for (const curFile of curFiles || []) {
							...
							if (
								...
								tracedFiles.add(
									_path.default.relative(distDir, filePath).replace(/\\/g, "/"),
								);
							}
						}
					}
				}
				const { entryNameFilesMap } =
					(buildTraceContext == null
						? void 0
				...
				await Promise.all(
					[
						...
						let route = entryName;
						if (isApp) {
							route = (0, _apppaths.normalizeAppPath)(
								route.substring("app".length),
							);
						}
						if (isPages) {
							route = (0, _normalizepagepath.normalizePagePath)(
								route.substring("pages".length),
							);
						}
						...
						const entryOutputPath = _path.default.join(
							distDir,
							"server",
							`${entryName}.js`,
						);
						...
						const existingTrace = JSON.parse(
							await _promises.default.readFile(traceOutputPath, "utf8"),
						);
						const traceOutputDir = _path.default.dirname(traceOutputPath);
						...
						for (const file of [...entryNameFiles, entryOutputPath]) {
							...
							for (const curFile of curFiles || []) {
								if (
									...
									const outputFile = _path.default
			...
			for (const type of moduleTypes) {
				...
				const contextDir = _path.default.join(
					_path.default.dirname(modulePath),
					"vendored",
					"contexts",
				);
				for (const item of await _promises.default.readdir(contextDir)) {
					const itemPath = _path.default.relative(
						root,
						_path.default.join(contextDir, item),
					);
	...
	await includeExcludeSpan.traceAsyncFn(async () => {
		...
		const { entryNameFilesMap } =
			(buildTraceContext == null ? void 0 : buildTraceContext.chunksTrace) ||
			{};
		const infos =
			pageInfos instanceof Map
				? pageInfos
				: (0, _utils.deserializePageInfos)(pageInfos);
		await Promise.all(
			[
				...
				if (
					!(combinedIncludes == null ? void 0 : combinedIncludes.size) &&
					!(combinedExcludes == null ? void 0 : combinedExcludes.size)
				) {
					return;
				}
				const traceFile = _path.default.join(
					distDir,
					`server`,
					`${entryName}.js.nft.json`,
				);
				...
				const traceContent = JSON.parse(
					await _promises.default.readFile(traceFile, "utf8"),
				);
				const includes = [];
				...
				if (combinedExcludes == null ? void 0 : combinedExcludes.size) {
					const resolvedGlobs = [...combinedExcludes].map((exclude) =>
						_path.default.join(dir, exclude),
					);
