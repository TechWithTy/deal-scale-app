...
function getPageFromPath(pagePath, pageExtensions) {
	let page = (0, _normalizepathsep.normalizePathSep)(
...
function createPagesMapping({
			...
			(0, _path.join)(
				pagesType === "pages"
					...
					: pagesType === "app"
						...
						: _constants.ROOT_DIR_ALIAS,
				pagePath,
			),
		);
		const route =
			pagesType === "app"
				? (0, _getmetadataroute.normalizeMetadataRoute)(pageKey)
				: pageKey;
	...
	switch (pagesType) {
		...
		case _pagetypes.PAGE_TYPES.PAGES: {
			...
			// added or removed.
			const root =
				isDev && pagesDir ? _constants.PAGES_DIR_ALIAS : "next/dist/pages";
			return {
				"/_app": `${root}/_app`,
				"/_error": `${root}/_error`,
...
function getEdgeServerEntry(opts) {
	...
	if (
		opts.pagesType === "app" &&
		(0, _isapprouteroute.isAppRouteRoute)(opts.page) &&
		opts.appDirLoader
	) {
		const loaderParams = {
	...
	if ((0, _utils.isMiddlewareFile)(opts.page)) {
		...
		const loaderParams = {
			...
			matchers: (
				(_opts_middleware = opts.middleware) == null
					? void 0
					: _opts_middleware.matchers
			)
				? (0, _nextmiddlewareloader.encodeMatchers)(opts.middleware.matchers)
				: "",
			preferredRegion: opts.preferredRegion,
			middlewareConfig: Buffer.from(
				JSON.stringify(opts.middlewareConfig || {}),
	...
	const loaderParams = {
		...
		sriEnabled:
			!opts.isDev &&
			!!((_opts_config_experimental_sri = opts.config.experimental.sri) == null
				? void 0
				: _opts_config_experimental_sri.algorithm),
...
function runDependingOnPageType(params) {
	if (
		params.pageType === _pagetypes.PAGE_TYPES.ROOT &&
		(0, _utils.isInstrumentationHookFile)(params.page)
	) {
		params.onServer();
		params.onEdgeServer();
		return;
...
async function createEntrypoints(params) {
	...
	if (appDir && appPaths) {
		for (const pathname in appPaths) {
			...
			appPathsPerRoute[normalizedPath].push(
				...
				getPageFromPath(actualPath, pageExtensions).replace(
					_constants.APP_DIR_ALIAS,
					"",
				),
			);
	...
	const getEntryHandler = (mappings, pagesType) => async (page) => {
		...
		const serverBundlePath =
			pagesType === _pagetypes.PAGE_TYPES.PAGES
				? _path.posix.join("pages", bundleFile)
				: pagesType === _pagetypes.PAGE_TYPES.APP
					? _path.posix.join("app", bundleFile)
					: bundleFile.slice(1);
		...
		const isInsideAppDir =
			!!appDir &&
			(absolutePagePath.startsWith(_constants.APP_DIR_ALIAS) ||
		...
		const isServerComponent =
			isInsideAppDir && staticInfo.rsc !== _constants1.RSC_MODULE_TYPES.client;
		if ((0, _utils.isMiddlewareFile)(page)) {
			var _staticInfo_middleware;
			middlewareMatchers = ((_staticInfo_middleware = staticInfo.middleware) ==
			null
				? void 0
			...
			onServer: () => {
				if (pagesType === "app" && appDir) {
					const matchedAppPaths =
						appPathsPerRoute[(0, _apppaths.normalizeAppPath)(page)];
					server[serverBundlePath] = getAppEntry({
						...
						middlewareConfig: (0, _utils1.encodeToBase64)(
					...
					!(0, _isinternalcomponent.isInternalComponent)(absolutePagePath) &&
	...
	if (rootPaths) {
		promises.push(
			Promise.all(
				Object.keys(rootPaths).map(
					getEntryHandler(rootPaths, _pagetypes.PAGE_TYPES.ROOT),
				),
			),
		);
	}
	promises.push(
		Promise.all(
			Object.keys(pages).map(
				getEntryHandler(pages, _pagetypes.PAGE_TYPES.PAGES),
			),
		),
	);
...
function finalizeEntrypoint({
	...
	switch (compilerType) {
		...
		case _constants1.COMPILER_NAMES.edgeServer: {
			return {
				layer:
					(0, _utils.isMiddlewareFilename)(name) || isApi || isInstrumentation
		...
		case _constants1.COMPILER_NAMES.client: {
			const isAppLayer =
				hasAppDir &&
				(name === _constants1.CLIENT_STATIC_FILES_RUNTIME_MAIN_APP ||
					name === _constants1.APP_CLIENT_INTERNALS ||
					name.startsWith("app/"));
			if (
				// Client special cases
				name !== _constants1.CLIENT_STATIC_FILES_RUNTIME_POLYFILLS &&
				name !== _constants1.CLIENT_STATIC_FILES_RUNTIME_MAIN &&
				name !== _constants1.CLIENT_STATIC_FILES_RUNTIME_MAIN_APP &&
				name !== _constants1.CLIENT_STATIC_FILES_RUNTIME_AMP &&
				name !== _constants1.CLIENT_STATIC_FILES_RUNTIME_REACT_REFRESH
			) {
				if (isAppLayer) {
					return {
