...
const _formatwebpackmessages = /*#__PURE__*/ _interop_require_default(
	require("../../client/components/react-dev-overlay/internal/helpers/format-webpack-messages"),
);
...
async function webpackBuildImpl(compilerName) {
				...
				(0, _webpackconfig.default)(dir, {
					...
					edgePreviewProps: {
						__NEXT_PREVIEW_MODE_ID:
							_buildcontext.NextBuildContext.previewProps.previewModeId,
						__NEXT_PREVIEW_MODE_ENCRYPTION_KEY:
							_buildcontext.NextBuildContext.previewProps
								.previewModeEncryptionKey,
						__NEXT_PREVIEW_MODE_SIGNING_KEY:
							_buildcontext.NextBuildContext.previewProps.previewModeSigningKey,
					},
					...info,
				}),
			]);
	...
	await runWebpackSpan.traceAsyncFn(async () => {
		...
		if (!compilerName || compilerName === "server") {
			...
			[serverResult, inputFileSystem] = await (0, _compiler.runCompiler)(
				serverConfig,
				{
					runWebpackSpan,
					inputFileSystem,
		...
		if (!compilerName || compilerName === "edge-server") {
			...
			[edgeServerResult, inputFileSystem] = edgeConfig
				? await (0, _compiler.runCompiler)(edgeConfig, {
						runWebpackSpan,
						inputFileSystem,
					})
				: [null];
		...
		if (
			!(serverResult == null ? void 0 : serverResult.errors.length) &&
			!(edgeServerResult == null ? void 0 : edgeServerResult.errors.length)
		) {
			...
			if (!compilerName || compilerName === "client") {
				...
				const start = Date.now();
				[clientResult, inputFileSystem] = await (0, _compiler.runCompiler)(
					clientConfig,
					{
						runWebpackSpan,
						inputFileSystem,
	...
	result = nextBuildSpan
		...
		.traceFn(() => (0, _formatwebpackmessages.default)(result, true));
	const telemetryPlugin =
		(_clientConfig_plugins = clientConfig.plugins) == null
			? void 0
			: _clientConfig_plugins.find(isTelemetryPlugin);
	const traceEntryPointsPlugin =
		(_serverConfig_plugins = serverConfig.plugins) == null
			? void 0
			: _serverConfig_plugins.find(isTraceEntryPointsPlugin);
		...
		return {
			...
			buildTraceContext:
				traceEntryPointsPlugin == null
					? void 0
					: traceEntryPointsPlugin.buildTraceContext,
			pluginState: (0, _buildcontext.getPluginState)(),
			telemetryState: {
				usages:
					(telemetryPlugin == null ? void 0 : telemetryPlugin.usages()) || [],
...
async function workerMain(workerData) {
	...
	// Resume plugin state
	(0, _buildcontext.resumePluginState)(
		_buildcontext.NextBuildContext.pluginState,
	);
	/// load the config because it's not serializable
	_buildcontext.NextBuildContext.config = await (0, _config.default)(
		_constants.PHASE_PRODUCTION_BUILD,
		_buildcontext.NextBuildContext.dir,
	);
	_buildcontext.NextBuildContext.nextBuildSpan = (0, _trace.trace)(
