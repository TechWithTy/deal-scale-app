...
async function batchedTraceSource(project, frame) {
	...
	return {
		frame: {
			...
			lineNumber:
				(_sourceFrame_line = sourceFrame.line) != null ? _sourceFrame_line : 0,
			column:
				(_sourceFrame_column = sourceFrame.column) != null
					? _sourceFrame_column
					: 0,
			methodName:
				(_ref =
					(_sourceFrame_methodName = sourceFrame.methodName) != null
						? _sourceFrame_methodName
...
function getOverlayMiddleware(project) {
	return async function (req, res) {
		...
		const frame = {
			...
			methodName:
				(_searchParams_get = searchParams.get("methodName")) != null
					? _searchParams_get
					: "<unknown>",
			line:
				parseInt(
					(_searchParams_get1 = searchParams.get("lineNumber")) != null
						? _searchParams_get1
						: "0",
					10,
				) || 0,
			column:
				parseInt(
					(_searchParams_get2 = searchParams.get("column")) != null
						? _searchParams_get2
						: "0",
					10,
				) || 0,
		...
		if (pathname === "/__nextjs_original-stack-frame") {
			...
			const fileExists = await _promises.default
				.access(frame.file, _promises.constants.F_OK)
				.then(
					() => true,
					() => false,
				);
			...
			try {
				...
				(0, _launchEditor.launchEditor)(
					frame.file,
					(_frame_line = frame.line) != null ? _frame_line : 1,
