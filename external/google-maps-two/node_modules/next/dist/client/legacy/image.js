...
function defaultLoader(param) {
	...
	return (
		(0, _normalizetrailingslash.normalizePathTrailingSlash)(config.path) +
		"?url=" +
...
function generateImgAttrs(param) {
	let { config, src, unoptimized, layout, width, quality, sizes, loader } =
		param;
	if (unoptimized) {
		return {
			src,
...
function defaultImageLoader(loaderProps) {
	...
	const loaderKey =
		((_loaderProps_config = loaderProps.config) == null
			? void 0
			: _loaderProps_config.loader) || "default";
...
function handleLoading(
	...
	p.catch(() => {}).then(() => {
		...
		if (process.env.NODE_ENV !== "production") {
			...
			if (
				(_img_parentElement = img.parentElement) == null
					? void 0
					: _img_parentElement.parentElement
			) {
				...
				if (!parent.position) {
					...
					(0, _warnonce.warnOnce)(
						'Image with src "' +
							src +
							'" may not render properly as a child of a flex container. Consider wrapping the image with a div to configure the width.',
					);
				} else if (
					layout === "fill" &&
					...
					(0, _warnonce.warnOnce)(
						'Image with src "' +
							src +
							'" may not render properly with a parent using position:"' +
							parent.position +
							'". Consider changing the parent style to position:"relative" with a width and height.',
					);
				}
			}
		}
		...
		if (!src) {
			...
			if (!VALID_LAYOUT_VALUES.includes(layout)) {
				throw new Error(
					'Image with src "' +
						src +
						'" has invalid "layout" property. Provided "' +
						layout +
						'" should be one of ' +
						VALID_LAYOUT_VALUES.map(String).join(",") +
						".",
				);
			}
			if (
				...
				throw new Error(
					'Image with src "' +
						src +
						'" has invalid "width" or "height" property. These should be numeric values.',
				);
			}
			if (layout === "fill" && (width || height)) {
				(0, _warnonce.warnOnce)(
					'Image with src "' +
						src +
						'" and "layout=\'fill\'" has unused properties assigned. Please remove "width" and "height".',
				);
			}
			...
			if (sizes && layout !== "fill" && layout !== "responsive") {
				(0, _warnonce.warnOnce)(
					'Image with src "' +
						src +
						'" has "sizes" property but it will be ignored. Only use "sizes" with "layout=\'fill\'" or "layout=\'responsive\'"',
				);
			}
			if (placeholder === "blur") {
				if (layout !== "fill" && (widthInt || 0) * (heightInt || 0) < 1600) {
					(0, _warnonce.warnOnce)(
						'Image with src "' +
							src +
							'" is smaller than 40x40. Consider removing the "placeholder=\'blur\'" property to improve performance.',
					);
				}
				if (!blurDataURL) {
			...
			if ("ref" in rest) {
				(0, _warnonce.warnOnce)(
					'Image with src "' +
						src +
						'" is using unsupported "ref" property. Consider using the "onLoadingComplete" property instead.',
				);
			}
			...
			if (style) {
				...
				if (overwrittenStyles.length) {
					(0, _warnonce.warnOnce)(
						"Image with src " +
							src +
							" is assigned the following styles, which are overwritten by automatically-generated styles: " +
							overwrittenStyles.join(", "),
					);
				}
			}
			if (
