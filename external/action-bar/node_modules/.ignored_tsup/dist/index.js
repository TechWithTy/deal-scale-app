...
var postcssPlugin = ({ css, inject, cssLoader }) => {
	return {
		name: "postcss",
		setup(build2) {
			let configCache;
			const getPostcssConfig = async () => {
				const loadConfig = _chunkTWFEYLU4js.__require.call(
					void 0,
					"postcss-load-config",
				);
				if (configCache) {
			...
			build2.onLoad({ filter: /\.css$/ }, async (args) => {
				...
				if (plugins && plugins.length > 0) {
					...
					const result = await _optionalChain([
						...
						(_4) => _4.process,
						"call",
						(_5) => _5(contents, { ...options, from: args.path }),
					]);
					contents = result.css;
				}
				if (inject) {
					contents = (
						await _esbuild.transform.call(void 0, contents, {
	...
	return {
		...
		setup(build2) {
			...
			build2.onLoad({ filter: /\.svelte$/ }, async (args) => {
				svelte =
					svelte ||
					_chunkTWFEYLU4js.localRequire.call(void 0, "svelte/compiler");
				sveltePreprocessor =
					sveltePreprocessor ||
					_chunkTWFEYLU4js.localRequire.call(void 0, "svelte-preprocess");
				if (!svelte) {
					return {
						...
						sveltePreprocessor
							...
							: {
									async script({ content, attributes }) {
										...
										const { code, map } = await _esbuild.transform.call(
											void 0,
											content,
											{
			...
			build2.onLoad({ filter: /\.[jt]sx?$/ }, async (args) => {
				...
				if (result.map) {
					...
					map.sources = map.sources.map((source) => {
						return _path2.default.isAbsolute(source)
							? _path2.default.relative(
									_path2.default.dirname(args.path),
									source,
								)
							: source;
					});
...
var getOutputExtensionMap = (options, format, pkgType) => {
	const outExtension =
		options.outExtension || _chunkTWFEYLU4js.defaultOutExtension;
...
var generateExternal = async (external) => {
	...
	for (const item of external) {
		...
		const pkgPath = _path2.default.isAbsolute(item)
			? _path2.default.dirname(item)
			: _path2.default.dirname(_path2.default.resolve(process.cwd(), item));
...
async function runEsbuild(
	options,
	{ format, css, logger: logger3, buildDependencies, pluginContainer },
) {
	...
	const esbuildPlugins = [
		...
		options.tsconfigDecoratorMetadata &&
			swcPlugin({ ...options.swc, logger: logger3 }),
	...
	try {
		result = await _esbuild.build.call(void 0, {
			...
			inject: [
				format === "cjs" && injectShims
					? _path2.default.join(__dirname, "../assets/cjs_shims.js")
					: "",
				format === "esm" && injectShims && platform === "node"
					? _path2.default.join(__dirname, "../assets/esm_shims.js")
					: "",
				...(options.inject || []),
			].filter(Boolean),
			outdir:
				options.legacyOutput && format !== "cjs"
					? _path2.default.join(outDir, format)
					: outDir,
	...
	if (result && result.outputFiles) {
		...
		logger3.success(
			format,
			`\u26A1\uFE0F Build success in ${Math.floor(timeInMs)}ms`,
		);
	}
	if (result.metafile) {
...
var cjsSplitting = () => {
	return {
		...
		async renderChunk(code, info) {
			...
			const { transform: transform3 } = await Promise.resolve().then(() =>
				_interopRequireWildcard(require("sucrase")),
			);
		...
		const files = outputFiles
			...
			.map((file) => {
				if (isJS(file.path) || isCSS(file.path)) {
					const relativePath = _chunkTWFEYLU4js.slash.call(
						void 0,
						_path2.default.relative(process.cwd(), file.path),
					);
					...
					return {
						...
						map: _optionalChain([
							...
							(_12) => _12.find,
							"call",
							(_13) => _13((f) => f.path === `${file.path}.map`),
							"optionalAccess",
							(_14) => _14.text,
						]),
				...
				for (const plugin of this.plugins) {
					if (info.type === "chunk" && plugin.renderChunk) {
						...
						if (result) {
							...
							if (result.map) {
								const originalConsumer = await new (0,
								_sourcemap.SourceMapConsumer)(parseSourceMap(info.map));
								const newConsumer = await new (0, _sourcemap.SourceMapConsumer)(
									parseSourceMap(result.map),
								);
								const generator =
									_sourcemap.SourceMapGenerator.fromSourceMap(newConsumer);
...
var getSourcemapComment = (inline, map, filepath, isCssFile) => {
	...
	const url = inline
		...
		: `${_path2.default.basename(filepath)}.map`;
...
var treeShakingPlugin = ({ treeshake, name, silent }) => {
	return {
		name: "tree-shaking",
		async renderChunk(code, info) {
			...
			for (const file of result.output) {
				if (
					file.type === "chunk" &&
					file.fileName === _path2.default.basename(info.path)
				) {
					return {
						code: file.code,
						map: file.map,
...
var isInPublicDir = (publicDir, filePath) => {
	...
	return _chunkTWFEYLU4js.slash
		.call(void 0, _path2.default.resolve(filePath))
...
function getExports(program, fileMapping) {
	...
	function extractExports(sourceFileName) {
		...
		sourceFileName = _chunkTWFEYLU4js.toAbsolutePath.call(
			void 0,
			sourceFileName,
			cwd,
		);
...
function emitDtsFiles(program, host) {
	...
	const diagnostics = _typescript2.default
	...
	diagnostics.forEach((diagnostic) => {
		if (diagnostic.file) {
			const { line, character } =
				_typescript2.default.getLineAndCharacterOfPosition(
					diagnostic.file,
					diagnostic.start,
				);
...
function formatAggregationExport(declaration, declarationDirPath) {
	const dest = _chunkTWFEYLU4js.replaceDtsWithJsExtensions.call(
		...
		`./${_path2.default.posix.normalize(
			_chunkTWFEYLU4js.slash.call(
				void 0,
				_path2.default.relative(declarationDirPath, declaration.destFileName),
			),
		)}`,
	);
...
function formatDistributionExports(exports, fromFilePath, toFilePath) {
	let importPath = _chunkTWFEYLU4js.replaceDtsWithJsExtensions.call(
		...
		_path2.default.posix.relative(
			_path2.default.posix.dirname(
				_path2.default.posix.normalize(
					_chunkTWFEYLU4js.slash.call(void 0, fromFilePath),
				),
			),
			_path2.default.posix.normalize(
				_chunkTWFEYLU4js.slash.call(void 0, toFilePath),
			),
		),
	);
	if (!/^\.+\//.test(importPath)) {
...
async function rollupDtsFiles(options, exports, format) {
	...
	const dtsOutputFilePath = _path2.default.join(
		outDir,
		`_tsup-dts-rollup${dtsExtension}`,
	);
...
var normalizeOptions = async (
		...
		dts:
			typeof _options.dts === "boolean"
				...
				: typeof _options.dts === "string"
					...
					: _options.dts,
		experimentalDts:
			await _chunkTWFEYLU4js.resolveInitialExperimentalDtsConfig.call(
				void 0,
	...
	const tsconfig = _bundlerequire.loadTsConfig.call(
		void 0,
		process.cwd(),
		options.tsconfig,
	);
	if (tsconfig) {
		...
		options.tsconfigResolvePaths =
			_optionalChain([
				tsconfig,
				"access",
				(_26) => _26.data,
				"optionalAccess",
				(_27) => _27.compilerOptions,
				"optionalAccess",
				(_28) => _28.paths,
			]) || {};
		options.tsconfigDecoratorMetadata = _optionalChain([
			tsconfig,
			"access",
			(_29) => _29.data,
			"optionalAccess",
			(_30) => _30.compilerOptions,
			"optionalAccess",
			(_31) => _31.emitDecoratorMetadata,
		]);
		if (options.dts) {
		...
		if (options.experimentalDts) {
			options.experimentalDts =
				await _chunkTWFEYLU4js.resolveExperimentalDtsConfig.call(
					void 0,
					options,
					tsconfig,
				);
		...
		if (!options.target) {
			options.target = _optionalChain([
				tsconfig,
				"access",
				(_32) => _32.data,
				"optionalAccess",
				(_33) => _33.compilerOptions,
				"optionalAccess",
				(_34) => _34.target,
				"optionalAccess",
				(_35) => _35.toLowerCase,
		...
		throw new (0, _chunkJZ25TPTYjs.PrettyError)(
			`Cannot find tsconfig: ${options.tsconfig}`,
		);
	}
	if (!options.target) {
...
async function build(_options) {
	const config =
		_options.config === false
			? {}
			: await _chunkVGC3FXLUjs.loadTsupConfig.call(
					void 0,
					process.cwd(),
	...
	await Promise.all(
		[...(Array.isArray(configData) ? configData : [configData])].map(
			async (item) => {
				const logger3 = _chunkVGC3FXLUjs.createLogger.call(
					void 0,
					_optionalChain([item, "optionalAccess", (_37) => _37.name]),
				);
					...
					if (options.dts) {
						await new Promise((resolve, reject) => {
							const worker = new (0, _worker_threads.Worker)(
								_path2.default.join(__dirname, "./rollup.js"),
							);
				...
				const mainTasks = async () => {
					if (
						...
						let depsHash = await _chunkVGC3FXLUjs.getAllDepsHash.call(
							void 0,
							process.cwd(),
						);
						...
						const buildAll = async () => {
							...
							if (options.onSuccess) {
								if (typeof options.onSuccess === "function") {
									...
									onSuccessProcess = _tinyexec.exec.call(
										void 0,
										options.onSuccess,
										[],
										{
						...
						const startWatcher = async () => {
							...
							const { watch } = await Promise.resolve().then(() =>
								_interopRequireWildcard(require("chokidar")),
							);
							...
							const watcher = watch(
								await _tinyglobby.glob.call(void 0, watchPaths),
								{
									ignoreInitial: true,
									ignorePermissionErrors: true,
									ignored: (p) =>
										_tinyglobby.globSync.call(void 0, p, { ignore: ignored })
											.length === 0,
								},
							);
							watcher.on("all", async (type, file) => {
								...
								if (options.watch === true) {
									if (file === "package.json" && !buildDependencies.has(file)) {
										const currentHash =
											await _chunkVGC3FXLUjs.getAllDepsHash.call(
												void 0,
												process.cwd(),
											);
