...
async function removeFiles(patterns, dir) {
	...
	files.forEach(
		(file) => _fs2.default.existsSync(file) && _fs2.default.unlinkSync(file),
	);
}
...
function toAbsolutePath(p, cwd) {
	...
	return slash(
		_path2.default.normalize(_path2.default.join(cwd || process.cwd(), p)),
	);
}
	...
	const objectEntries = Object.fromEntries(
		arrayOfEntries.map((entry) => [
			_path2.default.posix.join(
				...entry
					.split(_path2.default.posix.sep)
					.slice(1, -1)
					.concat(_path2.default.parse(entry).name),
			),
			entry,
		]),
	);
	...
	const resolvedEntryPaths =
		typeof entryPaths === "string" || Array.isArray(entryPaths)
			? convertArrayEntriesToObjectEntries(
					await _tinyglobby.glob.call(void 0, entryPaths),
				)
			: entryPaths;
...
var resolveInitialExperimentalDtsConfig = async (experimentalDts) => {
	...
	if (typeof experimentalDts === "string") {
		return {
			entry: convertArrayEntriesToObjectEntries(
				await _tinyglobby.glob.call(void 0, experimentalDts),
			),
		};
	}
	return {
		...experimentalDts,
		entry:
			_optionalChain([experimentalDts, "optionalAccess", (_6) => _6.entry]) ==
			null
				? {}
...
exports.resolveInitialExperimentalDtsConfig =
