...
var require_path_parse = _chunkTWFEYLU4js.__commonJS.call(void 0, {
	"node_modules/.pnpm/path-parse@1.0.7/node_modules/path-parse/index.js"(
		...
		if (isWindows) module.exports = win322.parse;
		else module.exports = posix2.parse;
...
var require_async = _chunkTWFEYLU4js.__commonJS.call(void 0, {
	"node_modules/.pnpm/resolve@1.22.10/node_modules/resolve/lib/async.js"(
		...
		var realpathFS =
			...
			fs2.realpath &&
			typeof fs2.realpath.native === "function"
				? fs2.realpath.native
				: fs2.realpath;
...
var require_sync = _chunkTWFEYLU4js.__commonJS.call(void 0, {
	"node_modules/.pnpm/resolve@1.22.10/node_modules/resolve/lib/sync.js"(
		...
		var realpathFS =
			process.platform !== "win32" &&
			fs2.realpathSync &&
			typeof fs2.realpathSync.native === "function"
				? fs2.realpathSync.native
				: fs2.realpathSync;
		var homedir = getHomedir();
		var defaultPaths = function () {
			return [
		...
		var Mappings = class {
			...
			addEdit(sourceIndex, content, loc, nameIndex) {
				if (content.length) {
					...
					while (
						contentLineEnd >= 0 &&
						contentLengthMinusOne > contentLineEnd
					) {
						const segment2 = [
...
var require_rollup_plugin_dts = _chunkTWFEYLU4js.__commonJS.call(void 0, {
	"node_modules/.pnpm/rollup-plugin-dts@6.1.1_rollup@4.34.8_typescript@5.7.3/node_modules/rollup-plugin-dts/dist/rollup-plugin-dts.cjs"(
		...
		var formatHost = {
			...
			getCanonicalFileName: ts2.sys.useCaseSensitiveFileNames
				? (f) => f
				: (f) => f.toLowerCase(),
		};
		var DEFAULT_OPTIONS = {
		...
		function cacheConfig([fromPath, toPath], config) {
			...
			while (
				fromPath !== toPath && // make sure we're not stuck in an infinite loop
		...
		function getCompilerOptions(input, overrideOptions, overrideConfigPath) {
			...
			if (!configByPath.has(cacheKey)) {
				...
				const configPath = overrideConfigPath
					...
					: ts2.findConfigFile(dirName, ts2.sys.fileExists);
				if (!configPath) {
		...
		function createProgram$1(fileName, overrideOptions, tsconfig) {
			...
			return ts2.createProgram(
				...
				compilerOptions,
				ts2.createCompilerHost(compilerOptions, true),
			);
		}
		...
		function getCodeFrame() {
			...
			try {
				...
				try {
					const esmRequire = module$1.createRequire(
						typeof document === "undefined"
							? _chunkTWFEYLU4js.__require
									.call(void 0, "url")
									.pathToFileURL(__filename).href
							: (_documentCurrentScript && _documentCurrentScript.src) ||
									new URL("rollup-plugin-dts.cjs", document.baseURI).href,
					);
		...
		var NamespaceFixer = class {
			...
			findNamespaces() {
				...
				for (const node of this.sourceFile.statements) {
					...
					if (
						(ts2.isImportDeclaration(node) || ts2.isExportDeclaration(node)) &&
						node.moduleSpecifier &&
						ts2.isStringLiteral(node.moduleSpecifier)
					) {
					...
					if (
						ts2.isModuleDeclaration(node) &&
						node.body &&
						ts2.isModuleBlock(node.body)
					) {
						for (const stmt of node.body.statements) {
							if (ts2.isExportDeclaration(stmt) && stmt.exportClause) {
								...
								for (const decl2 of stmt.exportClause.elements) {
									if (
										decl2.propertyName &&
										decl2.propertyName.getText() == decl2.name.getText()
									) {
										namespaces.unshift({
											name: "",
											exports: [],
					...
					if (
						...
						!ts2.isObjectLiteralExpression(obj)
					) {
						continue;
					}
					const exports2 = [];
					for (const prop of obj.properties) {
						if (
							!ts2.isPropertyAssignment(prop) ||
							!(
								ts2.isIdentifier(prop.name) || ts2.isStringLiteral(prop.name)
							) ||
							(prop.name.text !== "__proto__" &&
								!ts2.isIdentifier(prop.initializer))
						) {
...
`;
		...
		function preProcess({ sourceFile }) {
			...
			for (const node of sourceFile.statements) {
				...
				if (
					ts2.isEnumDeclaration(node) ||
					ts2.isFunctionDeclaration(node) ||
					ts2.isInterfaceDeclaration(node) ||
					ts2.isClassDeclaration(node) ||
					ts2.isTypeAliasDeclaration(node) ||
					ts2.isModuleDeclaration(node)
				) {
					if (node.name) {
					...
					if (declarations.length == 1) {
						...
						pushNamedNode(first.name.getText(), [
							getStart(node),
							first.getEnd(),
						]);
					...
					const type =
						flags & ts2.NodeFlags.Let
							? "let"
							: flags & ts2.NodeFlags.Const
								? "const"
								: "var";
					const prefix = `declare ${type} `;
					const list = node.declarationList
						.getChildren()
						.find((c) => c.kind === ts2.SyntaxKind.SyntaxList)
						.getChildren();
					let commaPos = 0;
			...
			for (const node of sourceFile.statements) {
				...
				if (ts2.isFunctionDeclaration(node) || ts2.isClassDeclaration(node)) {
					...
					const idx = children.findIndex(
						(node2) =>
							node2.kind === ts2.SyntaxKind.ClassKeyword ||
							node2.kind === ts2.SyntaxKind.FunctionKeyword,
					);
					const token = children[idx];
					const nextToken = children[idx + 1];
					const isPunctuation =
						nextToken.kind >= ts2.SyntaxKind.FirstPunctuation &&
						nextToken.kind <= ts2.SyntaxKind.LastPunctuation;
					if (isPunctuation) {
						const addSpace =
...
`);
			...
			function checkInlineImport(node) {
				...
				if (ts2.isImportTypeNode(node)) {
					if (
						!ts2.isLiteralTypeNode(node.argument) ||
						!ts2.isStringLiteral(node.argument.literal)
					) {
						throw new UnsupportedSyntaxError(
							node,
					...
					const start = children
						.find((t) => t.kind === ts2.SyntaxKind.ImportKeyword)
						.getStart();
					let end = node.getEnd();
					const token = children.find(
						(t) =>
							t.kind === ts2.SyntaxKind.DotToken ||
							t.kind === ts2.SyntaxKind.LessThanToken,
					);
					if (token) {
						end = token.getStart();
					}
		...
		var DeclarationScope = class {
			...
			convertPropertyAccess(node) {
				if (
					!ts2.isIdentifier(node.expression) &&
					!ts2.isPropertyAccessExpression(node.expression)
				) {
				...
				let object = ts2.isIdentifier(node.expression)
			...
			convertComputedPropertyName(node) {
				...
				if (
					ts2.isLiteralExpression(expression) ||
					ts2.isPrefixUnaryExpression(expression)
				) {
					return;
				}
			...
			convertMembers(members) {
				for (const node of members) {
					if (
						ts2.isPropertyDeclaration(node) ||
						ts2.isPropertySignature(node) ||
						ts2.isIndexSignatureDeclaration(node)
					) {
						if (
							ts2.isPropertyDeclaration(node) &&
							node.initializer &&
							ts2.isPropertyAccessExpression(node.initializer)
						) {
					...
					if (
						ts2.isMethodDeclaration(node) ||
						ts2.isMethodSignature(node) ||
						ts2.isConstructorDeclaration(node) ||
						ts2.isConstructSignatureDeclaration(node) ||
						ts2.isCallSignatureDeclaration(node) ||
						ts2.isGetAccessorDeclaration(node) ||
						ts2.isSetAccessorDeclaration(node)
					) {
			...
			convertTypeNode(node) {
				...
				if (
					ts2.isNamedTupleMember(node) ||
					ts2.isParenthesizedTypeNode(node) ||
					ts2.isTypeOperatorNode(node) ||
					ts2.isTypePredicateNode(node)
				) {
			...
			convertNamespace(node, relaxedModuleBlock = false) {
				...
				if (
					relaxedModuleBlock &&
					node.body &&
					ts2.isModuleDeclaration(node.body)
				) {
				...
				for (const stmt of statements) {
					if (
						ts2.isEnumDeclaration(stmt) ||
						ts2.isFunctionDeclaration(stmt) ||
						ts2.isClassDeclaration(stmt) ||
						ts2.isInterfaceDeclaration(stmt) ||
						ts2.isTypeAliasDeclaration(stmt) ||
						ts2.isModuleDeclaration(stmt)
					) {
				...
				for (const stmt of statements) {
					...
					if (
						...
						const typeVariables = this.convertTypeParameters(
							stmt.typeParameters,
						);
		...
		var Transformer = class {
			...
			createDeclaration(node, id) {
				...
				if (existingScope) {
					...
					let selfIdx = this.ast.body.findIndex(
						(node2) => node2 == existingScope.declaration,
					);
			...
			convertStatement(node) {
				...
				if (
					ts2.isImportDeclaration(node) ||
					ts2.isImportEqualsDeclaration(node)
				) {
			...
			convertNamespaceDeclaration(node) {
				const isGlobalAugmentation =
					node.flags & ts2.NodeFlags.GlobalAugmentation;
			...
			convertExportSpecifier(node) {
				...
				return {
					...
					exported,
					local: node.propertyName
						? createIdentifier(node.propertyName)
						: exported,
				};
			}
		};
		var ExportsFixer = class {
			...
			findExports() {
				...
				return rawExports.map((rawExport) => {
					const elements = rawExport.elements.map((e) => {
						...
						const localName = _nullishCoalesce(
							_optionalChain([
								e,
								"access",
								(_6) => _6.propertyName,
						...
						const kind =
							types.some((node) => node.getText() === localName) &&
							!values.some((node) => node.getText() === localName)
								? "type"
								: "value";
						this.DEBUG &&
							console.log(
								`export ${localName} as ${exportedName} is a ${kind}`,
							);
						return {
							exportedName,
							localName,
			...
			getExportsAndLocals(statements = this.source.statements) {
				...
				for (const statement of statements) {
					...
					if (
						ts2.isInterfaceDeclaration(statement) ||
						ts2.isTypeAliasDeclaration(statement)
					) {
						this.DEBUG &&
							console.log(`${statement.name.getFullText()} is a type`);
						types.push(statement.name);
						continue;
					}
					if (
						ts2.isEnumDeclaration(statement) ||
						ts2.isFunctionDeclaration(statement) ||
						ts2.isClassDeclaration(statement) ||
						ts2.isVariableStatement(statement)
					) {
						if (ts2.isVariableStatement(statement)) {
							for (const declaration of statement.declarationList
								...
								if (ts2.isIdentifier(declaration.name)) {
									this.DEBUG &&
										console.log(
											`${declaration.name.getFullText()} is a value (from var statement)`,
										);
							...
							if (statement.name) {
								this.DEBUG &&
									console.log(
										`${statement.name.getFullText()} is a value (from declaration)`,
									);
					...
					if (ts2.isModuleDeclaration(statement)) {
						if (statement.name && ts2.isIdentifier(statement.name)) {
							this.DEBUG &&
								console.log(
									`${statement.name.getFullText()} is a value (from module declaration)`,
								);
							values.push(statement.name);
						}
			...
			getExportStatement(exportDeclaration) {
				const isTypeOnly =
					exportDeclaration.exports.every((e) => e.kind === "type") &&
					exportDeclaration.exports.length > 0;
		...
		var transform = () => {
			...
			return {
				...
				transform(code, fileName) {
					...
					allTypeReferences.set(
						sourceFile.fileName,
						preprocessed.typeReferences,
					);
					allFileReferences.set(
						sourceFile.fileName,
						preprocessed.fileReferences,
					);
					...
					return {
						code,
						ast: converted.ast,
						map: preprocessed.code.generateMap(),
					};
				},
				renderChunk(inputCode, chunk, options) {
					...
					for (const fileName of Object.keys(chunk.modules)) {
						...
						for (const ref of allFileReferences.get(
							...
							if (ref.startsWith(".")) {
								const absolutePathToOriginal = path__namespace.join(
									path__namespace.dirname(fileName),
									ref,
								);
								const chunkFolder =
									(options.file && path__namespace.dirname(options.file)) ||
									(chunk.facadeModuleId &&
										path__namespace.dirname(chunk.facadeModuleId)) ||
									".";
								let targetRelPath = path__namespace
					...
					let code = writeBlock(
						Array.from(
							fileReferences,
							(ref) => `/// <reference path="${ref}" />`,
						),
					);
					code += writeBlock(
						Array.from(
							typeReferences,
							(ref) => `/// <reference types="${ref}" />`,
						),
					);
					code += fixer.fix();
					if (!code) {
		...
		function getModule(
			{ programs, resolvedOptions: { compilerOptions, tsconfig } },
			fileName,
			code,
		) {
			...
			return {
				...
				transform(code, id) {
					...
					const watchFiles = (module2) => {
						if (module2.program) {
							...
							const sourceFilesInProgram = module2.program
								.getSourceFiles()
					...
					const generateDtsFromTs = () => {
						const module2 = getModule(ctx, id, code);
						if (!module2 || !module2.source || !module2.program) return null;
						watchFiles(module2);
						const declarationId = id.replace(TS_EXTENSIONS, dts);
						let generated;
						...
						if (emitSkipped) {
							const errors = diagnostics.filter(
								(diag) => diag.category === ts2.DiagnosticCategory.Error,
							);
							if (errors.length) {
								console.error(ts2.formatDiagnostics(errors, formatHost));
				...
				resolveId(source, importer) {
					...
					if (ctx.resolvedOptions.tsconfig) {
						const resolvedSource = source.startsWith(".")
							? path__namespace.resolve(
									path__namespace.dirname(importer),
									source,
								)
							: source;
					...
					if (
						...
						return {
							id: path__namespace.resolve(resolvedModule.resolvedFileName),
						};
					}
				},
			};
...
var import_picomatch = _chunkTWFEYLU4js.__toESM.call(
	void 0,
	require_picomatch2(),
	1,
);
...
function getMatcherString(id, resolutionBase) {
	if (
		resolutionBase === false ||
		_path.isAbsolute.call(void 0, id) ||
		id.startsWith("**")
	) {
		return normalizePath(id);
	}
	const basePath = normalizePath(
		_path.resolve.call(void 0, resolutionBase || ""),
	).replace(/[-^$*+?.()|[\]{}]/g, "\\$&");
...
var dataToEsm = function dataToEsm2(data, options = {}) {
	...
	for (const key of Object.keys(data)) {
		const underbarPrefixLength =
			(_b =
				(_a = /^(_+)/.exec(key)) === null || _a === void 0
					? void 0
					: _a[0].length) !== null && _b !== void 0
				? _b
				: 0;
...
var resolveModule = (id, opts) =>
	new Promise((resolve2, reject) => {
		(0, import_resolve.default)(id, opts, (err, res) => {
			if (
				_optionalChain([err, "optionalAccess", (_12) => _12.code]) ===
				"MODULE_NOT_FOUND"
			)
	...
	return {
		...
		async resolveId(source, importer) {
			...
			const basedir = importer
				? await _fs2.default.promises.realpath(_path2.default.dirname(importer))
				: process.cwd();
			if (source[0] === ".") {
...
var getRollupConfig = async (options) => {
	...
	if (Array.isArray(dtsOptions.entry) && dtsOptions.entry.length > 1) {
		dtsOptions.entry = _chunkTWFEYLU4js.toObjectEntry.call(
			void 0,
			dtsOptions.entry,
		);
	}
	...
	const deps = await _chunkVGC3FXLUjs.getProductionDeps.call(
		void 0,
		process.cwd(),
	);
	const tsupCleanPlugin = {
		...
		async buildStart() {
			if (options.clean) {
				await _chunkTWFEYLU4js.removeFiles.call(
					void 0,
					["**/*.d.{ts,mts,cts}"],
					options.outDir,
				);
		...
		outputConfig: options.format.map((format) => {
			const outputExtension =
				_optionalChain([
					options,
					"access",
					(_15) => _15.outExtension,
					"optionalCall",
					(_16) => _16({ format, options, pkgType: pkg.type }),
					"access",
					(_17) => _17.dts,
				]) ||
				_chunkTWFEYLU4js.defaultOutExtension.call(void 0, {
					format,
					pkgType: pkg.type,
				}).dts;
			return {
				...
				plugins: [
					format === "cjs" &&
						options.cjsInterop &&
						_rollup.FixDtsDefaultCjsExportsPlugin.call(void 0),
				].filter(Boolean),
			};
		}),
	};
...
async function runRollup(options) {
	const { rollup } = await Promise.resolve().then(() =>
		_interopRequireWildcard(require("rollup")),
	);
	try {
...
async function watchRollup(options) {
	...
	watch({
		...
		if (event.code === "START") {
			...
			logger.success(
				"dts",
				`\u26A1\uFE0F Build success in ${event.duration}ms`,
			);
			_optionalChain([
