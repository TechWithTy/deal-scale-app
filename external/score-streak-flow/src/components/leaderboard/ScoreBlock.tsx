...
export const ScoreBlock: React.FC<ScoreBlockProps> = ({
	...
	const calculateCommission = () => {
		...
		const avgDealAmount =
			cityAverageDealAmounts[cityName] || cityAverageDealAmounts.default;
	...
	return (
		<div className="flex w-full min-w-0 shrink-0 flex-row flex-wrap items-center justify-start gap-2 md:w-auto md:flex-col md:items-end md:justify-start md:gap-1">
			...
			<div className="flex min-w-0 items-center gap-2">
				<motion.div
					...
					className={cn(
						"max-w-[10rem] truncate font-bold text-sm transition-colors duration-smooth sm:text-base md:text-lg",
			...
			<div className="max-w-full rounded-md bg-emerald-50 px-2 py-1 text-left md:text-right dark:bg-emerald-950/30">
				<div className="font-semibold text-[11px] text-emerald-700 sm:text-xs lg:text-sm dark:text-emerald-400">
					$
				...
				<div className="text-[9px] text-emerald-600/70 lg:text-[10px] dark:text-emerald-500/70">
					Est. Profit
				</div>
			</div>
		</div>
