...
class ModuleCacheMap extends Map {
	...
	invalidateDepTree(ids, invalidated = /* @__PURE__ */ new Set()) {
		for (const _id of ids) {
			...
			mod?.importers && this.invalidateDepTree(mod.importers, invalidated),
				super.delete(id);
		}
		return invalidated;
	}
...
function normalizeModuleId(file, root) {
	...
	let unixFile = slash(file)
		...
		.replace(/^\/+/, "/");
	return (
		unixFile.startsWith(root) && (unixFile = unixFile.slice(root.length - 1)),
		unixFile.replace(/^file:\//, "/")
	);
}
class HMRContext {
...
class HMRClient {
	...
	warnFailedUpdate(err, path) {
		err.message.includes("fetch") || this.logger.error(err),
			this.logger.error(
...
function interceptStackTrace(runtime, options = {}) {
	return (
		overridden ||
			((Error.prepareStackTrace = prepareStackTrace), (overridden = !0)),
		...
		() => resetInterceptor(runtime, options)
	);
}
...
function mapSourcePosition(position) {
	...
	if (
		(sourceMap || (sourceMap = sourceMapCache[position.source]), !sourceMap)
	) {
...
function wrapCallSite(frame, state) {
	if (
		(state === void 0 && (state = { nextPosition: null, curPosition: null }),
		frame.isNative())
	)
...
class ViteRuntime {
	constructor(options, runner, debug) {
		(this.options = options),
			...
			typeof options.hmr == "object" &&
				((this.hmrClient = new HMRClient(
					options.hmr.logger === !1
						? silentConsole
						: options.hmr.logger || console,
					options.hmr.connection,
					({ acceptedPath, ssrInvalidates }) => (
						this.moduleCache.invalidate(acceptedPath),
						ssrInvalidates && this.invalidateFiles(ssrInvalidates),
						this.executeUrl(acceptedPath)
					),
				)),
	...
	async cachedRequest(id, fetchedModule, callstack = [], metadata) {
		...
		const mod = this.moduleCache.getByModuleId(moduleId),
			{ imports, importers } = mod,
			importee = callstack[callstack.length - 1];
		if (
			(importee && importers.add(importee),
...
${getStack()}`,
		...
		try {
			...
			return (
				(mod.promise = promise),
				(mod.evaluated = !1),
				this.processImport(await promise, fetchedModule, metadata)
			);
		} finally {
	...
	async directRequest(id, fetchResult, _callstack) {
		const moduleId = fetchResult.id,
			...
			request = async (dep, metadata) => {
				...
				return (
					this.moduleCache
						.getByModuleId(fetchedModule.id)
						.importers.add(moduleId),
					mod.imports.add(fetchedModule.id),
					this.cachedRequest(dep, fetchedModule, callstack, metadata)
				);
			},
			dynamicRequest = async (dep) => (
				(dep = String(dep)),
				dep[0] === "." && (dep = posixResolve(posixDirname(id), dep)),
