...
export function init(args) {
	...
	let tailwindConfigLocation = path.resolve(
		args["_"][1] ?? `./tailwind.config.${extension}`,
	);
	...
	if (fs.existsSync(tailwindConfigLocation)) {
		...
		messages.push(
			`Created Tailwind CSS config file: ${path.basename(tailwindConfigLocation)}`,
		);
	}
	...
	if (args["--postcss"]) {
		...
		if (fs.existsSync(postcssConfigLocation)) {
			...
			messages.push(
				`Created PostCSS config file: ${path.basename(postcssConfigLocation)}`,
			);
		}
	}
