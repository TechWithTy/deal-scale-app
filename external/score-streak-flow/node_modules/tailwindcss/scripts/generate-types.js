...
fs.writeFileSync(
	...
	prettier.format(
		`export interface DefaultColors { ${JSON.stringify(
			colorsWithoutDeprecatedColors,
		).slice(1, -1)}\n${deprecatedColors}\n}`,
		{
			semi: false,
			singleQuote: true,
...
const defaultThemeTypes = Object.entries(defaultTheme)
	.map(([name, value]) => {
		...
		if (name === "keyframes") {
			return [
				name,
				`Record<${types.forKeys(value)}, Record<string, CSSDeclarationList>>`,
			];
		}
		...
		if (name === "fontSize") {
			return [
				name,
				`Record<${types.forKeys(value)}, [string, { lineHeight: string }]>`,
			];
		}
