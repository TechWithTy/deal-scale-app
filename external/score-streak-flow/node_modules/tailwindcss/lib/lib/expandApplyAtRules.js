...
/** @typedef {Map<string, [any, import('postcss').Rule[]]>} ApplyCache */ function extractClasses(
	node,
) {
	...
	let normalizedGroups = Array.from(groups.values(), (classes) =>
		Array.from(classes),
	);
 ...
 */ function buildLocalApplyCache(root, context) {
	...
	root.walkRules((rule) => {
		...
		for (let node of pathToRoot(rule)) {
			...
			if (
				((_node_raws_tailwind = node.raws.tailwind) === null ||
				_node_raws_tailwind === void 0
					? void 0
					: _node_raws_tailwind.layer) !== undefined
			) {
				return;
			}
		}
 ...
 */ function buildApplyCache(applyCandidates, context) {
	for (let candidate of applyCandidates) {
		...
		let matches = Array.from(
			(0, _generateRules.resolveMatches)(candidate, context),
		);
		if (matches.length === 0) {
...
function processApply(root, context, localCache) {
	 ...
	 */ function replaceSelector(selector, utilitySelectors, candidate) {
		...
		let candidateList = extractSelectors(
			`.${(0, _escapeClassName.default)(candidate)}`,
		);
		let candidateClass = candidateList.nodes[0].nodes[0];
		selectorList.each((sel) => {
			/** @type {Set<import('postcss-selector-parser').Selector>} */ let replaced =
				new Set();
	...
	for (let apply of applies) {
		...
		if (apply.parent.type === "atrule") {
			if (apply.parent.name === "screen") {
				...
				throw apply.error(
					`@apply is not supported within nested at-rules like @screen. We suggest you write this as @apply ${applyCandidates.map((c) => `${screenType}:${c}`).join(" ")} instead.`,
				);
			}
			throw apply.error(
		...
		for (let applyCandidate of applyCandidates) {
			if (
				[prefix(context, "group"), prefix(context, "peer")].includes(
					applyCandidate,
				)
			) {
				...
				if (canRewriteSelector) {
					root.walkRules((rule) => {
						...
						if (importantSelector && parentSelector !== parent.selector) {
							rule.selector = (0,
							_applyImportantSelector.applyImportantSelector)(
								rule.selector,
								importantSelector,
							);
						}
						...
						let selector = (0, _postcssselectorparser.default)().astSync(
							rule.selector,
						);
