...
const _bigSign = /*#__PURE__*/ _interop_require_default(
	require("../util/bigSign"),
);
...
class Offsets {
	 ...
	 */ compare(a, b) {
		...
		for (let aOptions of a.options) {
			for (let bOptions of b.options) {
				...
				var _max;
				let maxFnVariant =
					(_max = max([aOptions.variant, bOptions.variant])) !== null &&
					_max !== void 0
						? _max
						: 0n;
	 ...
	 */ recalculateVariantOffsets() {
		...
		/** @type {[bigint, bigint][]} */ let mapping = variants.map(
			([, oldOffset], i) => [oldOffset, newOffsets[i]],
		);
	 ...
	 */ sortArbitraryProperties(list) {
		...
		return list.map((item) => {
			...
			offset = {
				...offset,
				propertyOffset:
					(_offsets_get = offsets.get(offset.property)) !== null &&
					_offsets_get !== void 0
						? _offsets_get
						: 0n,
			};
