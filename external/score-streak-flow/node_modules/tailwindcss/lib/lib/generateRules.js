...
function applyImportant(matches, classCandidate) {
	...
	for (let [meta, rule] of matches) {
		...
		container.walkRules((r) => {
			...
			ast.each((sel) =>
				(0, _formatVariantSelector.eliminateIrrelevantSelectors)(
					sel,
					classCandidate,
				),
			);
...
function applyVariant(variant, matches, context) {
	...
	// Retrieve "modifier"
	{
		...
		_splitAtTopLevelOnly.splitAtTopLevelOnly)(variant, "/");
	...
	if (isArbitraryValue(variant) && !context.variantMap.has(variant)) {
		...
		let selectors = (0, _splitAtTopLevelOnly.splitAtTopLevelOnly)(
			selector,
			",",
		);
	...
	if (context.variantMap.has(variant)) {
		...
		let internalFeatures =
			(_context_variantOptions_get_INTERNAL_FEATURES =
				(_context_variantOptions_get = context.variantOptions.get(variant)) ===
					null || _context_variantOptions_get === void 0
					? void 0
					: _context_variantOptions_get[
		...
		for (let [meta, rule] of matches) {
			...
			for (let [
				...
				function prepareBackup() {
					...
					clone.walkRules(
						(rule) => (rule.raws.originalSelector = rule.selector),
					);
				}
				...
				let withOffset = [
					{
						...
						collectedFormats: ((_meta_collectedFormats =
							meta.collectedFormats) !== null &&
...
function extractArbitraryProperty(classCandidate, context) {
	...
	let [, property, value] =
		(_classCandidate_match = classCandidate.match(
			/^\[([a-zA-Z0-9-_]+):(\S+)\]$/,
		)) !== null && _classCandidate_match !== void 0
		...
		match[1].raws.tailwind = {
			...
			preserveSource:
				(_match__options_preserveSource =
					(_match__options = match[0].options) === null ||
					_match__options === void 0
						? void 0
	...
	for (let matchedPlugins of resolveMatchedPlugins(classCandidate, context)) {
			...
			if (matchesPerPlugin.length > 0) {
				...
				let matchingTypes = Array.from(
					(0, _pluginUtils.getMatchingTypes)(
						(_sort_options_types =
							(_sort_options = sort.options) === null ||
							_sort_options === void 0
								? void 0
								: _sort_options.types) !== null &&
							_sort_options_types !== void 0
							? _sort_options_types
							: [],
						modifier,
						(_sort_options1 = sort.options) !== null &&
							_sort_options1 !== void 0
							? _sort_options1
							: {},
						context.tailwindConfig,
					),
		...
		if (isArbitraryValue(modifier)) {
			if (matches.length > 1) {
				...
				let fallback =
					(_findFallback = findFallback(withoutAny)) !== null &&
					_findFallback !== void 0
						? _findFallback
				...
				if (fallback) {
					...
					let typesPerPlugin = matches.map(
						(match) =>
							new Set([
								...((_typesByMatches_get = typesByMatches.get(match)) !==
									null && _typesByMatches_get !== void 0
									? _typesByMatches_get
									: []),
							]),
					);
			...
			matches = matches.map((list) =>
				list.filter((match) => isParsableNode(match[1])),
			);
		}
...
function applyFinalFormat(match, { context, candidate }) {
	...
	let finalFormat;
	try {
		finalFormat = (0, _formatVariantSelector.formatVariantSelector)(
	...
	container.walkRules((rule) => {
		...
		try {
			let selector = (0, _formatVariantSelector.finalizeSelector)(
				rule.selector,
				finalFormat,
				{
					candidate,
...
function getImportantStrategy(important) {
	...
	if (typeof important === "string") {
		return (rule) => {
			...
			rule.selectors = rule.selectors.map((selector) => {
				return (0, _applyImportantSelector.applyImportantSelector)(
					selector,
					important,
				);
			});
	...
	for (let candidate of candidates) {
		...
		let rules =
			(_context_candidateRuleCache_get =
				context.candidateRuleCache.get(candidate)) !== null &&
