...
const _normalizepath = /*#__PURE__*/ _interop_require_default(
	require("normalize-path"),
);
const _parseGlob = require("../util/parseGlob");
const _sharedState = require("./sharedState");
const _log = /*#__PURE__*/ _interop_require_default(require("../util/log"));
const _micromatch = /*#__PURE__*/ _interop_require_default(
	require("micromatch"),
);
...
function parseCandidateFiles(context, tailwindConfig) {
	...
	for (const task of tasks) {
		included.push(
			...task.positive.map((filePath) => parseFilePath(filePath, false)),
		);
		excluded.push(
			...task.negative.map((filePath) => parseFilePath(filePath, true)),
		);
	}
...
function resolvedChangedContent(context, candidateFiles, fileModifiedMap) {
	...
	let [changedFiles, mTimesToCommit] = resolveChangedFiles(
		candidateFiles,
		fileModifiedMap,
	);
...
function createBroadPatternCheck(paths) {
	 ...
	 */ return (file) => {
		...
		let relativeMatchingGlob = _path.default.relative(
			process.cwd(),
			matchingGlob,
		);
		if (relativeMatchingGlob[0] !== ".")
			relativeMatchingGlob = `./${relativeMatchingGlob}`;
		let largeDirectory = LARGE_DIRECTORIES.find((directory) =>
