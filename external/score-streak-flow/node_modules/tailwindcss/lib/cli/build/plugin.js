...
const _postcssloadconfig = /*#__PURE__*/ _interop_require_default(
	require("postcss-load-config"),
);
const _lilconfig = require("lilconfig");
const _plugins = /*#__PURE__*/ _interop_require_default(
	require("postcss-load-config/src/plugins"), // Little bit scary, looking at private/internal API
);
const _options = /*#__PURE__*/ _interop_require_default(
	require("postcss-load-config/src/options"), // Little bit scary, looking at private/internal API
);
...
const _resolveConfig = /*#__PURE__*/ _interop_require_default(
	require("../../../resolveConfig.js"),
);
 ...
 */ async function loadPostCssPlugins(customPostCssPath) {
	let config = customPostCssPath
		? await (async () => {
				...
				// @ts-ignore
				let { config = {} } = await (0, _lilconfig.lilconfig)("postcss").load(
					file,
				);
	...
	let configPluginTailwindIdx = configPlugins.findIndex((plugin) => {
		...
		if (
			...
			plugin !== null &&
			plugin.postcssPlugin === "tailwindcss"
		) {
			return true;
		}
		return false;
	...
	let beforePlugins =
		configPluginTailwindIdx === -1
			? []
			: configPlugins.slice(0, configPluginTailwindIdx);
	let afterPlugins =
		configPluginTailwindIdx === -1
			? configPlugins
...
let state = {
	...
	readContentPaths() {
		...
		let checkBroadPattern = (0, _content.createBroadPatternCheck)(
			this.contentPatterns.all,
		);
	...
	getContext({ createContext, cliConfigPath, root, result, content }) {
		...
		let configPath =
			(_findAtConfigPath1 = (0, _findAtConfigPath.findAtConfigPath)(
				root,
				result,
			)) !== null && _findAtConfigPath1 !== void 0
				? _findAtConfigPath1
				: cliConfigPath;
...
async function createProcessor(args, cliConfigPath) {
	...
	let content =
		(_args_content_split =
			(_args_content = args["--content"]) === null || _args_content === void 0
				? void 0
				: _args_content.split(/(?<!{[^}]+),/)) !== null &&
		_args_content_split !== void 0
			? _args_content_split
			: [];
	let tailwindPlugin = () => {
		return {
	...
	async function readInput() {
		...
		if (input) {
			return _fs.default.promises.readFile(
				_path.default.resolve(input),
				"utf8",
			);
		}
	...
	async function build() {
		...
		return readInput()
			...
			.then((result) => {
				...
				_sharedState.env.DEBUG &&
					console.time("Recording PostCSS dependencies");
				...
				_sharedState.env.DEBUG &&
					console.timeEnd("Recording PostCSS dependencies");
			...
			state.watcher = (0, _watching.createWatcher)(args, {
				 ...
				 */ async rebuild(changes) {
					let needsNewContext = changes.some((change) => {
						...
						return (
							((_state_configBag = state.configBag) === null ||
							_state_configBag === void 0
								? void 0
