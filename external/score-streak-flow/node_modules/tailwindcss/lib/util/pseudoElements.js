 ...
 */ function movablePseudos(sel) {
	...
	for (let node of sel.nodes) {
		if (node.type === "combinator") {
			buffer = buffer.filter(([, node]) =>
				propertiesForPseudo(node).includes("jumpable"),
			);
			...
			if (isMovablePseudoElement(node)) {
				...
				lastSeenElement &&
				isAttachablePseudoClass(node, lastSeenElement)
			) {
			...
			var _node_nodes;
			for (let sub of (_node_nodes = node.nodes) !== null &&
			_node_nodes !== void 0
				? _node_nodes
				: []) {
 ...
 */ function isPseudoElement(node) {
	return (
		node.value.startsWith("::") || elementProperties[node.value] !== undefined
	);
}
/**
 * @param {Node} node
 ...
 */ function isMovablePseudoElement(node) {
	return (
		isPseudoElement(node) && propertiesForPseudo(node).includes("terminal")
	);
}
/**
 * @param {Node} node
 ...
 */ function propertiesForPseudo(pseudo) {
	...
	return (_elementProperties_pseudo_value = elementProperties[pseudo.value]) !==
		null && _elementProperties_pseudo_value !== void 0
