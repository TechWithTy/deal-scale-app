...
function normalize(value, context = null, isRoot = true) {
	let isVarException =
		context && AUTO_VAR_INJECTION_EXCEPTIONS.has(context.property);
	...
	return value.replace(/(calc|min|max|clamp)\(.+\)/g, (match) => {
		...
		for (let i = 0; i < match.length; i++) {
			...
			if (peek("var")) {
				...
				let keyword = preventFormattingKeywords.find((keyword) =>
					peek(keyword),
				);
				result += keyword;
				i += keyword.length - 1;
...
function shadow(value) {
	let parsedShadows = (0, _parseBoxShadowValue.parseBoxShadowValue)(
		normalize(value),
	);
...
function color(value) {
	let colors = 0;
	let result = (0, _splitAtTopLevelOnly.splitAtTopLevelOnly)(value, "_").every(
		(part) => {
			part = normalize(part);
...
function image(value) {
	let images = 0;
	let result = (0, _splitAtTopLevelOnly.splitAtTopLevelOnly)(value, ",").every(
		(part) => {
			part = normalize(part);
			if (part.startsWith("var(")) return true;
			if (
				url(part) ||
				gradient(part) ||
				["element(", "image(", "cross-fade(", "image-set("].some((fn) =>
					part.startsWith(fn),
				)
			) {
				images++;
