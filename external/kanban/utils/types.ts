...
export interface KanbanTask {
	...
	status: Status;
	priority?: Priority;
	// New: scheduled date displayed before due date
	scheduledDate?: string;
	// Optional IANA timezone for scheduledDate (e.g., "America/Denver")
	scheduledTimezone?: string;
	dueDate?: string;
	appointmentDate?: string;
	appointmentTime?: string;
	// Optional IANA timezone for appointment (e.g., "America/Denver")
	...
	// New optional AI outputs
	outputAttachments?: { filename: string; url: string }[]; // AI-generated files to download
	outputImageUrl?: string; // image to show when AI task completed
	outputMarkdown?: string; // markdown content rendered when AI task completed
	...
	// Optional credit metadata for AI/lead/skip-trace tasks
	costType?: "ai" | "leads" | "skipTraces"; // which credit bucket to use
	costAmount?: number; // default 1
}
